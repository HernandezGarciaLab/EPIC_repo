/*
 * GE Medical Systems
 * Copyright (C) 1993-2004 The General Electric Company
 *
 * 2dfast.e
 *
...
@inline MK_GradSpec.e GspecGlobal
...
int phaseres = 128 with {1, 1024, 128, INVIS, "Number of Points in the view table",};

/* CVs for oddnex cases. MHN */
int oddnex;                      /* flag for oddnex case */
int oddnex_npw;                  /* flag for oddnex and npw case */
int evennex_npw;                 /* flag for evennex(6, 10, 14 etc.) 
                                    and npw case */
int vwchp = PSD_OFF with {,,PSD_OFF, INVIS, "Do view rf choping(=1)",};
/* YMSmr06759 */
int tmp_nopwrap = PSD_OFF;
int true_nopwrap = PSD_OFF;
...
cvinit(void)
{
     ...
     */
    if( !strncasecmp( "tx_therm", psd_name, 8 ) ) {
...
{     
    ...
    if(!strncmp("2dfast_silent",psd_name,13))
    {
...
@inline Prescan.e PScvinit
    ...
    bw_mods = 1;	/* ok to modify bandwidth characterstics on first pass */
...
%endif /* PSD_CFH_MT */
    ...
    ampslice(&targ_slthick, bw_rf1, loggrd.tz, gscale_rf1,TYPDEF);
...
@inline AutoAdjustTR.e AutoAdjustTRDebug
    ...
    if (exist(opsquare) == PSD_ON)
        piphasfovnub = 0;
    else if ((exist(opnopwrap) == PSD_ON) && (rectfov_npw_support == PSD_OFF))
        piphasfovnub = 0;
    else
        piphasfovnub = 7;
...
@inline AutoAdjustTR.e AutoAdjustTRInit
    ...
    /* nextemp is declared as an integer.
       The intent is to guard against odd nex or fractional
       nex other than .5, .75, 1.5 */
    nextemp = (int)(exist(opnex));
    rhhnover = 0; /* init */
    fn = 1; /* init */
    nex = 1;

    if ( exist(opnopwrap) == PSD_ON
         || ((exist(opexor) == PSD_ON || cmon_flag == PSD_ON)
             && exist(opnex) > 1.4))
    {
        nop = 2;
    }
    ...
    tmp_nopwrap = exist(opnopwrap);

    /*MRIge35061 - 9/11/96 - VB*/ /*MRIgw35599*/
    if((exist(opexor) == PSD_ON) && existcv(opexor) && exist(opnex) > 1.4)
    {
        _opnopwrap.fixedflag = 0;
        opnopwrap = 1;
        _opnopwrap.existflag = 1;
        _opnopwrap.fixedflag = 1;
    }

    /* YMSmr06759 */
    if(epic_findputcvnum("opnopwrap") == SUCCESS)
    {
        if((exist(opnopwrap) == PSD_ON) && (tmp_nopwrap == PSD_ON))
        {
            true_nopwrap = PSD_ON;
        }
        else
        {
            true_nopwrap = PSD_OFF;
        }
    }

    cvmin(opnex,0); /* YMSmr06768 Aug 2005 HK */

    if( (((exist(opnex) == .50) && (existcv(opnex))) ||
        ((exist(opnex) == 1.0) && (existcv(opnex)))) &&
         (true_nopwrap == PSD_ON) )
    {
        /* YMSmr06768 Aug 2005 HK */
        if((exist(opnex) == 1.0) && (true_nopwrap))
        {
            cvmin(opnex,1.5);
        }

        epic_error( use_ermes, "Gradient recalled scans do not support .5 nex or 1 nex scans with No Phase Wrap.", EM_PSD_HALFNEX_INCOMPATIBLE, EE_ARGS(0) );
        return FAILURE;
    }
    else if( exist(opnex) == 0.75 )
    {
        if( true_nopwrap == PSD_ON )
        {
            cvmin(opnex,1.5); /* YMSmr06768 Aug 2005 HK */
    ...
    **************************************************************************/
    ...
    prerf_ssptime = RUP_GRD((int)(DAB_length[bd_index]+rffrequency_length[bd_index]-rfupa));
    ...
    if (cine_flag == PSD_ON)
    {
        pos_DAB = ( RUP_GRD((int)(GRAD_UPDATE_TIME + tlead +
                    ...
                    + rfupd + psd_rf_wait + rfunblank_length[bd_index] );
    }
    ...
    if ( ((exist(opnopwrap) == PSD_ON) && existcv(opnopwrap) && !rectfov_npw_support) || 
         ((exist(opsquare) != PSD_ON) && (piphasfovnub == 0) && rectfov_npw_support) )
    {
        rhphasescale = 1.0;
              ...
              && ( (exist(opsquare) != PSD_ON) && existcv(opsquare) ) )
    {
        ...
        if((eg_phaseres == exist(opxres)) && (eg_phaseres != exist(opyres) ) ) { 
            /* MRIge71078 - changing from sqPixel ON to OFF, restoring opphasefov */
...
%endif
    ...
    if (((oddnex == PSD_ON) || (oddnex_npw == PSD_ON) || (evennex_npw == PSD_ON)
         || (nex < 2)) && cine_flag)
        choplet = PSD_ON; 
    else 
        choplet = PSD_OFF;
...
%endif
    ...
    /* AMR - FOR ZIP512 - Additions */
    if ( ((exist(opzip512) == PSD_ON) && (existcv(opzip512)) ) || (exist(opxres) > 256) ) {
        rhrcxres = 512;           /* 512 ZIP or standard 512 recon */
    }
    else {
        rhrcxres = 256;             /* Standard 256 recon */
    }
    ...
    /* FOR ZIP512 - reuse existing code */
    if(rectfov_npw_support == PSD_OFF){
        rhrcyres = (int)(rhrcxres * (1 + opnopwrap)*(exist(opphasefov)));
    } else {
        rhrcyres = (int)(rhrcxres * nop);
    }
...
@inline Prescan.e PScveval
    ...
    avepepowscale(&ave_grady_gy1_scale, phaseres,
                  (int)(rhnframes-rhnframes/(2.0*fn))+rhhnover);
...
@inline AutoAdjustTR.e PulsegenonhostSwitch
    ...
    if (cine_flag == PSD_ON || mpgr_flag == PSD_ON)
        dda = IMax(2, 4, (( ((int) (3s/act_tr)) + 2)/2) * 2);
    ...
    else
    {
        ...
        fullcyc_tmp = (int)floor((double)((avmaxtr_tmp/ 1000000.0)
                                          / (1 / cflinfrq)));
...
%endif
    ...
    /* use opnopwrap to decide on the buttons due to the no phase wrap
       inherent in exorcist scans at 2 and 4 nex */
    if( cine_flag == PSD_OFF )
    {
        if (exist(opnopwrap) == PSD_ON) 
        {
            /* Don't show fract nex buttons */
            pinexnub = 1 + 8 + 16 + 32; 
            pinexval4 = 1.5;
            pinexval5 = 2;
            pinexval6 = 3;
        ...
        else if( (exist(opexor) == PSD_ON) || (cmon_flag == PSD_ON) ) /* no 3 nex button*/
        {
            if( cmon_flag == PSD_ON )
            {
                pinexnub = 1 + 2 + 4 + 8;
    ...
    else
    {
        ...
        if( cmon_flag == PSD_ON )
        {
            pinexnub = pinexnub - 32;  /* don't show NEX = 4 */
        } 
    }
...
#include "predownload.in"
    ...
    if (opcgate == PSD_ON) {
        ...
        */
        ...
        ps2_dda = IMax(2,2,(int)(((int)(2.0s/act_tr) + 1)/2)*2);
        if (optdel1 < pitdel1) {
    ...
    else
    {
        ...
        if ( existcv(opmt) && (exist(opmt) == PSD_ON) ) {
            ...
            ps2_dda = IMax(2,2,(int)(((int)(2.0s/exist(optr)) + 1)/2)*2);
...
nexcalc( void )
{
    ...
    if (fn == 1)
    {
        if ( (((int)exist(opnex))%2) && (exist(opnex) > 2) && (nop == 2) )
    ...
    /* Flag setting for Odd Nex. MHN */
    if ( (((int)exist(opnex))%2) && (exist(opnex) > 2) && (nop == 1) )
    {
        oddnex = PSD_ON;
    }
    ...
    else if ( (nex%2) && (exist(opnex)>5) && ( ((int)exist(opnex))%2 == 0) )
    {
        evennex_npw = PSD_ON;
    }

    /* In oddnex npw, last excitation covers only half k-space. MHN */
    if (oddnex_npw == 1)
...
@inline MK_GradSpec.e GspecCheck
    ...
    if ((exist(opnex) == 1.5) && (exist(opnopwrap)==PSD_OFF) && existcv(opnex))
    {
        epic_error( use_ermes, "1.5 NEX is only valid when No Phase Wrap is selected.", EM_PSD_15NEX_INCOMPATIBLE, EE_ARGS(0) );
        return FAILURE;
    }

    if (exist(opnex) == 0 && existcv(opnex)) {   /* guard against zero nex */
    ...
    if ((exist(opxres) > 512) || (exist(opxres) < 128)) {
    ...
    if ( (existcv(opgradmode) && (exist(opgradmode) == 1) && strcmp("2dfast_service",psd_name)) || existcv(opgradmode)==0 ) {
    ...
    if (fn != 1 && cine_flag == PSD_ON && existcv(opnex))
    {
    ...
    if ((fn == 0.5) && (exist(opte)< min_tenfe) && existcv(opnex))
    {
        temp_int = min_tenfe;
        advroundup(&temp_int);
    ...
    if ((fn == 0.75) && (exist(opte)< min_tenfe) && existcv(opnex))
    {
        temp_int = min_tenfe;
        advroundup(&temp_int);
    ...
    if ( (opexor == PSD_ON) && ( (oddnex) || (oddnex_npw) || (evennex_npw) ) )
    {
    ...
    if (cardcv)
    {
        ...
        if (seq_type == TYPMPMP)
        {
            ...
            if ((phasesPerLoc != 1.0) && (phasesPerLoc != 2.0) &&
                (phasesPerLoc != 3.0) && (exist(opslquant) > 1)) {
    ...
    if ( exist(opexor) && (nop!=1) && existcv(opnex) && (rectfov_npw_support == PSD_OFF) )
    {
        if ( (exist(opsquare) == PSD_ON) && existcv(opsquare) )
        {
            epic_error( use_ermes, "Resp. Comp. when used with Square Pixel will not allow NEX greater than 1.", EM_PSD_RESP_SQPXL_INCOMPATIBLE, EE_ARGS(0) );
            /* 4/21/96 RJL - Set values to be placed into Popup List as event handler */
            avminnex = 1.0;
            avmaxnex = 1.0;
            cvoverride(opnopwrap, PSD_OFF, PSD_FIX_OFF, PSD_EXIST_OFF);
            return ADVISORY_FAILURE;
        }
        else if ( (exist(opphasefov) != 1.0) && existcv(opphasefov) &&
                  (exist(opsquare) == PSD_OFF) && existcv(opsquare) )
        {
            epic_error( use_ermes, "Resp. Comp. when used with Variable FOV will not allow NEX greater than 1.", EM_PSD_RESP_VARFOV_INCOMPATIBLE, EE_ARGS(0) );
            /* 4/21/96 RJL - Set values to be placed into Popup List as event handler */
            avmaxphasefov = 1.0;
            avminphasefov = 1.0;
            avminnex = 1.0;
    ...
    if ( (exist(opnopwrap) == PSD_ON) && (rectfov_npw_support == PSD_OFF))
    {
        if ( (exist(opsquare) == PSD_ON) && existcv(opsquare) )
        {
            epic_error( use_ermes, "No Phase Wrap cannot be performed with Square Pixel.", EM_PSD_NPW_SQPXL_INCOMPATIBLE, EE_ARGS(0) );
            return FAILURE;
        }
        else if ( (exist(opphasefov) != 1.0) && existcv(opphasefov) &&
                  (exist(opsquare) == PSD_OFF) && existcv(opsquare) )
        {
            epic_error( use_ermes, "No Phase Wrap cannot be performed with Variable FOV.", EM_PSD_NPW_VARFOV_INCOMPATIBLE, EE_ARGS(0) );
            /* 4/21/96 RJL - Set values to be placed into Popup List as event handler */
            avmaxphasefov = 1.0;
            avminphasefov = 1.0;
            return ADVISORY_FAILURE;
    ...
    ******************************************************/
    ...
    if(exist(opcmon) == PSD_ON)
    {
        ...
        /* check for Exorcist and odd/even nex NPW. MHN */
        if ((oddnex) || (oddnex_npw) || (evennex_npw))
        {
        ...
        if ( (nop!=1) && existcv(opnex) && (rectfov_npw_support == PSD_OFF) )
        {
            if ( (exist(opsquare) == PSD_ON) && existcv(opsquare) )
            {
                epic_error( use_ermes, "Cardiac Compensation, when used with square pixel will not allow NEX greater than 1.", EM_PSD_CMON_SQRPIXEL_NEX_INCOMPATIBLE, EE_ARGS(0) );
                avminnex = 1 ;
                avmaxnex = 1 ;
                return ADVISORY_FAILURE;
            }
            ...
            {
                epic_error( use_ermes, "Cardiac Compensation, when used with variable FOV will not allow NEX greater than 1.", EM_PSD_CMON_VFOV_NEX_INCOMPATIBLE, EE_ARGS(0) );
                avmaxphasefov = 1.0;
                avminphasefov = 1.0;
                avminnex = 1;
                avmaxnex = 1;
...
@inline loadrheader.e rheaderinit
    ...
    /*** use nop to calculate rhrcyres instead of turning opnopwrap ON 
         internally for cmon case.  -- RJF, MRIge41778 **/

    if ((cmon_flag == PSD_ON) && (rectfov_npw_support == PSD_OFF))
    {
        rhrcyres =  (int)(rhimsize * nop * (exist(opphasefov)));
    }
    ...
    if( (cine_flag == PSD_ON) && ((oddnex == 1) || (oddnex_npw ==1 ) ||
                                  (evennex_npw ==1)) )
    {
    ...
    if (cine_flag == PSD_ON)
    {
        ...
        if( (nex == 1) || (oddnex == 1) || (oddnex_npw == 1) || (evennex_npw == 1) )
        {
            /* YMSmr06635 */
            if((exist(opasset) > PSD_OFF) || (exist(opfatwater) > PSD_OFF) ||
               (exist(opphasefov) == 1.0) || (nop > 1) ){

                rhrcyres = (int)(rhimsize * nop);
            ...
            */
            ...
            if (opexor == PSD_ON || cmon_flag == PSD_ON)
            {
                ...
                cine_iamp = (SHORT)viewtable[ (cine_nview == 0 ? 1 : 
                                               ( (rspnex_int == 4)?(cine_nview+1)/2:cine_nview )) ];
                ...
                if (oddnex_npw == 1) 
                {
                    if (cine_pview <= opyres*rspnop_int*(nex-1))
                    {
                    ...
                    {
                        oddnex_cineview = cine_pview - opyres*rspnop_int*(nex-1) +
                            (opyres*rspnop_int)/4;
                    }

                    if (cine_nview <= opyres*rspnop_int*(nex-1)) 
                    {
                        cine_iamp = (SHORT)viewtable[ (cine_nview == 0 ? 1 : 
                    ...
                    {
                        cine_nview = cine_nview - opyres*rspnop_int*(nex-1) +
                            (opyres*rspnop_int)/4; 
                ...
                else if ((evennex_npw ==1) || (oddnex == 1))
                {
                    oddnex_cineview =  (cine_pview == 0 ? 1 : 
...
%endif
            if(cineseqinfo.c_send == 1 ) {
                ...
                if (((oddnex_npw == 1) || (evennex_npw ==1) || (oddnex ==1))
                    && (opexor == 0) )
                {
...
psdinit(void) 
{
    ...
    if ( (nex==1) || (oddnex==1) || (evennex_npw==1) || (oddnex_npw==1) )
        vwchp = PSD_ON;
    if ( ((oddnex == 0) && (oddnex_npw == 0) && (evennex_npw == 0)) ||
...
mps2(void) 
{
    ...
    if (opcgate == PSD_ON) {
        ...
        xrr_trig_time = .01 * oparr * (60.0/ophrate)* 1e6;
