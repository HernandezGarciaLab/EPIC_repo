...
int  	myramptime = 40;  /* lhg 4/12/18   ramp times for the pcasl pulses */
...
abstract("Real Time, interactive, Pseudo CASL with 3D, Spiral acquisition for fMRI sequence with GRAPPA-Z, optimizedi BGS");
psdname("pcasl3d11");
...
2*pw_gzspoila);
	...
	t_adjust = RUP_GRD(optr - psdseqtime*opslquant - t_tag - t_delay - tpre - t_preBS);                 /* LHG 11.29.10 */
...
STATUS pulsegen(void)
{
	...
	fprintf(stderr, "\npulsegen: generating refocuser gzrf1r at ... %d ", pend(&gzrf1d, "gzrf1d",0) );
	fprintf(stderr, "\npulsegen: generating refocuser gzrf1r at ... %d ", pendall(&gzrf1, 0) + pw_gzrf1ra);
	TRAPEZOID(ZGRAD,
			gzrf1r,
	...
	EXTWAVE(RHO,
			rftag2,
			RUP_GRD(t_rfrepeat + pw_gzrftag2a + 0),
			pw_gzrftag2,
			a_rftag2,
			res_rftag2,
			/usr/g/bin/myhanning.rho,
	...
	fprintf(stderr," start: %d	end: %d", 
			RUP_RF(BS1_time - pw_BS1rf/2 ),
			RUP_RF(BS1_time - pw_BS1rf/2 + pw_BS1rf   ));
	/*
	...
	EXTWAVE(RHO,
			BS2rf,
			RUP_RF(BS1_time +  BS2_time - pw_BS2rf/2 ),
	...
	fprintf(stderr," start: %d	end: %d", 
			RUP_RF(BS1_time +  BS2_time - pw_BS2rf/2 ),
			RUP_RF(BS1_time +  BS2_time - pw_BS2rf/2 + pw_BS2rf));
	/*
	...
	SINC2(RHO, 
			rf0,  
			RUP_GRD(t_delay - fatsattime  ), 
			pw_rf0, 
			a_rf0,
			,0.5,,,loggrd);
	fprintf(stderr," start: %d	end: %d", 
			RUP_GRD(t_delay - fatsattime  ), 
			RUP_GRD(t_delay - fatsattime + pw_rf0 )); 
