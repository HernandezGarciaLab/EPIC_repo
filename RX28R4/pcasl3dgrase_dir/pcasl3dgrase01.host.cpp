...
#include "pcasl3dgrase01.hostdecl.h"
#include "pcasl3dgrase01.exportstr.h"
#include "pcasl3dgrase01.runtime.h"
...
#include "cvinit.in"	/* Runs the code generated by macros in preproc.*/
	...
	cvmax(oprbw, 250);
	cvdef(oprbw, 200);
	oprbw  = _oprbw.fixedflag ?  ((void)(200), oprbw) : 200;
...
int cveval()
{
	...
	opflip  = _opflip.fixedflag ?  ((void)(90.0), opflip) : 90.0;
	cvdef(opflip, 90.0);
	if (oppseq == 2) /* gradient echo */
	{
		pifanub = 6;
	...
	/* label FOV buttons */
	cvdef(opfov, 200);
	opfov  = _opfov.fixedflag ?  ((void)(200), opfov) : 200;
	pifovval2 = 200;
	pifovval3 = 220;
	pifovval4 = 240;
	pifovval5 = 360;
	...
	cvdef(opuser0, 1.0);
	opuser0  = _opuser0.fixedflag ?  ((void)(2), opuser0) : 2;
	cvmin(opuser0, 1.0);
	cvmax(opuser0, 128.0);
	nl  = _nl.fixedflag ?  ((void)(opuser0), nl) : opuser0;

	cvdesc(opuser1, "number of temporal frames");
	cvdef(opuser1, 1);
	opuser1  = _opuser1.fixedflag ?  ((void)(16), opuser1) : 16;
	cvmin(opuser1, 1);
	cvmax(opuser1, 16384);
	nframes  = _nframes.fixedflag ?  ((void)(opuser1), nframes) : opuser1;

	cvdesc(opuser2, "Ext trig: (0)none, start (1)scan, (2)frame, (3)by file");
	cvdef(opuser2, 0);
	opuser2  = _opuser2.fixedflag ?  ((void)(0), opuser2) : 0;
	cvmin(opuser2, 0);
	cvmax(opuser2, 3);
    	gating  = _gating.fixedflag ?       ((void)((((int)opuser2)==1||((int)opuser2)==2)?TRIG_AUX:TRIG_INTERN), gating) : (((int)opuser2)==1||((int)opuser2)==2)?TRIG_AUX:TRIG_INTERN;        /* aux trig */
	...
	cvmin(opuser4, 0);
	cvmax(opuser4, 1);
	scluster  = _scluster.fixedflag ?  ((void)(opuser4), scluster) : opuser4;

	cvdesc(opuser5, "Number of extra shots before data acq");
	cvdef(opuser5, 2);
	opuser5  = _opuser5.fixedflag ?  ((void)(2), opuser5) : 2;
	cvmin(opuser5, 0);
	cvmax(opuser5, 100);
	nextra  = _nextra.fixedflag ?  ((void)(opuser5), nextra) : opuser5;
	...
	cvmin(opuser6, 0);
	cvmax(opuser6, 2);
	gtype  = _gtype.fixedflag ?  ((void)(opuser6), gtype) : opuser6;
	bmapnav  = _bmapnav.fixedflag ?     ((void)((gtype==1)?0:1), bmapnav) : (gtype==1)?0:1;		/* for now */

	cvdesc(opuser10, "Record physio data (1) or not(0)");
	cvdef(opuser10, 0);
	opuser10  = _opuser10.fixedflag ?  ((void)(0), opuser10) : 0;
	cvmin(opuser10, 0);
	cvmax(opuser10, 1);
	...
	cvmax(opuser11,1);
	cvdef(opuser11,0);
	opuser11  = _opuser11.fixedflag ?  ((void)(0), opuser11) : 0;

	domap  = _domap.fixedflag ?  ((void)(opuser11), domap) : opuser11;


	cvdesc(opuser12, "short rf pulse (1) or not(0)");
	cvdef(opuser12, 1);
	opuser12  = _opuser12.fixedflag ?  ((void)(1), opuser12) : 1;
	cvmin(opuser12, 0);
	cvmax(opuser12, 1);
	if(((int)opuser12) == 1)  {
		cycrf1  = _cycrf1.fixedflag ?  ((void)(2), cycrf1) : 2;
		pwrf1  = _pwrf1.fixedflag ?  ((void)(3200), pwrf1) : 3200;
		res_rf1  = _res_rf1.fixedflag ?  ((void)(1600), res_rf1) : 1600;
		res_rf2  = _res_rf2.fixedflag ?  ((void)(1600), res_rf2) : 1600;
	}  else  {
		cycrf1  = _cycrf1.fixedflag ?  ((void)(4), cycrf1) : 4;
		pwrf1  = _pwrf1.fixedflag ?  ((void)(6400), pwrf1) : 6400;
	}

	cvdesc(opuser13, "vd (1) or cd (0)");
	cvdef(opuser13, 0);
	opuser13  = _opuser13.fixedflag ?  ((void)(0), opuser13) : 0;
	cvmin(opuser13, 0);
	cvmax(opuser13, 1);
	vdflag  = _vdflag.fixedflag ?  ((void)(opuser13), vdflag) : opuser13;

	cvdesc(opuser14, "alpha");
	cvdef(opuser14, 3.6);
	opuser14  = _opuser14.fixedflag ?  ((void)(3.6), opuser14) : 3.6;
	cvmin(opuser14, 1.01);
	cvmax(opuser14, 10);
	alpha  = _alpha.fixedflag ?  ((void)(opuser14), alpha) : opuser14;

	cvdesc(opuser15, "kmaxfrac");
	cvdef(opuser15, 0.5);
	opuser15  = _opuser15.fixedflag ?  ((void)(0.5), opuser15) : 0.5;
	cvmin(opuser15, 0.05);
	cvmax(opuser15, 0.95);
	kmaxfrac  = _kmaxfrac.fixedflag ?  ((void)(opuser15), kmaxfrac) : opuser15;

	cvdesc(opuser16, "Optimize Phase Correction ? (will search in steps of 0.2 rads)");
	cvdef(opuser16, 0);
	opuser16  = _opuser16.fixedflag ?  ((void)(0), opuser16) : 0;
	cvmin(opuser16, 0.);
	cvmax(opuser16, 1.);
	multiphsFlag  = _multiphsFlag.fixedflag ?  ((void)(opuser16), multiphsFlag) : opuser16;
	 ...
	 */
	slord  = _slord.fixedflag ?  ((void)(TYPNORMORDER), slord) : TYPNORMORDER;
	...
	my_maxB1Seq  = _my_maxB1Seq.fixedflag ?    ((void)(1000*maxB1Seq), my_maxB1Seq) : 1000*maxB1Seq;
	...
	cvmax(opuser20,8000);
	cvdef(opuser20,3000);
	opuser20  = _opuser20.fixedflag ?  ((void)(1800), opuser20) : 1800;

	cvdesc(opuser21,"Delay between tagging period and imaging time (ms)");
	cvmin(opuser21,1000);
	cvmax(opuser21, 5000);
	cvdef(opuser21,1200);
	opuser21  = _opuser21.fixedflag ?  ((void)(2000), opuser21) : 2000;

	cvdesc(opuser22,"Dist. from landmark to tagging plane (cm)");
	cvmin(opuser22,-50);
	cvmax(opuser22, 50);
	cvdef(opuser22,8);
	opuser22  = _opuser22.fixedflag ?  ((void)(10), opuser22) : 10;
	...
	cvmax(opuser23, 2*PI);
	cvdef(opuser23,0.0);
	opuser23  = _opuser23.fixedflag ?  ((void)(0.0), opuser23) : 0.0;

	cvdesc(opuser24,"Do GRAPPA-Z (0=NO  1=YES)");
	cvmin(opuser24,0);
	cvmax(opuser24, 1);
	cvdef(opuser24,0);
	opuser24  = _opuser24.fixedflag ?  ((void)(0), opuser24) : 0;

	cvdesc(opuser25,"Control Type (0= Toggle RF, 1= Flip Grads)");
	cvmin(opuser25,0);
	cvmax(opuser25, 1);
	cvdef(opuser25,0);
	opuser25  = _opuser25.fixedflag ?  ((void)(0), opuser25) : 0;

	cvdesc(opuser26,"SE Crusher Type (1= Straddle 180  else  bipolar)");
	cvmin(opuser26,0);
	cvmax(opuser26, 1);
	cvdef(opuser26,0);
	opuser26  = _opuser26.fixedflag ?  ((void)(0), opuser26) : 0;

	cvdesc(opuser27,"# M0frames");
	cvmin(opuser27,0);
	cvmax(opuser27, 4);
	cvdef(opuser27,0);
	opuser27  = _opuser27.fixedflag ?  ((void)(2), opuser27) : 2;
	M0frames  = _M0frames.fixedflag ?  ((void)(opuser27), M0frames) : opuser27;

	cvdesc(opuser28,"x_tof [cm]");
	cvmin(opuser28,-8);
	cvmax(opuser28, 8);
	cvdef(opuser28,0);
	opuser28  = _opuser28.fixedflag ?  ((void)(0), opuser28) : 0;
	x_tof  = _x_tof.fixedflag ?  ((void)(opuser28), x_tof) : opuser28;

	cvdesc(opuser29,"y_tof [cm]");
	cvmin(opuser29,-8);
	cvmax(opuser29, 8);
	cvdef(opuser29,0);
	opuser29  = _opuser29.fixedflag ?  ((void)(0), opuser29) : 0;
	y_tof  = _y_tof.fixedflag ?  ((void)(opuser29), y_tof) : opuser29;

	cvdesc(opuser30,"vessel-selective gradient a_gxyrftag  [cm]");
	cvmin(opuser30,-8);
	cvmax(opuser30, 8);
	cvdef(opuser30,0);
	opuser30  = _opuser30.fixedflag ?  ((void)(0), opuser30) : 0;
	a_gxyrftag  = _a_gxyrftag.fixedflag ?  ((void)(opuser30), a_gxyrftag) : opuser30;

	

	t_tag  = _t_tag.fixedflag ?  ((void)(RUP_GRD(opuser20*1000)), t_tag) : RUP_GRD(opuser20*1000);
	t_delay  = _t_delay.fixedflag ?    ((void)(RUP_GRD(opuser21*1000)), t_delay) : RUP_GRD(opuser21*1000);
	B1tag_distance  = _B1tag_distance.fixedflag ?  ((void)(opuser22-(float)piscancenter/10), B1tag_distance) : opuser22-(float)piscancenter/10;
	mycrush = _mycrush.fixedflag ? ((void)(0), mycrush) : 0;
	doZgrappa  = _doZgrappa.fixedflag ?  ((void)(opuser24), doZgrappa) : opuser24;
	mycontrol = _mycontrol.fixedflag ? ((void)(opuser25), mycontrol) : opuser25;
	crushtype = _crushtype.fixedflag ? ((void)(opuser26), crushtype) : opuser26;
	/* LHG 9/8/14 */
	manual_phase_correction  = _manual_phase_correction.fixedflag ?  ((void)(opuser23), manual_phase_correction) : opuser23;
	...
	/* LHG 11/29/12 
	   ...
	   ======>  3.5592 
	 */
	flip_rf2  = _flip_rf2.fixedflag ?  ((void)(120), flip_rf2) : 120;

	pw_rftag1  = _pw_rftag1.fixedflag ?  ((void)(500), pw_rftag1) : 500;     
	/*pw_rftag1 = 1000; */  /* increased from 500  - LHG 12/7/12 */  

	/*a_rftag1 = (3.5592)*(pcasl_flip/180) * 500/pw_rftag1;*/
	a_rftag1  = _a_rftag1.fixedflag ?  ((void)(0.222), a_rftag1) : 0.222; /* 08/2019 JS scaled based on b1max = 0.2943 G and peak B1 for rftag1 (25deg) = 0.0654 G*/
	a_rftag1  = _a_rftag1.fixedflag ?  ((void)(0.40), a_rftag1) : 0.40; /* 0i7/2021 LHG scaled based on empirical test- pretty sure B1max is 0.147 */
	a_rftag2  = _a_rftag2.fixedflag ?  ((void)(a_rftag1), a_rftag2) : a_rftag1;
	a_rftag1c  = _a_rftag1c.fixedflag ?  ((void)(a_rftag1), a_rftag1c) : a_rftag1;
	a_rftag2c  = _a_rftag2c.fixedflag ?  ((void)(-a_rftag1), a_rftag2c) : -a_rftag1;

	pw_rftag2  = _pw_rftag2.fixedflag ?  ((void)(pw_rftag1), pw_rftag2) : pw_rftag1;
	pw_rftag1c  = _pw_rftag1c.fixedflag ?  ((void)(pw_rftag1), pw_rftag1c) : pw_rftag1;  
	pw_rftag2c  = _pw_rftag2c.fixedflag ?  ((void)(pw_rftag1), pw_rftag2c) : pw_rftag1;

	res_rftag1  = _res_rftag1.fixedflag ?  ((void)(250), res_rftag1) : 250;
	res_rftag2  = _res_rftag2.fixedflag ?  ((void)(res_rftag1), res_rftag2) : res_rftag1;
	res_rftag1c  = _res_rftag1c.fixedflag ?  ((void)(res_rftag1), res_rftag1c) : res_rftag1;
	res_rftag2c  = _res_rftag2c.fixedflag ?  ((void)(res_rftag1), res_rftag2c) : res_rftag1;

	a_gzrftag1  = _a_gzrftag1.fixedflag ?  ((void)(pcasl_Gmax), a_gzrftag1) : pcasl_Gmax; /* G/cm */
	a_gzrftag2  = _a_gzrftag2.fixedflag ?  ((void)(a_gzrftag1), a_gzrftag2) : a_gzrftag1;
	a_gzrftag1c  = _a_gzrftag1c.fixedflag ?  ((void)(a_gzrftag1), a_gzrftag1c) : a_gzrftag1;
	a_gzrftag2c  = _a_gzrftag2c.fixedflag ?  ((void)(a_gzrftag1), a_gzrftag2c) : a_gzrftag1;

	pw_gzrftag1  = _pw_gzrftag1.fixedflag ?  ((void)(pw_rftag1), pw_gzrftag1) : pw_rftag1;
	pw_gzrftag1a  = _pw_gzrftag1a.fixedflag ?  ((void)(myramptime), pw_gzrftag1a) : myramptime;
	pw_gzrftag1d  = _pw_gzrftag1d.fixedflag ?  ((void)(pw_gzrftag1a), pw_gzrftag1d) : pw_gzrftag1a;

	pw_gzrftag1c  = _pw_gzrftag1c.fixedflag ?  ((void)(pw_rftag1), pw_gzrftag1c) : pw_rftag1;
	pw_gzrftag1ca  = _pw_gzrftag1ca.fixedflag ?  ((void)(pw_gzrftag1a), pw_gzrftag1ca) : pw_gzrftag1a;
	pw_gzrftag1cd  = _pw_gzrftag1cd.fixedflag ?  ((void)(pw_gzrftag1a), pw_gzrftag1cd) : pw_gzrftag1a;

	pw_gzrftag2  = _pw_gzrftag2.fixedflag ?  ((void)(pw_rftag1), pw_gzrftag2) : pw_rftag1;
	pw_gzrftag2a  = _pw_gzrftag2a.fixedflag ?  ((void)(pw_gzrftag1a), pw_gzrftag2a) : pw_gzrftag1a;
	pw_gzrftag2d  = _pw_gzrftag2d.fixedflag ?  ((void)(pw_gzrftag1a), pw_gzrftag2d) : pw_gzrftag1a;

	pw_gzrftag2c  = _pw_gzrftag2c.fixedflag ?  ((void)(pw_rftag1), pw_gzrftag2c) : pw_rftag1;
	pw_gzrftag2ca  = _pw_gzrftag2ca.fixedflag ?  ((void)(pw_gzrftag1a), pw_gzrftag2ca) : pw_gzrftag1a;
	pw_gzrftag2cd  = _pw_gzrftag2cd.fixedflag ?  ((void)(pw_gzrftag1a), pw_gzrftag2cd) : pw_gzrftag1a;
	/* refocusers:  */
	pw_gzrftag1r  = _pw_gzrftag1r.fixedflag ?  ((void)(80), pw_gzrftag1r) : 80;  
	pw_gzrftag1ra  = _pw_gzrftag1ra.fixedflag ?  ((void)(myramptime+40), pw_gzrftag1ra) : myramptime+40;  

	pw_gzrftag1rd  = _pw_gzrftag1rd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gzrftag1rd) : pw_gzrftag1ra;

	pw_gzrftag1rc  = _pw_gzrftag1rc.fixedflag ?  ((void)(pw_gzrftag1r), pw_gzrftag1rc) : pw_gzrftag1r;
	pw_gzrftag1rca  = _pw_gzrftag1rca.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gzrftag1rca) : pw_gzrftag1ra;
	pw_gzrftag1rcd  = _pw_gzrftag1rcd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gzrftag1rcd) : pw_gzrftag1ra;

	pw_gzrftag2r  = _pw_gzrftag2r.fixedflag ?  ((void)(pw_gzrftag1r), pw_gzrftag2r) : pw_gzrftag1r;
	pw_gzrftag2ra  = _pw_gzrftag2ra.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gzrftag2ra) : pw_gzrftag1ra;
	pw_gzrftag2rd  = _pw_gzrftag2rd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gzrftag2rd) : pw_gzrftag1ra;

	pw_gzrftag2rc  = _pw_gzrftag2rc.fixedflag ?  ((void)(pw_gzrftag1r), pw_gzrftag2rc) : pw_gzrftag1r;
	pw_gzrftag2rca  = _pw_gzrftag2rca.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gzrftag2rca) : pw_gzrftag1ra;
	pw_gzrftag2rcd  = _pw_gzrftag2rcd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gzrftag2rcd) : pw_gzrftag1ra;

	/*Additional tagging gradient gx*/

	pw_gxrftag1  = _pw_gxrftag1.fixedflag ?  ((void)(pw_gzrftag1r), pw_gxrftag1) : pw_gzrftag1r; 
	pw_gxrftag1a  = _pw_gxrftag1a.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag1a) : pw_gzrftag1ra;
	pw_gxrftag1d  = _pw_gxrftag1d.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag1d) : pw_gzrftag1ra;

	pw_gxrftag2  = _pw_gxrftag2.fixedflag ?  ((void)(pw_gzrftag1r), pw_gxrftag2) : pw_gzrftag1r; 
	pw_gxrftag2a  = _pw_gxrftag2a.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag2a) : pw_gzrftag1ra;
	pw_gxrftag2d  = _pw_gxrftag2d.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag2d) : pw_gzrftag1ra;

	pw_gxrftag1c  = _pw_gxrftag1c.fixedflag ?  ((void)(pw_gzrftag1r), pw_gxrftag1c) : pw_gzrftag1r; 
	pw_gxrftag1ca  = _pw_gxrftag1ca.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag1ca) : pw_gzrftag1ra;
	pw_gxrftag1cd  = _pw_gxrftag1cd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag1cd) : pw_gzrftag1ra;

	pw_gxrftag2c  = _pw_gxrftag2c.fixedflag ?  ((void)(pw_gzrftag1r), pw_gxrftag2c) : pw_gzrftag1r; 

	pw_gxrftag2ca  = _pw_gxrftag2ca.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag2ca) : pw_gzrftag1ra;
	pw_gxrftag2cd  = _pw_gxrftag2cd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gxrftag2cd) : pw_gzrftag1ra;
	
	/*Additional tagging gradient gy*/
	pw_gyrftag1  = _pw_gyrftag1.fixedflag ?  ((void)(pw_gzrftag1r), pw_gyrftag1) : pw_gzrftag1r; 
	pw_gyrftag1a  = _pw_gyrftag1a.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag1a) : pw_gzrftag1ra;
	pw_gyrftag1d  = _pw_gyrftag1d.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag1d) : pw_gzrftag1ra;

	pw_gyrftag2  = _pw_gyrftag2.fixedflag ?  ((void)(pw_gzrftag1r), pw_gyrftag2) : pw_gzrftag1r; 
	pw_gyrftag2a  = _pw_gyrftag2a.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag2a) : pw_gzrftag1ra;
	pw_gyrftag2d  = _pw_gyrftag2d.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag2d) : pw_gzrftag1ra;

	pw_gyrftag1c  = _pw_gyrftag1c.fixedflag ? ((void)(pw_gzrftag1r), pw_gyrftag1c) : pw_gzrftag1r; 
	pw_gyrftag1ca  = _pw_gyrftag1ca.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag1ca) : pw_gzrftag1ra;
	pw_gyrftag1cd  = _pw_gyrftag1cd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag1cd) : pw_gzrftag1ra;

	pw_gyrftag2c  = _pw_gyrftag2c.fixedflag ?  ((void)(pw_gzrftag1r), pw_gyrftag2c) : pw_gzrftag1r; 
	pw_gyrftag2ca  = _pw_gyrftag2ca.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag2ca) : pw_gzrftag1ra;
	pw_gyrftag2cd  = _pw_gyrftag2cd.fixedflag ?  ((void)(pw_gzrftag1ra), pw_gyrftag2cd) : pw_gzrftag1ra;


	/* compute the areas of the slice select pulses */
	area_gzrftag1  = _area_gzrftag1.fixedflag ?         ((void)((double)(a_gzrftag1*(pw_gzrftag1+pw_gzrftag1a))), area_gzrftag1) : (double)(a_gzrftag1*(pw_gzrftag1+pw_gzrftag1a));
	area_gzrftag2  = _area_gzrftag2.fixedflag ?         ((void)((double)(a_gzrftag2*(pw_gzrftag2+pw_gzrftag2a))), area_gzrftag2) : (double)(a_gzrftag2*(pw_gzrftag2+pw_gzrftag2a));
	area_gzrftag1c  = _area_gzrftag1c.fixedflag ?      ((void)((double)(a_gzrftag1c*(pw_gzrftag1c+pw_gzrftag1ca))), area_gzrftag1c) : (double)(a_gzrftag1c*(pw_gzrftag1c+pw_gzrftag1ca));
	area_gzrftag2c  = _area_gzrftag2c.fixedflag ?      ((void)((double)(a_gzrftag2c*(pw_gzrftag2c+pw_gzrftag2ca))), area_gzrftag2c) : (double)(a_gzrftag2c*(pw_gzrftag2c+pw_gzrftag2ca));

	/* COmpute refocuser gradient strength based on desired G_ave (instead of fractional moment) */
	a_gzrftag1r  = _a_gzrftag1r.fixedflag ?      ((void)((pcasl_Gave*t_rfrepeat-area_gzrftag1)/(pw_gzrftag1ra+pw_gzrftag1r)), a_gzrftag1r) : (pcasl_Gave*t_rfrepeat-area_gzrftag1)/(pw_gzrftag1ra+pw_gzrftag1r);
	a_gzrftag1rc  = _a_gzrftag1rc.fixedflag ?  ((void)(a_gzrftag1r), a_gzrftag1rc) : a_gzrftag1r;
	a_gzrftag2r  = _a_gzrftag2r.fixedflag ?  ((void)(a_gzrftag1r), a_gzrftag2r) : a_gzrftag1r;
	a_gzrftag2rc  = _a_gzrftag2rc.fixedflag ?  ((void)(a_gzrftag1r), a_gzrftag2rc) : a_gzrftag1r;

	/* Compute amplitude of x and y gradients */
	/*a_gxyrftag = 0.0;*/
	a_gyrftag1  = _a_gyrftag1.fixedflag ?  ((void)(a_gxyrftag), a_gyrftag1) : a_gxyrftag;
	a_gyrftag2  = _a_gyrftag2.fixedflag ?  ((void)(a_gxyrftag), a_gyrftag2) : a_gxyrftag;
	a_gxrftag1  = _a_gxrftag1.fixedflag ?  ((void)(a_gxyrftag), a_gxrftag1) : a_gxyrftag;
	a_gxrftag2  = _a_gxrftag2.fixedflag ?  ((void)(a_gxyrftag), a_gxrftag2) : a_gxyrftag;
	a_gxrftag1c  = _a_gxrftag1c.fixedflag ?  ((void)(a_gxrftag1), a_gxrftag1c) : a_gxrftag1;
	a_gxrftag2c  = _a_gxrftag2c.fixedflag ?  ((void)(a_gxrftag2), a_gxrftag2c) : a_gxrftag2;
	a_gyrftag1c  = _a_gyrftag1c.fixedflag ?  ((void)(a_gyrftag1), a_gyrftag1c) : a_gyrftag1;
	a_gyrftag2c  = _a_gyrftag2c.fixedflag ?  ((void)(a_gyrftag2), a_gyrftag2c) : a_gyrftag2;


	/* compute as of the slice refocusers */
	area_gzrftag1r  = _area_gzrftag1r.fixedflag ?         ((void)((double)(a_gzrftag1r*(pw_gzrftag1r+pw_gzrftag1ra))), area_gzrftag1r) : (double)(a_gzrftag1r*(pw_gzrftag1r+pw_gzrftag1ra));
	area_gzrftag2r  = _area_gzrftag2r.fixedflag ?         ((void)((double)(a_gzrftag2r*(pw_gzrftag2r+pw_gzrftag2ra))), area_gzrftag2r) : (double)(a_gzrftag2r*(pw_gzrftag2r+pw_gzrftag2ra));
	area_gzrftag1rc  = _area_gzrftag1rc.fixedflag ?         ((void)((double)(a_gzrftag1rc*(pw_gzrftag1rc+pw_gzrftag1rca))), area_gzrftag1rc) : (double)(a_gzrftag1rc*(pw_gzrftag1rc+pw_gzrftag1rca));
	area_gzrftag2rc  = _area_gzrftag2rc.fixedflag ?         ((void)((double)(a_gzrftag2rc*(pw_gzrftag2rc+pw_gzrftag2rca))), area_gzrftag2rc) : (double)(a_gzrftag2rc*(pw_gzrftag2rc+pw_gzrftag2rca));

	/* compute the areas of the vessel selective gradients gx and gy */
	area_gxrftag1  = _area_gxrftag1.fixedflag ?         ((void)((double)(a_gxrftag1*(pw_gxrftag1+pw_gxrftag1a))), area_gxrftag1) : (double)(a_gxrftag1*(pw_gxrftag1+pw_gxrftag1a));
	area_gxrftag2  = _area_gxrftag2.fixedflag ?         ((void)((double)(a_gxrftag2*(pw_gxrftag2+pw_gxrftag2a))), area_gxrftag2) : (double)(a_gxrftag2*(pw_gxrftag2+pw_gxrftag2a));
	area_gyrftag1  = _area_gyrftag1.fixedflag ?         ((void)((double)(a_gyrftag1*(pw_gyrftag1+pw_gyrftag1a))), area_gyrftag1) : (double)(a_gyrftag1*(pw_gyrftag1+pw_gyrftag1a));
	area_gyrftag2  = _area_gyrftag2.fixedflag ?         ((void)((double)(a_gyrftag2*(pw_gyrftag2+pw_gyrftag2a))), area_gyrftag2) : (double)(a_gyrftag2*(pw_gyrftag2+pw_gyrftag2a));
	area_gxrftag1c  = _area_gxrftag1c.fixedflag ?         ((void)((double)(a_gxrftag1c*(pw_gxrftag1c+pw_gxrftag1ca))), area_gxrftag1c) : (double)(a_gxrftag1c*(pw_gxrftag1c+pw_gxrftag1ca));
	area_gxrftag2c  = _area_gxrftag2c.fixedflag ?         ((void)((double)(a_gxrftag2c*(pw_gxrftag2c+pw_gxrftag2ca))), area_gxrftag2c) : (double)(a_gxrftag2c*(pw_gxrftag2c+pw_gxrftag2ca));
	area_gyrftag1c  = _area_gyrftag1c.fixedflag ?         ((void)((double)(a_gyrftag1c*(pw_gyrftag1c+pw_gyrftag1ca))), area_gyrftag1c) : (double)(a_gyrftag1c*(pw_gyrftag1c+pw_gyrftag1ca));
	area_gyrftag2c  = _area_gyrftag2c.fixedflag ?         ((void)((double)(a_gyrftag2c*(pw_gyrftag2c+pw_gyrftag2ca))), area_gyrftag2c) : (double)(a_gyrftag2c*(pw_gyrftag2c+pw_gyrftag2ca));

	B1tag_offset  = _B1tag_offset.fixedflag ?     ((void)(-a_gzrftag1*(B1tag_distance*GAMMA_H1)/(2*PI)), B1tag_offset) : -a_gzrftag1*(B1tag_distance*GAMMA_H1)/(2*PI);

	tmin_rfrepeat  = _tmin_rfrepeat.fixedflag ?   
		 
		 
		 
		 
		((void)(pw_gzrftag2+2*pw_gzrftag2d+2*pw_gzrftag2ra+pw_gzrftag2r+timessi+mytpre), tmin_rfrepeat) : pw_gzrftag2+2*pw_gzrftag2d+2*pw_gzrftag2ra+pw_gzrftag2r+timessi+mytpre;
	...
	myphase_increment  = _myphase_increment.fixedflag ?          ((void)(-GAMMA_H1*(double)(area_gzrftag1+area_gzrftag1r)/(1000000)*B1tag_distance), myphase_increment) : -GAMMA_H1*(double)(area_gzrftag1+area_gzrftag1r)/(1000000)*B1tag_distance;
	/* (REMOVED on 12/5/12)----> The negative sign is because the tag_distance should be a negative number!  */

	myphase_increment_test  = _myphase_increment_test.fixedflag ?  ((void)(myphase_increment), myphase_increment_test) : myphase_increment;

	/* 9/8/14 :  phase correction from the user interface */ 
	myphase_increment  = _myphase_increment.fixedflag ?  ((void)(manual_phase_correction), myphase_increment) : myphase_increment+manual_phase_correction;

	/* wrap phase to (-pi,pi) range */
	myphase_increment  = _myphase_increment.fixedflag ?    ((void)(atan2(sin(myphase_increment),cos(myphase_increment))), myphase_increment) : atan2(sin(myphase_increment),cos(myphase_increment)); 	

	nreps = _nreps.fixedflag ?    ((void)((int)(t_tag/(2*t_rfrepeat))), nreps) : (int)(t_tag/(2*t_rfrepeat));
...
i	*/
	if(doZgrappa) slab_fraction  = _slab_fraction.fixedflag ?  ((void)(0.85), slab_fraction) : 0.85;

	pw_gzrf1ra  = _pw_gzrf1ra.fixedflag ?  ((void)(500), pw_gzrf1ra) : 500;
	pw_gzrf1rd  = _pw_gzrf1rd.fixedflag ?  ((void)(500), pw_gzrf1rd) : 500;
	pw_gzrf1r  = _pw_gzrf1r.fixedflag ?  ((void)(200), pw_gzrf1r) : 200;  
	...
	zfov  = _zfov.fixedflag ?  ((void)((double)opslquant*opslthick/10), zfov) : (double)opslquant*opslthick/10;

	ampGzstep  = _ampGzstep.fixedflag ?          ((void)(1/((pwGzstep+Gzramp)*1e-6*GAMMA_H1/(2*PI)*zfov)), ampGzstep) : 1/((pwGzstep+Gzramp)*1e-6*GAMMA_H1/(2*PI)*zfov);  /* z phase-ecncode readout gradient for this zfov G/cm*/

	if (doZgrappa) 
		ampGzstep  = _ampGzstep.fixedflag ?  ((void)(2), ampGzstep) : ampGzstep*2;
	
	areaGzstep  = _areaGzstep.fixedflag ?       ((void)(ampGzstep*(pwGzstep+Gzramp)), areaGzstep) : ampGzstep*(pwGzstep+Gzramp);        /* max area of z encoding gradient in G/cm*usec */

	/* get the specs of the z gradient */
	gettarget(_target.fixedflag ? (_temp0_target=target,&_temp0_target) : &target, ZGRAD, &loggrd);
	getramptime(_rtime.fixedflag ? (_temp1_rtime=rtime,&_temp1_rtime) : &rtime, _ftime.fixedflag ? (_temp2_ftime=ftime,&_temp2_ftime) : &ftime, ZGRAD, &loggrd);
	...
	iampGzstep  = _iampGzstep.fixedflag ?        ((void)(2*((ampGzstep/target)*MAX_PG_IAMP/2.0)), iampGzstep) : 2*((ampGzstep/target)*MAX_PG_IAMP/2.0);

	/* Now stick those values into the actual trapezoid: */
	a_gzphase1  = _a_gzphase1.fixedflag ?  ((void)(-ampGzstep), a_gzphase1) : -ampGzstep;
	a_gzphase2  = _a_gzphase2.fixedflag ?  ((void)(ampGzstep), a_gzphase2) : ampGzstep;

	pw_gzphase1 = _pw_gzphase1.fixedflag ?  ((void)(pwGzstep), pw_gzphase1) : pwGzstep;
	pw_gzphase1a = _pw_gzphase1a.fixedflag ?  ((void)(Gzramp), pw_gzphase1a) : Gzramp;/*careful not to exceed slew rate */
	pw_gzphase1d = _pw_gzphase1d.fixedflag ?  ((void)(Gzramp), pw_gzphase1d) : Gzramp;
	pw_gzphase2 = _pw_gzphase2.fixedflag ?  ((void)(pwGzstep), pw_gzphase2) : pwGzstep;
	pw_gzphase2a = _pw_gzphase2a.fixedflag ?  ((void)(Gzramp), pw_gzphase2a) : Gzramp;
	pw_gzphase2d = _pw_gzphase2d.fixedflag ?  ((void)(Gzramp), pw_gzphase2d) : Gzramp;

	fprintf(stderr, "predownload(): ampGzstep = %.3f, iampGzstep = %d \n", ampGzstep, iampGzstep);
	/* ----------------------------------------------------------------------------------------*/
	pw_gz0 = _pw_gz0.fixedflag ? ((void)(3000), pw_gz0) : 3000;
	pw_gz0a = _pw_gz0a.fixedflag ? ((void)(400), pw_gz0a) : 400;
	pw_gz0d = _pw_gz0d.fixedflag ? ((void)(400), pw_gz0d) : 400;
	a_gz0  = _a_gz0.fixedflag ?  ((void)(2.0), a_gz0) : 2.0;
	...
	rfscalesech  = _rfscalesech.fixedflag ?  ((void)(0.2153), rfscalesech) : 0.2153;
	rfscalesech  = _rfscalesech.fixedflag ?  ((void)(1.0), rfscalesech) : 1.0;  /* Empirically on sphere phantom, I didn't get an inversion unutl 0.65) */


	pw_BS1rf  = _pw_BS1rf.fixedflag ?  ((void)(8000), pw_BS1rf) : 8000;
	a_BS1rf  = _a_BS1rf.fixedflag ?     ((void)(rfscalesech*(float)doBS), a_BS1rf) : rfscalesech*(float)doBS; /* the sech is used to generate a 180 */ 
	res_BS1rf  = _res_BS1rf.fixedflag ?  ((void)(2000), res_BS1rf) : 2000; /* the number of points in the file */
	res_BS1rf  = _res_BS1rf.fixedflag ?  ((void)(500), res_BS1rf) : 500; /* the number of points in the file */

	pw_BS2rf  = _pw_BS2rf.fixedflag ?  ((void)(pw_BS1rf), pw_BS2rf) : pw_BS1rf;;
	a_BS2rf  = _a_BS2rf.fixedflag ?  ((void)(a_BS1rf), a_BS2rf) : a_BS1rf;
	res_BS2rf  = _res_BS2rf.fixedflag ?  ((void)(res_BS1rf), res_BS2rf) : res_BS1rf;

	/* JS 03/14/2018: Parameters for BS0 pulses*/
	pw_BS0rf  = _pw_BS0rf.fixedflag ?  ((void)(pw_BS1rf), pw_BS0rf) : pw_BS1rf;
	a_BS0rf  = _a_BS0rf.fixedflag ?  ((void)(a_BS1rf), a_BS0rf) : a_BS1rf;
	res_BS0rf  = _res_BS0rf.fixedflag ?  ((void)(res_BS1rf/2), res_BS0rf) : res_BS1rf/2;

	a_gzBS0rfspoiler  = _a_gzBS0rfspoiler.fixedflag ?  ((void)((float)doBS), a_gzBS0rfspoiler) : (float)doBS; /*DO 1 G/cm */
	pw_gzBS0rfspoiler = _pw_gzBS0rfspoiler.fixedflag ? ((void)(3000), pw_gzBS0rfspoiler) : 3000;
	pw_gzBS0rfspoilera = _pw_gzBS0rfspoilera.fixedflag ? ((void)(400), pw_gzBS0rfspoilera) : 400;
	pw_gzBS0rfspoilerd = _pw_gzBS0rfspoilerd.fixedflag ? ((void)(400), pw_gzBS0rfspoilerd) : 400;	
        area_gzBSOrfspoiler  = _area_gzBSOrfspoiler.fixedflag ?  ((void)((double)(a_gzBS0rfspoiler*(pw_gzBS0rfspoiler+pw_gzBS0rfspoilera))), area_gzBSOrfspoiler) : (double)(a_gzBS0rfspoiler*(pw_gzBS0rfspoiler+pw_gzBS0rfspoilera));

	t_preBS  = _t_preBS.fixedflag ?        ((void)(pw_BS0rf+pw_gzBS0rfspoiler+2*pw_gzBS0rfspoilera+600), t_preBS) : pw_BS0rf+pw_gzBS0rfspoiler+2*pw_gzBS0rfspoilera+600;
	...
	pw_gz180crush1  = _pw_gz180crush1.fixedflag ?  ((void)(200), pw_gz180crush1) : 200;
	pw_gz180crush2  = _pw_gz180crush2.fixedflag ?  ((void)(pw_gz180crush1), pw_gz180crush2) : pw_gz180crush1;

	pw_gz180crush1a  = _pw_gz180crush1a.fixedflag ?  ((void)(160), pw_gz180crush1a) : 160;
	pw_gz180crush2a  = _pw_gz180crush2a.fixedflag ?  ((void)(pw_gz180crush1a), pw_gz180crush2a) : pw_gz180crush1a;

	pw_gz180crush1d  = _pw_gz180crush1d.fixedflag ?  ((void)(pw_gz180crush1a), pw_gz180crush1d) : pw_gz180crush1a;
	pw_gz180crush2d  = _pw_gz180crush2d.fixedflag ?  ((void)(pw_gz180crush1d), pw_gz180crush2d) : pw_gz180crush1d;
	...
	a_gz180crush1  = _a_gz180crush1.fixedflag ?  ((void)(1.0), a_gz180crush1) : 1.0;
	a_gz180crush2  = _a_gz180crush2.fixedflag ?  ((void)(a_gz180crush1), a_gz180crush2) : a_gz180crush1;

	BStime  = _BStime.fixedflag ?         ((void)(BS1_time+BS2_time+pw_BS1rf+pw_BS2rf), BStime) : BS1_time+BS2_time+pw_BS1rf+pw_BS2rf;

	fatsattime  = _fatsattime.fixedflag ?       ((void)(pw_rf0+2*pw_gz0d+pw_gz0+2*timessi), fatsattime) : pw_rf0+2*pw_gz0d+pw_gz0+2*timessi;

	astseqtime2  = _astseqtime2.fixedflag ?    ((void)(t_tag+t_delay), astseqtime2) : t_tag+t_delay;  /*duration of tagging + postabeling delay */
	
	t_tipdown_core  = _t_tipdown_core.fixedflag ?       ((void)(RUP_GRD(opte/2-pw_gz180crush1-2*pw_gz180crush1a)), t_tipdown_core) : RUP_GRD(opte/2-pw_gz180crush1-2*pw_gz180crush1a);
	t_seqcore  = _t_seqcore.fixedflag ?  ((void)(opte), t_seqcore) : opte;	
	...
	ophrep = _ophrep.fixedflag ? ((void)(3), ophrep) : 3; 
...
int predownload()
{
	...
	ihtr  = _ihtr.fixedflag ?  ((void)(optr), ihtr) : optr;

	/*  make the end dead time = TR */
	endtime  = _endtime.fixedflag ?  ((void)(optr), endtime) : optr;
	...
	if((fpin=fopen("daqdeloff.sprl", "r"))) {     /* look for ext. file */
		fscanf(fpin, "%f", _daqdeloff.fixedflag ? (_temp3_daqdeloff=daqdeloff,&_temp3_daqdeloff) : &daqdeloff);
		fclose(fpin);
	}  
...
2*pw_gzspoila);
	 */
	astseqtime  = _astseqtime.fixedflag ?   ((void)((2*t_rfrepeat)), astseqtime) : (2*t_rfrepeat);
	astseqtime2  = _astseqtime2.fixedflag ?    ((void)(nreps*astseqtime+t_delay), astseqtime2) : nreps*astseqtime+t_delay;  /*duration of tagging + postabeling delay */

	BStime  = _BStime.fixedflag ?         ((void)(BS1_time+BS2_time+pw_BS1rf+pw_BS2rf), BStime) : BS1_time+BS2_time+pw_BS1rf+pw_BS2rf; /* + pw_BS0rf + pw_gzBS0 + 2*pw_gzBS0a; */

	fatsattime  = _fatsattime.fixedflag ?       ((void)(pw_rf0+2*pw_gz0d+pw_gz0+2*timessi), fatsattime) : pw_rf0+2*pw_gz0d+pw_gz0+2*timessi;
	fuzz  = _fuzz.fixedflag ?  ((void)(10000), fuzz) : 10000;

	/* make sure time between 90 and 180 is half of the time between 180s*/
	time_90_180  = _time_90_180.fixedflag ?          
		        ((void)(RUP_GRD(pw_gzrf1/2+pw_gzrf1a+pw_gzrf1r+2*pw_gzrf1ra+pw_gz180crush1+2*pw_gz180crush1a+pw_gzrf2/2+pw_gzrf2a+100)), time_90_180) : RUP_GRD(pw_gzrf1/2+pw_gzrf1a+pw_gzrf1r+2*pw_gzrf1ra+pw_gz180crush1+2*pw_gz180crush1a+pw_gzrf2/2+pw_gzrf2a+100);

	time_180_180  = _time_180_180.fixedflag ?        
		         
		       ((void)(RUP_GRD(pw_gz180crush1+2*pw_gz180crush1a+pw_gz180crush2+2*pw_gz180crush2a+pw_gzrf2+2*pw_gzrf2a+pw_gzphase1+2*pw_gzphase1a+pw_gx+pwgpre+pw_gzphase2+2*pw_gzphase2a+1000)), time_180_180) : RUP_GRD(pw_gz180crush1+2*pw_gz180crush1a+pw_gz180crush2+2*pw_gz180crush2a+pw_gzrf2+2*pw_gzrf2a+pw_gzphase1+2*pw_gzphase1a+pw_gx+pwgpre+pw_gzphase2+2*pw_gzphase2a+1000);

	if (2*time_90_180 < time_180_180) {
		textra90  = _textra90.fixedflag ?   ((void)((time_180_180-2*time_90_180)/2), textra90) : (time_180_180-2*time_90_180)/2;
		textra180  = _textra180.fixedflag ?  ((void)(0), textra180) : 0;
	}
	else {
		textra90  = _textra90.fixedflag ?  ((void)(0), textra90) : 0;
		textra180  = _textra180.fixedflag ?   ((void)(2*time_90_180-time_180_180), textra180) : 2*time_90_180-time_180_180;
	}
...
/*
	...
	seqtr = t_tipdown_core + t_seqcore * opslquant;
*/
	seqtr  = _seqtr.fixedflag ?      ((void)(opte/2+opte*opslquant), seqtr) : opte/2+opte*opslquant;

	psdseqtime  = _psdseqtime.fixedflag ?        ((void)(seqtr+t_tag+t_delay+t_preBS), psdseqtime) : seqtr+t_tag+t_delay+t_preBS;	

	t_adjust  = _t_adjust.fixedflag ?          ((void)(RUP_GRD(optr-seqtr-t_tag-t_delay-t_preBS)), t_adjust) : RUP_GRD(optr-seqtr-t_tag-t_delay-t_preBS);                 /* LHG 7.18.18 */
	...
	if(gtype==1)  tmin  = _tmin.fixedflag ?  ((void)(pw_gx), tmin) : tmin-pw_gx;
	if(gtype==2)  tmin  = _tmin.fixedflag ?  ((void)(espace), tmin) : tmin+espace;
	...
	psdtottime  = _psdtottime.fixedflag ?  ((void)(psdseqtime*opslquant), psdtottime) : psdseqtime*opslquant;
	trerror  = _trerror.fixedflag ?    ((void)(psdtottime-optr), trerror) : psdtottime-optr;
	nerror  = _nerror.fixedflag ?  ((void)(fabs((double)trerror)/GRAD_UPDATE_TIME), nerror) : fabs((double)trerror)/GRAD_UPDATE_TIME;
	...
	gfov  = _gfov.fixedflag ?  ((void)(opfov*.1), gfov) : opfov*.1;
	calcvalidrbw((double)oprbw,_bandwidth.fixedflag ? (_temp4_bandwidth=bandwidth,&_temp4_bandwidth) : &bandwidth,&max_rbw,_decimation.fixedflag ? (_temp5_decimation=decimation,&_temp5_decimation) : &decimation,OVERWRITE_OPRBW,0);
	gmax  = _gmax.fixedflag ?  ((void)(2e3*bandwidth/(GAM*gfov)), gmax) : 2e3*bandwidth/(GAM*gfov);    /* max grad allowed for BW */
	gamp  = _gamp.fixedflag ?   ((void)(MIN(cfxfs,gmax)), gamp) : MIN(cfxfs,gmax);
	tsp  = _tsp.fixedflag ?  ((void)(TSP*decimation), tsp) : TSP*decimation;

	/* readout gradients - calculate 'em here  */

	tadmax  = _tadmax.fixedflag ?  ((void)(tsp*GRESMAX), tadmax) : tsp*GRESMAX;		/* max readout */
	if(vdflag==0) {
...
//a_rf2  = _a_rf2.fixedflag ?  ((void)(max_pg_iamp*(*rfpulse[0].amp)), a_rf2) : max_pg_iamp*(*rfpulse[0].amp);


	/* set up sat pulse */
	satBW  = _satBW.fixedflag ?     ((void)((cffield==15000)?220:440), satBW) : (cffield==15000)?220:440;
	satoff  = _satoff.fixedflag ?     ((void)((cffield==15000)?-260:-520), satoff) : (cffield==15000)?-260:-520;  /* offset  down a little */

	/*JS 08/09/18 switched values */
	satoff  = _satoff.fixedflag ?     ((void)((cffield==15000)?220:-520), satoff) : (cffield==15000)?220:-520;
	satBW  = _satBW.fixedflag ?     ((void)((cffield==15000)?-260:440), satBW) : (cffield==15000)?-260:440;  /* offset  down a little */

	arf0  = _arf0.fixedflag ?  ((void)(0.5*satBW/1250.0), arf0) : 0.5*satBW/1250.0;
	pwrf0  = _pwrf0.fixedflag ?  ((void)(RUP_GRD((int)(4000000*cyc_rf0/satBW))), pwrf0) : RUP_GRD((int)(4000000*cyc_rf0/satBW));
	fprintf(stderr,"\nsatBW, satoff, arf0, pwrf0 = %f  %f  %f  %d\n", satBW, satoff,
			arf0, pwrf0);

	pw_rf0  = _pw_rf0.fixedflag ?  ((void)(pwrf0), pw_rf0) : pwrf0;
	a_rf0  = _a_rf0.fixedflag ?  ((void)(arf0), a_rf0) : arf0;		
        /*LHG 6/12/2018: empirical calibration for fat sat */
        a_rf0  = _a_rf0.fixedflag ?  ((void)(0.28), a_rf0) : 0.28;

	/*JS optimized fatsat amplitude*/
	a_rf0  = _a_rf0.fixedflag ?  ((void)(0.14), a_rf0) : 0.14;/*0.28;*/ /* 08/19 JS scaled based on b1max = 0.2943 G and peak B1 for fatsat RF (90) = 0.0306 G*/
	/*actually ended up scaling based on experiment, slightly different from calculated value*/
	a_gz0  = _a_gz0.fixedflag ?  ((void)(0.5), a_gz0) : 0.5;
	/*a_gx0 = a_gz0;
	a_gy0 = a_gz0;*/

	nbang  = _nbang.fixedflag ?  ((void)((nextra+nl*nframes)*opslquant), nbang) : (nextra+nl*nframes)*opslquant;
	ndisdaq  = _ndisdaq.fixedflag ?   ((void)(MIN(nextra,6e6/optr)), ndisdaq) : MIN(nextra,6e6/optr);	/* for prescan only  */

	/* set up clock */
	if ((exist(opcgate) == PSD_ON) && existcv(opcgate))
	{
		pidmode  = _pidmode.fixedflag ?  ((void)(PSD_CLOCK_CARDIAC), pidmode) : PSD_CLOCK_CARDIAC;
		piviews  = _piviews.fixedflag ?  ((void)(nextra+nl*nframes), piviews) : nextra+nl*nframes;
		piclckcnt  = _piclckcnt.fixedflag ?  ((void)(ophrep), piclckcnt) : ophrep;
		pitscan  = _pitscan.fixedflag ?  ((void)((float)(optr)*nbang/opslquant), pitscan) : (float)(optr)*nbang/opslquant;
		pitslice  = _pitslice.fixedflag ?  ((void)(seqtr), pitslice) : seqtr;
	}
	else
	{
	...
	minte  = _minte.fixedflag ?  
		...
		((void)(pw_gzrf2+2*pw_gzrf2a+pw_gz180crush2+2*pw_gz180crush2a+pw_gzphase1+2*pw_gzphase1a+pw_gx+pw_gzphase2+2*pw_gzphase2a+pwgpre+textra180+timessi), minte) : pw_gzrf2+2*pw_gzrf2a+pw_gz180crush2+2*pw_gz180crush2a+pw_gzphase1+2*pw_gzphase1a+pw_gx+pw_gzphase2+2*pw_gzphase2a+pwgpre+textra180+timessi;
	...
	*/
	...
	if(gtype>0) minte  = _minte.fixedflag ?    ((void)(pw_gx+pwgpre), minte) : minte+pw_gx+pwgpre;
	cvmin(opte, minte);
	cvdef(opte, minte);
	if ((exist(opautote) == PSD_MINTE)||(exist(opautote) == PSD_MINTEFULL))
		opte  = _opte.fixedflag ?  ((void)(minte), opte) : minte;
    ...
    { /* Start of code inlined from loadrheader.e rheaderinit */
     ...
     rhnavs  = _rhnavs.fixedflag ?      ((void)(IMax(2,(INT)(nex/2),(INT)1)), rhnavs) : IMax(2,(INT)(nex/2),(INT)1);
        ...
        set_echo_flip(_rhdacqctrl.fixedflag ? (_temp6_rhdacqctrl=rhdacqctrl,&_temp6_rhdacqctrl) : &rhdacqctrl, &chksum_rhdacqctrl, eepf, oepf, eeff, oeff);
        ...
        */
...

        {
            ...
            {
                if (FAILURE == getPUREMixParameters(model_parameters, coilInfo, exist(opanatomy), &pure_mix, _pure_mix_tx_scale.fixedflag ? (_temp7_pure_mix_tx_scale=pure_mix_tx_scale,&_temp7_pure_mix_tx_scale) : &pure_mix_tx_scale))
                {
	...
	rhimsize  = _rhimsize.fixedflag ?  ((void)(64), rhimsize) : 64;
	while(rhimsize < opxres)  rhimsize  = _rhimsize.fixedflag ?  ((void)(2), rhimsize) : rhimsize*2;
	if(off_fov==1)  {
		rhxoff  = _rhxoff.fixedflag ?  ((void)(0), rhxoff) : 0;
		rhyoff  = _rhyoff.fixedflag ?  ((void)(0), rhyoff) : 0;
	}
	else  {
		rhxoff  = _rhxoff.fixedflag ?  ((void)(rhimsize*scan_info[0].oprloc/opfov), rhxoff) : rhimsize*scan_info[0].oprloc/opfov;
		rhyoff  = _rhyoff.fixedflag ?  ((void)(rhimsize*scan_info[0].opphasoff/opfov), rhyoff) : rhimsize*scan_info[0].opphasoff/opfov;
	}
	rhuser0  = _rhuser0.fixedflag ?  ((void)(gfov), rhuser0) : gfov; 	/* grad design fov in cm  */
	rhuser1  = _rhuser1.fixedflag ?  ((void)(nframes), rhuser1) : nframes; 	/* number of temporal frames */
	rhuser3  = _rhuser3.fixedflag ?  ((void)(opxres), rhuser3) : opxres;		/*  equiv resolution  */
	rhuser4  = _rhuser4.fixedflag ?  ((void)(nl), rhuser4) : nl;   	/* number of interleaves  */
	rhuser5  = _rhuser5.fixedflag ?  ((void)(gtype), rhuser5) : gtype;   	/* gradient wavewform rev  */
	rhuser6  = _rhuser6.fixedflag ?  ((void)(gamp), rhuser6) : gamp; 	/* grad design ampl  */
	rhuser7  = _rhuser7.fixedflag ?  ((void)(gslew), rhuser7) : gslew; 	/* grad slew rate  */
	rhuser9  = _rhuser9.fixedflag ?  ((void)(bmapnav), rhuser9) : bmapnav;  /* if set, keeps dacq fixed in time for ifr=0 */
	rhuser10  = _rhuser10.fixedflag ?  ((void)(0), rhuser10) : 0;	/* ngap between concat acq's */
	rhuser11  = _rhuser11.fixedflag ?  ((void)(0), rhuser11) : 0;
	rhuser12  = _rhuser12.fixedflag ?  ((void)(0), rhuser12) : 0;  	/* receiver center freq, kHz */
	rhuser13  = _rhuser13.fixedflag ?  ((void)(tsp), rhuser13) : tsp; 	/* a/d sampling time (us) */
	rhuser14  = _rhuser14.fixedflag ?       ((void)((nframes>1&&oppseq==2)?domap:0), rhuser14) : (nframes>1&&oppseq==2)?domap:0;  /* do it if GRE */
	rhuser15  = _rhuser15.fixedflag ?  ((void)(mapdel), rhuser15) : mapdel;  /* echo difference (us) */
	rhuser21  = _rhuser21.fixedflag ?  ((void)(alpha), rhuser21) : alpha;	/* for vd trajectory  */
	rhuser22  = _rhuser22.fixedflag ?  ((void)(kmaxfrac), rhuser22) : kmaxfrac;
	rhuser23  = _rhuser23.fixedflag ?  ((void)(vdflag), rhuser23) : vdflag;
	rhuser30  = _rhuser30.fixedflag ?  ((void)(opslthick), rhuser30) : opslthick;
	rhuser31  = _rhuser31.fixedflag ?  ((void)(opslspace), rhuser31) : opslspace;

	rhbline  = _rhbline.fixedflag ?  ((void)(0), rhbline) : 0;
	rhnecho  = _rhnecho.fixedflag ?   ((void)((gtype==2)?2:1), rhnecho) : (gtype==2)?2:1;
	rhnslices  = _rhnslices.fixedflag ?  ((void)(opslquant), rhnslices) : opslquant;
	rhrcctrl  = _rhrcctrl.fixedflag ?  ((void)(1), rhrcctrl) : 1;
	rhexecctrl  = _rhexecctrl.fixedflag ?  ((void)(2), rhexecctrl) : 2; /*  + 16;  save Pfile + make rhrcctrl work */
	rhrecon  = _rhrecon.fixedflag ?  ((void)(opuser3), rhrecon) : opuser3;		/* invoke son of recon */

	rhtype1  = _rhtype1.fixedflag ?  ((void)(RHTYP1RCALLPASS), rhtype1) : RHTYP1RCALLPASS;  /* all frames accumulate in one Pfile */
	if((int)opuser11==1)           /* turn on RDS for realtime */
		rhtype1  = _rhtype1.fixedflag ?    ((void)((0x00080000|0x00100000)), rhtype1) : rhtype1|(0x00080000|0x00100000);
	else
		rhtype1  = _rhtype1.fixedflag ?   ((void)(0x0007FFFF), rhtype1) : rhtype1&0x0007FFFF;

	acqs  = _acqs.fixedflag ?  ((void)(1), acqs) : 1;
	slquant1  = _slquant1.fixedflag ?  ((void)(rhnslices), slquant1) : rhnslices;
	...
	rhmaxcoef1a  = _rhmaxcoef1a.fixedflag ?  ((void)(rsprot[0][0]/(float)cfxfull), rhmaxcoef1a) : rsprot[0][0]/(float)cfxfull;	/* save x rotator */
	rhmaxcoef1b  = _rhmaxcoef1b.fixedflag ?  ((void)(rsprot[0][1]/(float)cfxfull), rhmaxcoef1b) : rsprot[0][1]/(float)cfxfull;
	rhmaxcoef2a  = _rhmaxcoef2a.fixedflag ?  ((void)(rsprot[0][3]/(float)cfyfull), rhmaxcoef2a) : rsprot[0][3]/(float)cfyfull; /* y  */
	rhmaxcoef2b  = _rhmaxcoef2b.fixedflag ?  ((void)(rsprot[0][4]/(float)cfyfull), rhmaxcoef2b) : rsprot[0][4]/(float)cfyfull;
	rhmaxcoef3a  = _rhmaxcoef3a.fixedflag ?  ((void)(rsprot[0][6]/(float)cfzfull), rhmaxcoef3a) : rsprot[0][6]/(float)cfzfull; /* z  */
	rhmaxcoef3b  = _rhmaxcoef3b.fixedflag ?  ((void)(rsprot[0][7]/(float)cfzfull), rhmaxcoef3b) : rsprot[0][7]/(float)cfzfull;
	...
	rhfrsize  = _rhfrsize.fixedflag ?  ((void)((res_gx-RES_GRAMP)*4/tsp), rhfrsize) : (res_gx-RES_GRAMP)*4/tsp;      /* num points sampled */
	rhfrsize  = _rhfrsize.fixedflag ?  ((void)(4*(rhfrsize/4)), rhfrsize) : 4*(rhfrsize/4);          /* wants to be divisible by 4 */
	total_views = _total_views.fixedflag ? ((void)(2*((nl*nframes+1)/2)), total_views) : 2*((nl*nframes+1)/2);  /* has to be an even number */
	cvmax(rhnframes, total_views);
	rhnframes  = _rhnframes.fixedflag ?  ((void)(total_views), rhnframes) : total_views;
	...
	filter_aps2  = _filter_aps2.fixedflag ?  ((void)(aps2_filt.fslot), filter_aps2) : aps2_filt.fslot;

	rhrawsize = 2*rhptsize*rhfrsize*(rhnframes+1)*rhnslices*rhnecho;
	numrecv  = _numrecv.fixedflag ?  ((void)(rhdab0e-rhdab0s+1), numrecv) : rhdab0e-rhdab0s+1;
	if((float)rhrawsize*numrecv > cftpssize-2e7)  {      /* reserve 20 MB */
		epic_error(use_ermes,"Oops! Tooo much memory requested.\n",0,0);
		return FAILURE;
	}

	daqdel  = _daqdel.fixedflag ?      ((void)(psd_grd_wait+daqdeloff+0.5), daqdel) : psd_grd_wait+daqdeloff+0.5;
	...
	trigfreq  = _trigfreq.fixedflag ?  ((void)(opslquant), trigfreq) : opslquant; /* default for TTL trig is every TR */
...
{ /* Start of code inlined from Prescan.e PSpredownload */
    ...
    if (FAILURE==ExtCalpredownload())
    {
		...
		phys_record_flag  = _phys_record_flag.fixedflag ?  ((void)(opuser10), phys_record_flag) : opuser10;        /* put here so it isn't overwritten in predownload */
	phys_record_channelsel  = _phys_record_channelsel.fixedflag ?  ((void)(14), phys_record_channelsel) : 14;        /* PG wave,trig & RESP */
...
PS1cvinit( void )
{
    ...
    if (obloptimize(&ps1loggrd, &phygrd, ps1scan_info, 1, PSD_OBL,
                    0, obl_method, ps1obl_debug, _ps1_newgeo.fixedflag ? (_temp8_ps1_newgeo=ps1_newgeo,&_temp8_ps1_newgeo) : &ps1_newgeo,
                    cfsrmode) == FAILURE)
    {
...
CFHcvinit( void )
{
    ...
    {
        setuprfpulse(RF1_CFH_SLOT, _pw_rf1cfh.fixedflag ? (_temp9_pw_rf1cfh=pw_rf1cfh,&_temp9_pw_rf1cfh) : &pw_rf1cfh, _a_rf1cfh.fixedflag ? (_temp10_a_rf1cfh=a_rf1cfh,&_temp10_a_rf1cfh) : &a_rf1cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf1cfh.fixedflag ? (_temp11_flip_rf1cfh=flip_rf1cfh,&_temp11_flip_rf1cfh) : &flip_rf1cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf1cfh.fixedflag ? (_temp12_res_rf1cfh=res_rf1cfh,&_temp12_res_rf1cfh) : &res_rf1cfh, 0, _wg_rf1cfh.fixedflag ? (_temp13_wg_rf1cfh=wg_rf1cfh,&_temp13_wg_rf1cfh) : &wg_rf1cfh, 1, rfpulse);

        setuprfpulse(RF2_CFH_SLOT, _pw_rf2cfh.fixedflag ? (_temp14_pw_rf2cfh=pw_rf2cfh,&_temp14_pw_rf2cfh) : &pw_rf2cfh, _a_rf2cfh.fixedflag ? (_temp15_a_rf2cfh=a_rf2cfh,&_temp15_a_rf2cfh) : &a_rf2cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf2cfh.fixedflag ? (_temp16_flip_rf2cfh=flip_rf2cfh,&_temp16_flip_rf2cfh) : &flip_rf2cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf2cfh.fixedflag ? (_temp17_res_rf2cfh=res_rf2cfh,&_temp17_res_rf2cfh) : &res_rf2cfh, 0, _wg_rf2cfh.fixedflag ? (_temp18_wg_rf2cfh=wg_rf2cfh,&_temp18_wg_rf2cfh) : &wg_rf2cfh, 1, rfpulse);

        setuprfpulse(RF3_CFH_SLOT, _pw_rf3cfh.fixedflag ? (_temp19_pw_rf3cfh=pw_rf3cfh,&_temp19_pw_rf3cfh) : &pw_rf3cfh, _a_rf3cfh.fixedflag ? (_temp20_a_rf3cfh=a_rf3cfh,&_temp20_a_rf3cfh) : &a_rf3cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf3cfh.fixedflag ? (_temp21_flip_rf3cfh=flip_rf3cfh,&_temp21_flip_rf3cfh) : &flip_rf3cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf3cfh.fixedflag ? (_temp22_res_rf3cfh=res_rf3cfh,&_temp22_res_rf3cfh) : &res_rf3cfh, 0, _wg_rf3cfh.fixedflag ? (_temp23_wg_rf3cfh=wg_rf3cfh,&_temp23_wg_rf3cfh) : &wg_rf3cfh, 1, rfpulse);

        setuprfpulse(RF4_CFH_SLOT, _pw_rf4cfh.fixedflag ? (_temp24_pw_rf4cfh=pw_rf4cfh,&_temp24_pw_rf4cfh) : &pw_rf4cfh, _a_rf4cfh.fixedflag ? (_temp25_a_rf4cfh=a_rf4cfh,&_temp25_a_rf4cfh) : &a_rf4cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf4cfh.fixedflag ? (_temp26_flip_rf4cfh=flip_rf4cfh,&_temp26_flip_rf4cfh) : &flip_rf4cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf4cfh.fixedflag ? (_temp27_res_rf4cfh=res_rf4cfh,&_temp27_res_rf4cfh) : &res_rf4cfh, 0, _wg_rf4cfh.fixedflag ? (_temp28_wg_rf4cfh=wg_rf4cfh,&_temp28_wg_rf4cfh) : &wg_rf4cfh, 1, rfpulse);
    ...
    {
        setuprfpulse(RF1_CFH_SLOT, _pw_rf1cfh.fixedflag ? (_temp29_pw_rf1cfh=pw_rf1cfh,&_temp29_pw_rf1cfh) : &pw_rf1cfh, _a_rf1cfh.fixedflag ? (_temp30_a_rf1cfh=a_rf1cfh,&_temp30_a_rf1cfh) : &a_rf1cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf1cfh.fixedflag ? (_temp31_flip_rf1cfh=flip_rf1cfh,&_temp31_flip_rf1cfh) : &flip_rf1cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf1cfh.fixedflag ? (_temp32_res_rf1cfh=res_rf1cfh,&_temp32_res_rf1cfh) : &res_rf1cfh, 0, _wg_rf1cfh.fixedflag ? (_temp33_wg_rf1cfh=wg_rf1cfh,&_temp33_wg_rf1cfh) : &wg_rf1cfh, 1, rfpulse);

        setuprfpulse(RF2_CFH_SLOT, _pw_rf2cfh.fixedflag ? (_temp34_pw_rf2cfh=pw_rf2cfh,&_temp34_pw_rf2cfh) : &pw_rf2cfh, _a_rf2cfh.fixedflag ? (_temp35_a_rf2cfh=a_rf2cfh,&_temp35_a_rf2cfh) : &a_rf2cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf2cfh.fixedflag ? (_temp36_flip_rf2cfh=flip_rf2cfh,&_temp36_flip_rf2cfh) : &flip_rf2cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf2cfh.fixedflag ? (_temp37_res_rf2cfh=res_rf2cfh,&_temp37_res_rf2cfh) : &res_rf2cfh, 0, _wg_rf2cfh.fixedflag ? (_temp38_wg_rf2cfh=wg_rf2cfh,&_temp38_wg_rf2cfh) : &wg_rf2cfh, 1, rfpulse);

        setuprfpulse(RF3_CFH_SLOT, _pw_rf3cfh.fixedflag ? (_temp39_pw_rf3cfh=pw_rf3cfh,&_temp39_pw_rf3cfh) : &pw_rf3cfh, _a_rf3cfh.fixedflag ? (_temp40_a_rf3cfh=a_rf3cfh,&_temp40_a_rf3cfh) : &a_rf3cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf3cfh.fixedflag ? (_temp41_flip_rf3cfh=flip_rf3cfh,&_temp41_flip_rf3cfh) : &flip_rf3cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf3cfh.fixedflag ? (_temp42_res_rf3cfh=res_rf3cfh,&_temp42_res_rf3cfh) : &res_rf3cfh, 0, _wg_rf3cfh.fixedflag ? (_temp43_wg_rf3cfh=wg_rf3cfh,&_temp43_wg_rf3cfh) : &wg_rf3cfh, 1, rfpulse);

        setuprfpulse(RF4_CFH_SLOT, _pw_rf4cfh.fixedflag ? (_temp44_pw_rf4cfh=pw_rf4cfh,&_temp44_pw_rf4cfh) : &pw_rf4cfh, _a_rf4cfh.fixedflag ? (_temp45_a_rf4cfh=a_rf4cfh,&_temp45_a_rf4cfh) : &a_rf4cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf4cfh.fixedflag ? (_temp46_flip_rf4cfh=flip_rf4cfh,&_temp46_flip_rf4cfh) : &flip_rf4cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf4cfh.fixedflag ? (_temp47_res_rf4cfh=res_rf4cfh,&_temp47_res_rf4cfh) : &res_rf4cfh, 0, _wg_rf4cfh.fixedflag ? (_temp48_wg_rf4cfh=wg_rf4cfh,&_temp48_wg_rf4cfh) : &wg_rf4cfh, 1, rfpulse);
...
PScvinit( void )
{
    ...
    pimrs[0] = _pimrsaps1.fixedflag ? (_temp49_pimrsaps1=pimrsaps1,&_temp49_pimrsaps1) : &pimrsaps1;
    pimrs[1] = _pimrsaps2.fixedflag ? (_temp50_pimrsaps2=pimrsaps2,&_temp50_pimrsaps2) : &pimrsaps2;
    pimrs[2] = _pimrsaps3.fixedflag ? (_temp51_pimrsaps3=pimrsaps3,&_temp51_pimrsaps3) : &pimrsaps3;
    pimrs[3] = _pimrsaps4.fixedflag ? (_temp52_pimrsaps4=pimrsaps4,&_temp52_pimrsaps4) : &pimrsaps4;
    pimrs[4] = _pimrsaps5.fixedflag ? (_temp53_pimrsaps5=pimrsaps5,&_temp53_pimrsaps5) : &pimrsaps5;
    pimrs[5] = _pimrsaps6.fixedflag ? (_temp54_pimrsaps6=pimrsaps6,&_temp54_pimrsaps6) : &pimrsaps6;
    pimrs[6] = _pimrsaps7.fixedflag ? (_temp55_pimrsaps7=pimrsaps7,&_temp55_pimrsaps7) : &pimrsaps7;
    pimrs[7] = _pimrsaps8.fixedflag ? (_temp56_pimrsaps8=pimrsaps8,&_temp56_pimrsaps8) : &pimrsaps8;
    pimrs[8] = _pimrsaps9.fixedflag ? (_temp57_pimrsaps9=pimrsaps9,&_temp57_pimrsaps9) : &pimrsaps9;
    pimrs[9] = _pimrsaps10.fixedflag ? (_temp58_pimrsaps10=pimrsaps10,&_temp58_pimrsaps10) : &pimrsaps10;
    pimrs[10] = _pimrsaps11.fixedflag ? (_temp59_pimrsaps11=pimrsaps11,&_temp59_pimrsaps11) : &pimrsaps11;
    pimrs[11] = _pimrsaps12.fixedflag ? (_temp60_pimrsaps12=pimrsaps12,&_temp60_pimrsaps12) : &pimrsaps12;
    pimrs[12] = _pimrsaps13.fixedflag ? (_temp61_pimrsaps13=pimrsaps13,&_temp61_pimrsaps13) : &pimrsaps13;
    pimrs[13] = _pimrsaps14.fixedflag ? (_temp62_pimrsaps14=pimrsaps14,&_temp62_pimrsaps14) : &pimrsaps14;
    pimrs[14] = _pimrsaps15.fixedflag ? (_temp63_pimrsaps15=pimrsaps15,&_temp63_pimrsaps15) : &pimrsaps15;
...
AScvinit( void )
{
    ...
    if (FAILURE==obloptimize(&asloggrd, &phygrd, total_asscan_info, total_asplane, PSD_OBL,
                             0, PSD_OBL_RESTRICT, asobl_debug, _as_newgeo.fixedflag ? (_temp64_as_newgeo=as_newgeo,&_temp64_as_newgeo) : &as_newgeo,
                             cfsrmode))
    {
    ...
    if (FAILURE==amppwgrad(asx_killer_area, asloggrd.tx_xz, 0.0, 0.0, asloggrd.xrt,
                           MIN_PLATEAU_TIME, _a_gxkas.fixedflag ? (_temp65_a_gxkas=a_gxkas,&_temp65_a_gxkas) : &a_gxkas, _pw_gxkasa.fixedflag ? (_temp66_pw_gxkasa=pw_gxkasa,&_temp66_pw_gxkasa) : &pw_gxkasa,
                           _pw_gxkas.fixedflag ? (_temp67_pw_gxkas=pw_gxkas,&_temp67_pw_gxkas) : &pw_gxkas, _pw_gxkasd.fixedflag ? (_temp68_pw_gxkasd=pw_gxkasd,&_temp68_pw_gxkasd) : &pw_gxkasd ))
    {
    ...
    if (FAILURE==amppwgrad(asz_killer_area, asloggrd.tz_xz, 0.0, 0.0, asloggrd.zrt,
                           MIN_PLATEAU_TIME, _a_gzkas.fixedflag ? (_temp69_a_gzkas=a_gzkas,&_temp69_a_gzkas) : &a_gzkas, _pw_gzkasa.fixedflag ? (_temp70_pw_gzkasa=pw_gzkasa,&_temp70_pw_gzkasa) : &pw_gzkasa,
                           _pw_gzkas.fixedflag ? (_temp71_pw_gzkas=pw_gzkas,&_temp71_pw_gzkas) : &pw_gzkas, _pw_gzkasd.fixedflag ? (_temp72_pw_gzkasd=pw_gzkasd,&_temp72_pw_gzkasd) : &pw_gzkasd ))
    {
        epic_error(use_ermes, "%s failed in AScvinit.",
                   EM_PSD_SUPPORT_FAILURE,EE_ARGS(1),
...
RScvinit( void )
{
    ...
    rfpulse[RFB_RFSHIM_SLOT].act_fa = _flip_rfbrs.fixedflag ? (_temp73_flip_rfbrs=flip_rfbrs,&_temp73_flip_rfbrs) : &flip_rfbrs;
...
DTGcvinit( void )
{
    ...
    rfpulse[RFB_DYNTG_SLOT].act_fa = _flip_rfbdtg.fixedflag ? (_temp75_flip_rfbdtg=flip_rfbdtg,&_temp75_flip_rfbdtg) : &flip_rfbdtg;
...
ExtCalcvinit( void )
{
    ...
    else
    {
        if (obloptimize(&calloggrd, &original_pgrd, calscan_info, cal_slq, PSD_AXIAL,
                        0, obl_method, ps1obl_debug, _ps1_newgeo.fixedflag ? (_temp77_ps1_newgeo=ps1_newgeo,&_temp77_ps1_newgeo) : &ps1_newgeo,
                        cfsrmode) == FAILURE)
        {
            epic_error(use_ermes, "%s failed in RScvinit.", EM_PSD_SUPPORT_FAILURE,
                       EE_ARGS(1), STRING_ARG, "obloptimize");
...
AutoCoilcvinit( void )
{
    ...
    if (obloptimize(&coilloggrd, &phygrd, coilscan_info, coil_slq, PSD_AXIAL,
                    0, obl_method, ps1obl_debug, _ps1_newgeo.fixedflag ? (_temp78_ps1_newgeo=ps1_newgeo,&_temp78_ps1_newgeo) : &ps1_newgeo,
                    cfsrmode) == FAILURE)
    {
        epic_error(use_ermes, "%s failed in RScvinit.", EM_PSD_SUPPORT_FAILURE,
                   EE_ARGS(1), STRING_ARG, "obloptimize");
...
PS1cveval( FLOAT *opthickPS )
{
    ...
    if(PSD_ON == local_tg)
    {
        ...
        if (FAILURE==ampslice(_a_gyrf1mps1.fixedflag ? (_temp79_a_gyrf1mps1=a_gyrf1mps1,&_temp79_a_gyrf1mps1) : &a_gyrf1mps1, bw_rf1mps1,TGopslthicky,gscale_rf1mps1,TYPDEF))
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
        ...
        if (optramp(_pw_gyrf1mps1a.fixedflag ? (_temp80_pw_gyrf1mps1a=pw_gyrf1mps1a,&_temp80_pw_gyrf1mps1a) : &pw_gyrf1mps1a, a_gyrf1mps1, ps1loggrd.ty, ps1loggrd.yrt,
        ...
        if (amppwgz1(_a_gy1mps1.fixedflag ? (_temp81_a_gy1mps1=a_gy1mps1,&_temp81_a_gy1mps1) : &a_gy1mps1,_pw_gy1mps1.fixedflag ? (_temp82_pw_gy1mps1=pw_gy1mps1,&_temp82_pw_gy1mps1) : &pw_gy1mps1,_pw_gy1mps1a.fixedflag ? (_temp83_pw_gy1mps1a=pw_gy1mps1a,&_temp83_pw_gy1mps1a) : &pw_gy1mps1a,_pw_gy1mps1d.fixedflag ? (_temp84_pw_gy1mps1d=pw_gy1mps1d,&_temp84_pw_gy1mps1d) : &pw_gy1mps1d,area_pulse,
    ...
    else
    {
        ...
        if (FAILURE==ampslice(_a_gzrf1mps1.fixedflag ? (_temp85_a_gzrf1mps1=a_gzrf1mps1,&_temp85_a_gzrf1mps1) : &a_gzrf1mps1, bw_rf1mps1,thickPS_mod,gscale_rf1mps1,TYPDEF))
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
        ...
        /* slice selection ramp */
        if (optramp(_pw_gzrf1mps1a.fixedflag ? (_temp86_pw_gzrf1mps1a=pw_gzrf1mps1a,&_temp86_pw_gzrf1mps1a) : &pw_gzrf1mps1a, a_gzrf1mps1, ps1loggrd.tz, ps1loggrd.zrt,
                    TYPDEF)==FAILURE) /* vmx 5/9/95 YI */ 
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
        ...
        if (amppwgz1(_a_gz1mps1.fixedflag ? (_temp87_a_gz1mps1=a_gz1mps1,&_temp87_a_gz1mps1) : &a_gz1mps1,_pw_gz1mps1.fixedflag ? (_temp88_pw_gz1mps1=pw_gz1mps1,&_temp88_pw_gz1mps1) : &pw_gz1mps1,_pw_gz1mps1a.fixedflag ? (_temp89_pw_gz1mps1a=pw_gz1mps1a,&_temp89_pw_gz1mps1a) : &pw_gz1mps1a,_pw_gz1mps1d.fixedflag ? (_temp90_pw_gz1mps1d=pw_gz1mps1d,&_temp90_pw_gz1mps1d) : &pw_gz1mps1d,area_pulse,
                     (int)(1000000),MIN_PLATEAU_TIME,ps1loggrd.zrt,ps1_tz_xz) == FAILURE)
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
                       EE_ARGS(1), STRING_ARG, "amppwgz1 for gz1mps1.");
    ...
    if (amppwgrad(ps_crusher_area, ps1_tz_xz, 0.0, 0.0, ps1loggrd.zrt,
                  MIN_PLATEAU_TIME, _a_gzrf2lmps1.fixedflag ? (_temp91_a_gzrf2lmps1=a_gzrf2lmps1,&_temp91_a_gzrf2lmps1) : &a_gzrf2lmps1, _pw_gzrf2lmps1a.fixedflag ? (_temp92_pw_gzrf2lmps1a=pw_gzrf2lmps1a,&_temp92_pw_gzrf2lmps1a) : &pw_gzrf2lmps1a,
                  _pw_gzrf2lmps1.fixedflag ? (_temp93_pw_gzrf2lmps1=pw_gzrf2lmps1,&_temp93_pw_gzrf2lmps1) : &pw_gzrf2lmps1, _pw_gzrf2lmps1d.fixedflag ? (_temp94_pw_gzrf2lmps1d=pw_gzrf2lmps1d,&_temp94_pw_gzrf2lmps1d) : &pw_gzrf2lmps1d) == FAILURE)
    {
    ...
    /* begin aps1_mod changes (GE) */
    if (FAILURE==ampslice(_a_gzrf2mps1.fixedflag ? (_temp95_a_gzrf2mps1=a_gzrf2mps1,&_temp95_a_gzrf2mps1) : &a_gzrf2mps1, bw_rf2mps1, thickPS_mod, gscale_rf2mps1, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (ampfov(_a_gxwmps1.fixedflag ? (_temp96_a_gxwmps1=a_gxwmps1,&_temp96_a_gxwmps1) : &a_gxwmps1, echo1mps1_filt.bw, mpsfov) == FAILURE)
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (optramp(_pw_gxwmps1a.fixedflag ? (_temp97_pw_gxwmps1a=pw_gxwmps1a,&_temp97_pw_gxwmps1a) : &pw_gxwmps1a, a_gxwmps1, ps1_tx, ps1_xrt,
                TYPDEF)==FAILURE)
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (amppwgx1(_a_gx1mps1.fixedflag ? (_temp98_a_gx1mps1=a_gx1mps1,&_temp98_a_gx1mps1) : &a_gx1mps1, _pw_gx1mps1.fixedflag ? (_temp99_pw_gx1mps1=pw_gx1mps1,&_temp99_pw_gx1mps1) : &pw_gx1mps1, _pw_gx1mps1a.fixedflag ? (_temp100_pw_gx1mps1a=pw_gx1mps1a,&_temp100_pw_gx1mps1a) : &pw_gx1mps1a ,_pw_gx1mps1d.fixedflag ? (_temp101_pw_gx1mps1d=pw_gx1mps1d,&_temp101_pw_gx1mps1d) : &pw_gx1mps1d, TYPSPIN,
...
CFLcveval( FLOAT opthickPS )
{
    ...
    if ( FAILURE==ampslice(_a_gzrf1cfl.fixedflag ? (_temp102_a_gzrf1cfl=a_gzrf1cfl,&_temp102_a_gzrf1cfl) : &a_gzrf1cfl, bw_rf1cfl, opthickPS, gscale_rf1cfl, TYPDEF) ) 
    {
    ...
    if ( FAILURE==optramp(_pw_gzrf1cfla.fixedflag ? (_temp103_pw_gzrf1cfla=pw_gzrf1cfla,&_temp103_pw_gzrf1cfla) : &pw_gzrf1cfla, a_gzrf1cfl, cflloggrd.tz, cflloggrd.zrt, TYPDEF) )  
    {
        epic_error(use_ermes, "%s failed for gzrf1cfl.", 
    ...
    if ( FAILURE==amppwgz1(_a_gz1cfl.fixedflag ? (_temp104_a_gz1cfl=a_gz1cfl,&_temp104_a_gz1cfl) : &a_gz1cfl, _pw_gz1cfl.fixedflag ? (_temp105_pw_gz1cfl=pw_gz1cfl,&_temp105_pw_gz1cfl) : &pw_gz1cfl, _pw_gz1cfla.fixedflag ? (_temp106_pw_gz1cfla=pw_gz1cfla,&_temp106_pw_gz1cfla) : &pw_gz1cfla, _pw_gz1cfld.fixedflag ? (_temp107_pw_gz1cfld=pw_gz1cfld,&_temp107_pw_gz1cfld) : &pw_gz1cfld, 
    ...
    if ( FAILURE==amppwgrad(area_gykcfl, cflloggrd.ty, 0.0, 0.0, cflloggrd.yrt,
                            MIN_PLATEAU_TIME, _a_gykcfl.fixedflag ? (_temp108_a_gykcfl=a_gykcfl,&_temp108_a_gykcfl) : &a_gykcfl, _pw_gykcfla.fixedflag ? (_temp109_pw_gykcfla=pw_gykcfla,&_temp109_pw_gykcfla) : &pw_gykcfla,
                            _pw_gykcfl.fixedflag ? (_temp110_pw_gykcfl=pw_gykcfl,&_temp110_pw_gykcfl) : &pw_gykcfl, _pw_gykcfld.fixedflag ? (_temp111_pw_gykcfld=pw_gykcfld,&_temp111_pw_gykcfld) : &pw_gykcfld) ) 
    {
        epic_error(use_ermes, "%s failed in cfl.", EM_PSD_SUPPORT_FAILURE,
...
RCVNcveval( void )
{
    ...
    if ( FAILURE==amppwgrad( area_gxkrcvn, rcvnloggrd.tx_xyz, 0.0, 0.0, rcvnloggrd.xrt,
                             MIN_PLATEAU_TIME, _a_gxkrcvn.fixedflag ? (_temp112_a_gxkrcvn=a_gxkrcvn,&_temp112_a_gxkrcvn) : &a_gxkrcvn, _pw_gxkrcvna.fixedflag ? (_temp113_pw_gxkrcvna=pw_gxkrcvna,&_temp113_pw_gxkrcvna) : &pw_gxkrcvna,
                             _pw_gxkrcvn.fixedflag ? (_temp114_pw_gxkrcvn=pw_gxkrcvn,&_temp114_pw_gxkrcvn) : &pw_gxkrcvn, _pw_gxkrcvnd.fixedflag ? (_temp115_pw_gxkrcvnd=pw_gxkrcvnd,&_temp115_pw_gxkrcvnd) : &pw_gxkrcvnd ) )
    {
    ...
    if ( FAILURE==amppwgrad( area_gykrcvn, rcvnloggrd.ty_xyz, 0.0, 0.0, rcvnloggrd.yrt,
                             MIN_PLATEAU_TIME, _a_gykrcvn.fixedflag ? (_temp116_a_gykrcvn=a_gykrcvn,&_temp116_a_gykrcvn) : &a_gykrcvn, _pw_gykrcvna.fixedflag ? (_temp117_pw_gykrcvna=pw_gykrcvna,&_temp117_pw_gykrcvna) : &pw_gykrcvna,
                             _pw_gykrcvn.fixedflag ? (_temp118_pw_gykrcvn=pw_gykrcvn,&_temp118_pw_gykrcvn) : &pw_gykrcvn, _pw_gykrcvnd.fixedflag ? (_temp119_pw_gykrcvnd=pw_gykrcvnd,&_temp119_pw_gykrcvnd) : &pw_gykrcvnd ) )
    {
        epic_error(use_ermes, "Support routine %s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if ( FAILURE==amppwgrad( area_gzkrcvn, rcvnloggrd.tz_xyz, 0.0, 0.0, rcvnloggrd.zrt,
                             MIN_PLATEAU_TIME, _a_gzkrcvn.fixedflag ? (_temp120_a_gzkrcvn=a_gzkrcvn,&_temp120_a_gzkrcvn) : &a_gzkrcvn, _pw_gzkrcvna.fixedflag ? (_temp121_pw_gzkrcvna=pw_gzkrcvna,&_temp121_pw_gzkrcvna) : &pw_gzkrcvna,
                             _pw_gzkrcvn.fixedflag ? (_temp122_pw_gzkrcvn=pw_gzkrcvn,&_temp122_pw_gzkrcvn) : &pw_gzkrcvn, _pw_gzkrcvnd.fixedflag ? (_temp123_pw_gzkrcvnd=pw_gzkrcvnd,&_temp123_pw_gzkrcvnd) : &pw_gzkrcvnd ) )
    {
        epic_error(use_ermes, "Support routine %s failed", EM_PSD_SUPPORT_FAILURE,
...
AScveval( void )
{
    ...
    /*   Slice Selection                                    */
    /********************************************************/
    if (FAILURE==ampslice(_a_gzrf1as.fixedflag ? (_temp124_a_gzrf1as=a_gzrf1as,&_temp124_a_gzrf1as) : &a_gzrf1as, bw_rf1as, asslthick, gscale_rf1as, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==optramp(_pw_gzrf1asa.fixedflag ? (_temp125_pw_gzrf1asa=pw_gzrf1asa,&_temp125_pw_gzrf1asa) : &pw_gzrf1asa, a_gzrf1as, asloggrd.tz_xyz, asloggrd.zrt, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if ( FAILURE==amppwgz1(_a_gz1as.fixedflag ? (_temp126_a_gz1as=a_gz1as,&_temp126_a_gz1as) : &a_gz1as, _pw_gz1as.fixedflag ? (_temp127_pw_gz1as=pw_gz1as,&_temp127_pw_gz1as) : &pw_gz1as, _pw_gz1asa.fixedflag ? (_temp128_pw_gz1asa=pw_gz1asa,&_temp128_pw_gz1asa) : &pw_gz1asa, _pw_gz1asd.fixedflag ? (_temp129_pw_gz1asd=pw_gz1asd,&_temp129_pw_gz1asd) : &pw_gz1asd,
    ...
    if ( FAILURE==ampfov(_a_gxwas.fixedflag ? (_temp130_a_gxwas=a_gxwas,&_temp130_a_gxwas) : &a_gxwas, echo1as_filt.bw, asfov) )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==optramp(_pw_gxwasa.fixedflag ? (_temp131_pw_gxwasa=pw_gxwasa,&_temp131_pw_gxwasa) : &pw_gxwasa, a_gxwas, asloggrd.tx_xyz, asloggrd.xrt, TYPDEF)) 
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if ( FAILURE==amppwgx1(_a_gx1as.fixedflag ? (_temp132_a_gx1as=a_gx1as,&_temp132_a_gx1as) : &a_gx1as, _pw_gx1as.fixedflag ? (_temp133_pw_gx1as=pw_gx1as,&_temp133_pw_gx1as) : &pw_gx1as, _pw_gx1asa.fixedflag ? (_temp134_pw_gx1asa=pw_gx1asa,&_temp134_pw_gx1asa) : &pw_gx1asa, _pw_gx1asd.fixedflag ? (_temp135_pw_gx1asd=pw_gx1asd,&_temp135_pw_gx1asd) : &pw_gx1asd,
    ...
    if ( FAILURE==endview((int)(asyres), _endview_iampas.fixedflag ? (_temp136_endview_iampas=endview_iampas,&_temp136_endview_iampas) : &endview_iampas) )
    {
    ...
    if ( FAILURE==amppwtpe(_a_gy1asa.fixedflag ? (_temp137_a_gy1asa=a_gy1asa,&_temp137_a_gy1asa) : &a_gy1asa, _a_gy1asb.fixedflag ? (_temp138_a_gy1asb=a_gy1asb,&_temp138_a_gy1asb) : &a_gy1asb, _pw_gy1as.fixedflag ? (_temp139_pw_gy1as=pw_gy1as,&_temp139_pw_gy1as) : &pw_gy1as, _pw_gy1asa.fixedflag ? (_temp140_pw_gy1asa=pw_gy1asa,&_temp140_pw_gy1asa) : &pw_gy1asa, _pw_gy1asd.fixedflag ? (_temp141_pw_gy1asd=pw_gy1asd,&_temp141_pw_gy1asd) : &pw_gy1asd,
...
CFHcveval( FLOAT opthickPS )
{
    ...
    psd_cfhleftcrush.attack = _pw_gzrf2lcfha.fixedflag ? (_temp142_pw_gzrf2lcfha=pw_gzrf2lcfha,&_temp142_pw_gzrf2lcfha) : &pw_gzrf2lcfha;
    psd_cfhleftcrush.decay = _pw_gzrf2lcfhd.fixedflag ? (_temp143_pw_gzrf2lcfhd=pw_gzrf2lcfhd,&_temp143_pw_gzrf2lcfhd) : &pw_gzrf2lcfhd;
    psd_cfhleftcrush.pw = _pw_gzrf2lcfh.fixedflag ? (_temp144_pw_gzrf2lcfh=pw_gzrf2lcfh,&_temp144_pw_gzrf2lcfh) : &pw_gzrf2lcfh;
    psd_cfhleftcrush.amp = _a_gzrf2lcfh.fixedflag ? (_temp145_a_gzrf2lcfh=a_gzrf2lcfh,&_temp145_a_gzrf2lcfh) : &a_gzrf2lcfh;
 
    psd_cfhrightcrush.attack = _pw_gzrf2rcfha.fixedflag ? (_temp146_pw_gzrf2rcfha=pw_gzrf2rcfha,&_temp146_pw_gzrf2rcfha) : &pw_gzrf2rcfha;
    psd_cfhrightcrush.decay = _pw_gzrf2rcfhd.fixedflag ? (_temp147_pw_gzrf2rcfhd=pw_gzrf2rcfhd,&_temp147_pw_gzrf2rcfhd) : &pw_gzrf2rcfhd;
    psd_cfhrightcrush.pw = _pw_gzrf2rcfh.fixedflag ? (_temp148_pw_gzrf2rcfh=pw_gzrf2rcfh,&_temp148_pw_gzrf2rcfh) : &pw_gzrf2rcfh;
    psd_cfhrightcrush.amp = _a_gzrf2rcfh.fixedflag ? (_temp149_a_gzrf2rcfh=a_gzrf2rcfh,&_temp149_a_gzrf2rcfh) : &a_gzrf2rcfh;

    if( presscfh_ctrl != PRESSCFH_NONE && cfh_steam_flag == PSD_ON )
    {
        psd_cfhrightcrush.attack = _pw_gzrf3rcfha.fixedflag ? (_temp150_pw_gzrf3rcfha=pw_gzrf3rcfha,&_temp150_pw_gzrf3rcfha) : &pw_gzrf3rcfha;
        psd_cfhrightcrush.decay = _pw_gzrf3rcfhd.fixedflag ? (_temp151_pw_gzrf3rcfhd=pw_gzrf3rcfhd,&_temp151_pw_gzrf3rcfhd) : &pw_gzrf3rcfhd;
        psd_cfhrightcrush.pw = _pw_gzrf3rcfh.fixedflag ? (_temp152_pw_gzrf3rcfh=pw_gzrf3rcfh,&_temp152_pw_gzrf3rcfh) : &pw_gzrf3rcfh;
        psd_cfhrightcrush.amp = _a_gzrf3rcfh.fixedflag ? (_temp153_a_gzrf3rcfh=a_gzrf3rcfh,&_temp153_a_gzrf3rcfh) : &a_gzrf3rcfh;
    }

    if( presscfh_ctrl != PRESSCFH_NONE && cfh_steam_flag == PSD_ON )
    {
        setuprfpulse(RF1_CFH_SLOT, _pw_rf1cfh.fixedflag ? (_temp154_pw_rf1cfh=pw_rf1cfh,&_temp154_pw_rf1cfh) : &pw_rf1cfh, _a_rf1cfh.fixedflag ? (_temp155_a_rf1cfh=a_rf1cfh,&_temp155_a_rf1cfh) : &a_rf1cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf1cfh.fixedflag ? (_temp156_flip_rf1cfh=flip_rf1cfh,&_temp156_flip_rf1cfh) : &flip_rf1cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf1cfh.fixedflag ? (_temp157_res_rf1cfh=res_rf1cfh,&_temp157_res_rf1cfh) : &res_rf1cfh, 0, _wg_rf1cfh.fixedflag ? (_temp158_wg_rf1cfh=wg_rf1cfh,&_temp158_wg_rf1cfh) : &wg_rf1cfh, 1, rfpulse);

        setuprfpulse(RF2_CFH_SLOT, _pw_rf2cfh.fixedflag ? (_temp159_pw_rf2cfh=pw_rf2cfh,&_temp159_pw_rf2cfh) : &pw_rf2cfh, _a_rf2cfh.fixedflag ? (_temp160_a_rf2cfh=a_rf2cfh,&_temp160_a_rf2cfh) : &a_rf2cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf2cfh.fixedflag ? (_temp161_flip_rf2cfh=flip_rf2cfh,&_temp161_flip_rf2cfh) : &flip_rf2cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf2cfh.fixedflag ? (_temp162_res_rf2cfh=res_rf2cfh,&_temp162_res_rf2cfh) : &res_rf2cfh, 0, _wg_rf2cfh.fixedflag ? (_temp163_wg_rf2cfh=wg_rf2cfh,&_temp163_wg_rf2cfh) : &wg_rf2cfh, 1, rfpulse);

        setuprfpulse(RF3_CFH_SLOT, _pw_rf3cfh.fixedflag ? (_temp164_pw_rf3cfh=pw_rf3cfh,&_temp164_pw_rf3cfh) : &pw_rf3cfh, _a_rf3cfh.fixedflag ? (_temp165_a_rf3cfh=a_rf3cfh,&_temp165_a_rf3cfh) : &a_rf3cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf3cfh.fixedflag ? (_temp166_flip_rf3cfh=flip_rf3cfh,&_temp166_flip_rf3cfh) : &flip_rf3cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf3cfh.fixedflag ? (_temp167_res_rf3cfh=res_rf3cfh,&_temp167_res_rf3cfh) : &res_rf3cfh, 0, _wg_rf3cfh.fixedflag ? (_temp168_wg_rf3cfh=wg_rf3cfh,&_temp168_wg_rf3cfh) : &wg_rf3cfh, 1, rfpulse);

        setuprfpulse(RF4_CFH_SLOT, _pw_rf4cfh.fixedflag ? (_temp169_pw_rf4cfh=pw_rf4cfh,&_temp169_pw_rf4cfh) : &pw_rf4cfh, _a_rf4cfh.fixedflag ? (_temp170_a_rf4cfh=a_rf4cfh,&_temp170_a_rf4cfh) : &a_rf4cfh, SAR_ABS_SINC3, SAR_PSINC3,
                     SAR_ASINC3, SAR_DTYCYC_SINC3, SAR_MAXPW_SINC3, 1,
                     MAX_B1_SINC3_90, MAX_INT_B1_SQ_SINC3_90,
                     MAX_RMS_B1_SINC3_90, 90.0, _flip_rf4cfh.fixedflag ? (_temp171_flip_rf4cfh=flip_rf4cfh,&_temp171_flip_rf4cfh) : &flip_rf4cfh, 3200.0,
                     3750, PSD_CFH_ON, 0,
                     0, 0, _res_rf4cfh.fixedflag ? (_temp172_res_rf4cfh=res_rf4cfh,&_temp172_res_rf4cfh) : &res_rf4cfh, 0, _wg_rf4cfh.fixedflag ? (_temp173_wg_rf4cfh=wg_rf4cfh,&_temp173_wg_rf4cfh) : &wg_rf4cfh, 1, rfpulse);
    ...
    {
        setuprfpulse(RF1_CFH_SLOT, _pw_rf1cfh.fixedflag ? (_temp174_pw_rf1cfh=pw_rf1cfh,&_temp174_pw_rf1cfh) : &pw_rf1cfh, _a_rf1cfh.fixedflag ? (_temp175_a_rf1cfh=a_rf1cfh,&_temp175_a_rf1cfh) : &a_rf1cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf1cfh.fixedflag ? (_temp176_flip_rf1cfh=flip_rf1cfh,&_temp176_flip_rf1cfh) : &flip_rf1cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf1cfh.fixedflag ? (_temp177_res_rf1cfh=res_rf1cfh,&_temp177_res_rf1cfh) : &res_rf1cfh, 0, _wg_rf1cfh.fixedflag ? (_temp178_wg_rf1cfh=wg_rf1cfh,&_temp178_wg_rf1cfh) : &wg_rf1cfh, 1, rfpulse);

        setuprfpulse(RF2_CFH_SLOT, _pw_rf2cfh.fixedflag ? (_temp179_pw_rf2cfh=pw_rf2cfh,&_temp179_pw_rf2cfh) : &pw_rf2cfh, _a_rf2cfh.fixedflag ? (_temp180_a_rf2cfh=a_rf2cfh,&_temp180_a_rf2cfh) : &a_rf2cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf2cfh.fixedflag ? (_temp181_flip_rf2cfh=flip_rf2cfh,&_temp181_flip_rf2cfh) : &flip_rf2cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf2cfh.fixedflag ? (_temp182_res_rf2cfh=res_rf2cfh,&_temp182_res_rf2cfh) : &res_rf2cfh, 0, _wg_rf2cfh.fixedflag ? (_temp183_wg_rf2cfh=wg_rf2cfh,&_temp183_wg_rf2cfh) : &wg_rf2cfh, 1, rfpulse);

        setuprfpulse(RF3_CFH_SLOT, _pw_rf3cfh.fixedflag ? (_temp184_pw_rf3cfh=pw_rf3cfh,&_temp184_pw_rf3cfh) : &pw_rf3cfh, _a_rf3cfh.fixedflag ? (_temp185_a_rf3cfh=a_rf3cfh,&_temp185_a_rf3cfh) : &a_rf3cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf3cfh.fixedflag ? (_temp186_flip_rf3cfh=flip_rf3cfh,&_temp186_flip_rf3cfh) : &flip_rf3cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf3cfh.fixedflag ? (_temp187_res_rf3cfh=res_rf3cfh,&_temp187_res_rf3cfh) : &res_rf3cfh, 0, _wg_rf3cfh.fixedflag ? (_temp188_wg_rf3cfh=wg_rf3cfh,&_temp188_wg_rf3cfh) : &wg_rf3cfh, 1, rfpulse);

        setuprfpulse(RF4_CFH_SLOT, _pw_rf4cfh.fixedflag ? (_temp189_pw_rf4cfh=pw_rf4cfh,&_temp189_pw_rf4cfh) : &pw_rf4cfh, _a_rf4cfh.fixedflag ? (_temp190_a_rf4cfh=a_rf4cfh,&_temp190_a_rf4cfh) : &a_rf4cfh, SAR_ABS_SINC1, SAR_PSINC1,
                     SAR_ASINC1, SAR_DTYCYC_SINC1, SAR_MAXPW_SINC1, 1,
                     MAX_B1_SINC1_90, MAX_INT_B1_SQ_SINC1_90,
                     MAX_RMS_B1_SINC1_90, 90.0, _flip_rf4cfh.fixedflag ? (_temp191_flip_rf4cfh=flip_rf4cfh,&_temp191_flip_rf4cfh) : &flip_rf4cfh, 3200.0,
                     1250, PSD_CFH_ON, 0,
                     0, 0, _res_rf4cfh.fixedflag ? (_temp192_res_rf4cfh=res_rf4cfh,&_temp192_res_rf4cfh) : &res_rf4cfh, 0, _wg_rf4cfh.fixedflag ? (_temp193_wg_rf4cfh=wg_rf4cfh,&_temp193_wg_rf4cfh) : &wg_rf4cfh, 1, rfpulse);
    ...
    if (PSD_ON == PSir)
    {
        ...
        if(amppwgrad(cfhir_killer_area, cfhloggrd.ty_yz, 0.0, 0.0, cfhloggrd.yrt,
                     MIN_PLATEAU_TIME, _a_gyrf0kcfh.fixedflag ? (_temp194_a_gyrf0kcfh=a_gyrf0kcfh,&_temp194_a_gyrf0kcfh) : &a_gyrf0kcfh, _pw_gyrf0kcfha.fixedflag ? (_temp195_pw_gyrf0kcfha=pw_gyrf0kcfha,&_temp195_pw_gyrf0kcfha) : &pw_gyrf0kcfha, _pw_gyrf0kcfh.fixedflag ? (_temp196_pw_gyrf0kcfh=pw_gyrf0kcfh,&_temp196_pw_gyrf0kcfh) : &pw_gyrf0kcfh, _pw_gyrf0kcfhd.fixedflag ? (_temp197_pw_gyrf0kcfhd=pw_gyrf0kcfhd,&_temp197_pw_gyrf0kcfhd) : &pw_gyrf0kcfhd) == FAILURE) 
        {
            epic_error(use_ermes, "%s failed in PScveval.", EM_PSD_SUPPORT_FAILURE,
        ...
        if(ampslice(_a_gzrf0cfh.fixedflag ? (_temp198_a_gzrf0cfh=a_gzrf0cfh,&_temp198_a_gzrf0cfh) : &a_gzrf0cfh, bw_rf0cfh, ((presscfh_ctrl == PRESSCFH_NONE) ? opthickPS : presscfh_ir_slthick),
        ...
        /* YMSmr09211  04/26/2006 YI */
        if(optramp(_pw_gzrf0cfha.fixedflag ? (_temp199_pw_gzrf0cfha=pw_gzrf0cfha,&_temp199_pw_gzrf0cfha) : &pw_gzrf0cfha,a_gzrf0cfh, cfhloggrd.tz, cfhloggrd.zrt, TYPDEF)==FAILURE) 
        {
    ...
    if (ampslice(_a_gzrf1cfh.fixedflag ? (_temp200_a_gzrf1cfh=a_gzrf1cfh,&_temp200_a_gzrf1cfh) : &a_gzrf1cfh, bw_rf1cfh, ( (presscfh_ctrl == PRESSCFH_NONE) ? opthickPS : presscfh_slthick),
    ...
    /* YMSmr09211  04/26/2006 YI */
    if (optramp(_pw_gzrf1cfha.fixedflag ? (_temp201_pw_gzrf1cfha=pw_gzrf1cfha,&_temp201_pw_gzrf1cfha) : &pw_gzrf1cfha, a_gzrf1cfh, cfhloggrd.tz, cfhloggrd.zrt, TYPDEF) == FAILURE) 
    {
    ...
    if( presscfh_ctrl != PRESSCFH_NONE ) 
    {
        ...
        if (optramp(_pw_gxrf2cfha.fixedflag ? (_temp202_pw_gxrf2cfha=pw_gxrf2cfha,&_temp202_pw_gxrf2cfha) : &pw_gxrf2cfha, a_gxrf2cfh, cfhloggrd.tx, cfhloggrd.xrt, TYPDEF) == FAILURE) 
        {
        ...
        if (optramp(_pw_gyrf3cfha.fixedflag ? (_temp203_pw_gyrf3cfha=pw_gyrf3cfha,&_temp203_pw_gyrf3cfha) : &pw_gyrf3cfha, a_gyrf3cfh, cfhloggrd.ty, cfhloggrd.yrt, TYPDEF) == FAILURE) 
        {
        ...
        if( presscfh_ctrl == PRESSCFH_SHIMVOL_SLICE )
        {
            ...
            if (optramp(_pw_gzrf4cfha.fixedflag ? (_temp204_pw_gzrf4cfha=pw_gzrf4cfha,&_temp204_pw_gzrf4cfha) : &pw_gzrf4cfha, a_gzrf4cfh, loggrd.tz, loggrd.zrt, TYPDEF) == FAILURE) 
            {
        ...
        if( cfh_steam_flag != PSD_ON )
        {
            ...
            if (amppwgrad(cfh_crusher_area, target_cfh_crusher, 0.0, 0.0, cfhloggrd.zrt,
                          MIN_PLATEAU_TIME, _a_gzrf2rcfh.fixedflag ? (_temp205_a_gzrf2rcfh=a_gzrf2rcfh,&_temp205_a_gzrf2rcfh) : &a_gzrf2rcfh, _pw_gzrf2rcfha.fixedflag ? (_temp206_pw_gzrf2rcfha=pw_gzrf2rcfha,&_temp206_pw_gzrf2rcfha) : &pw_gzrf2rcfha,
                          _pw_gzrf2rcfh.fixedflag ? (_temp207_pw_gzrf2rcfh=pw_gzrf2rcfh,&_temp207_pw_gzrf2rcfh) : &pw_gzrf2rcfh, _pw_gzrf2rcfhd.fixedflag ? (_temp208_pw_gzrf2rcfhd=pw_gzrf2rcfhd,&_temp208_pw_gzrf2rcfhd) : &pw_gzrf2rcfhd) == FAILURE) 
            {
        ...
        else
        {
            if (amppwgrad(cfh_crusher_area, target_cfh_crusher2, 0.0, 0.0, cfhloggrd.zrt,
                          MIN_PLATEAU_TIME, _a_gzrf3rcfh.fixedflag ? (_temp209_a_gzrf3rcfh=a_gzrf3rcfh,&_temp209_a_gzrf3rcfh) : &a_gzrf3rcfh, _pw_gzrf3rcfha.fixedflag ? (_temp210_pw_gzrf3rcfha=pw_gzrf3rcfha,&_temp210_pw_gzrf3rcfha) : &pw_gzrf3rcfha,
                          _pw_gzrf3rcfh.fixedflag ? (_temp211_pw_gzrf3rcfh=pw_gzrf3rcfh,&_temp211_pw_gzrf3rcfh) : &pw_gzrf3rcfh, _pw_gzrf3rcfhd.fixedflag ? (_temp212_pw_gzrf3rcfhd=pw_gzrf3rcfhd,&_temp212_pw_gzrf3rcfhd) : &pw_gzrf3rcfhd) == FAILURE) 
            {
                epic_error(use_ermes, "%s failed in PScveval.",
                           EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
        ...
        if( cfh_steam_flag != PSD_ON )
        {
            /* Z CRUSHER CVs */ /* YMSmr09211  04/26/2006 YI */
            if (amppwgrad(cfh_crusher_area, target_cfh_crusher2, 0.0, 0.0, cfhloggrd.zrt,
                          MIN_PLATEAU_TIME, _a_gzrf3rcfh.fixedflag ? (_temp213_a_gzrf3rcfh=a_gzrf3rcfh,&_temp213_a_gzrf3rcfh) : &a_gzrf3rcfh, _pw_gzrf3rcfha.fixedflag ? (_temp214_pw_gzrf3rcfha=pw_gzrf3rcfha,&_temp214_pw_gzrf3rcfha) : &pw_gzrf3rcfha,
                          _pw_gzrf3rcfh.fixedflag ? (_temp215_pw_gzrf3rcfh=pw_gzrf3rcfh,&_temp215_pw_gzrf3rcfh) : &pw_gzrf3rcfh, _pw_gzrf3rcfhd.fixedflag ? (_temp216_pw_gzrf3rcfhd=pw_gzrf3rcfhd,&_temp216_pw_gzrf3rcfhd) : &pw_gzrf3rcfhd) == FAILURE) 
            {
            ...
            /* Z CRUSHER CVs */ /* YMSmr09211  04/26/2006 YI */
            if (amppwgrad(cfh_crusher_area, target_cfh_crusher2, 0.0, 0.0, cfhloggrd.zrt,
                          MIN_PLATEAU_TIME, _a_gzrf3lcfh.fixedflag ? (_temp217_a_gzrf3lcfh=a_gzrf3lcfh,&_temp217_a_gzrf3lcfh) : &a_gzrf3lcfh, _pw_gzrf3lcfha.fixedflag ? (_temp218_pw_gzrf3lcfha=pw_gzrf3lcfha,&_temp218_pw_gzrf3lcfha) : &pw_gzrf3lcfha,
                          _pw_gzrf3lcfh.fixedflag ? (_temp219_pw_gzrf3lcfh=pw_gzrf3lcfh,&_temp219_pw_gzrf3lcfh) : &pw_gzrf3lcfh, _pw_gzrf3lcfhd.fixedflag ? (_temp220_pw_gzrf3lcfhd=pw_gzrf3lcfhd,&_temp220_pw_gzrf3lcfhd) : &pw_gzrf3lcfhd) == FAILURE) 
            {
                epic_error(use_ermes, "%s failed in PScveval.", EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
            ...
            if( presscfh_ctrl == PRESSCFH_SHIMVOL_SLICE )
            {
                /* Z CRUSHER CVs */ /* YMSmr09211  04/26/2006 YI */
                if (amppwgrad(cfh_crusher_area, target_cfh_crusher2, 0.0, 0.0, cfhloggrd.zrt,
                              MIN_PLATEAU_TIME, _a_gzrf4rcfh.fixedflag ? (_temp221_a_gzrf4rcfh=a_gzrf4rcfh,&_temp221_a_gzrf4rcfh) : &a_gzrf4rcfh, _pw_gzrf4rcfha.fixedflag ? (_temp222_pw_gzrf4rcfha=pw_gzrf4rcfha,&_temp222_pw_gzrf4rcfha) : &pw_gzrf4rcfha,
                              _pw_gzrf4rcfh.fixedflag ? (_temp223_pw_gzrf4rcfh=pw_gzrf4rcfh,&_temp223_pw_gzrf4rcfh) : &pw_gzrf4rcfh, _pw_gzrf4rcfhd.fixedflag ? (_temp224_pw_gzrf4rcfhd=pw_gzrf4rcfhd,&_temp224_pw_gzrf4rcfhd) : &pw_gzrf4rcfhd) == FAILURE) 
                {
                    epic_error(use_ermes, "%s failed in PScveval.", EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
                ...
                /* Z CRUSHER CVs */ /* YMSmr09211  04/26/2006 YI */
                if (amppwgrad(cfh_crusher_area, target_cfh_crusher2, 0.0, 0.0, cfhloggrd.zrt,
                              MIN_PLATEAU_TIME, _a_gzrf4lcfh.fixedflag ? (_temp225_a_gzrf4lcfh=a_gzrf4lcfh,&_temp225_a_gzrf4lcfh) : &a_gzrf4lcfh, _pw_gzrf4lcfha.fixedflag ? (_temp226_pw_gzrf4lcfha=pw_gzrf4lcfha,&_temp226_pw_gzrf4lcfha) : &pw_gzrf4lcfha,
                              _pw_gzrf4lcfh.fixedflag ? (_temp227_pw_gzrf4lcfh=pw_gzrf4lcfh,&_temp227_pw_gzrf4lcfh) : &pw_gzrf4lcfh, _pw_gzrf4lcfhd.fixedflag ? (_temp228_pw_gzrf4lcfhd=pw_gzrf4lcfhd,&_temp228_pw_gzrf4lcfhd) : &pw_gzrf4lcfhd) == FAILURE) 
                {
                    epic_error(use_ermes, "%s failed in PScveval.", EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
        ...
        {
            if (amppwgrad(cfh_crusher_area, target_cfh_crusher, 0.0, 0.0, cfhloggrd.zrt,
                          MIN_PLATEAU_TIME, _a_gzrf2rcfh.fixedflag ? (_temp229_a_gzrf2rcfh=a_gzrf2rcfh,&_temp229_a_gzrf2rcfh) : &a_gzrf2rcfh, _pw_gzrf2rcfha.fixedflag ? (_temp230_pw_gzrf2rcfha=pw_gzrf2rcfha,&_temp230_pw_gzrf2rcfha) : &pw_gzrf2rcfha,
                          _pw_gzrf2rcfh.fixedflag ? (_temp231_pw_gzrf2rcfh=pw_gzrf2rcfh,&_temp231_pw_gzrf2rcfh) : &pw_gzrf2rcfh, _pw_gzrf2rcfhd.fixedflag ? (_temp232_pw_gzrf2rcfhd=pw_gzrf2rcfhd,&_temp232_pw_gzrf2rcfhd) : &pw_gzrf2rcfhd) == FAILURE) 
            {
                epic_error(use_ermes, "%s failed in PScveval.", EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
                           STRING_ARG, "amppwgrad:gzrf2rcfh"); 
                return FAILURE;
            ...
            if (amppwgrad(cfh_crusher_area, target_cfh_crusher2, 0.0, 0.0, cfhloggrd.zrt,
                          MIN_PLATEAU_TIME, _a_gzrf3lcfh.fixedflag ? (_temp233_a_gzrf3lcfh=a_gzrf3lcfh,&_temp233_a_gzrf3lcfh) : &a_gzrf3lcfh, _pw_gzrf3lcfha.fixedflag ? (_temp234_pw_gzrf3lcfha=pw_gzrf3lcfha,&_temp234_pw_gzrf3lcfha) : &pw_gzrf3lcfha,
                          _pw_gzrf3lcfh.fixedflag ? (_temp235_pw_gzrf3lcfh=pw_gzrf3lcfh,&_temp235_pw_gzrf3lcfh) : &pw_gzrf3lcfh, _pw_gzrf3lcfhd.fixedflag ? (_temp236_pw_gzrf3lcfhd=pw_gzrf3lcfhd,&_temp236_pw_gzrf3lcfhd) : &pw_gzrf3lcfhd) == FAILURE) 
            {
                epic_error(use_ermes, "%s failed in PScveval.", EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
                           STRING_ARG, "amppwgrad:gzrf3lcfh"); 
                return FAILURE;
        ...
        if (opspf == 0) 
        {
            ...
            if (optramp(_pw_gxrf2cfha.fixedflag ? (_temp237_pw_gxrf2cfha=pw_gxrf2cfha,&_temp237_pw_gxrf2cfha) : &pw_gxrf2cfha, a_gxrf2cfh, cfhloggrd.tx, cfhloggrd.xrt, TYPDEF) == FAILURE) 
            {
        ...
        else 
        {
            ...
            if (optramp(_pw_gyrf2cfha.fixedflag ? (_temp238_pw_gyrf2cfha=pw_gyrf2cfha,&_temp238_pw_gyrf2cfha) : &pw_gyrf2cfha, a_gyrf2cfh, cfhloggrd.ty, cfhloggrd.yrt, TYPDEF) == FAILURE) 
            {
        ...
        /* Z CRUSHER CVs */ /* YMSmr09211  04/26/2006 YI */
        if (amppwgrad(cfh_crusher_area, target_cfh_crusher, 0.0, 0.0, cfhloggrd.zrt,
                      MIN_PLATEAU_TIME, _a_gzrf2rcfh.fixedflag ? (_temp239_a_gzrf2rcfh=a_gzrf2rcfh,&_temp239_a_gzrf2rcfh) : &a_gzrf2rcfh, _pw_gzrf2rcfha.fixedflag ? (_temp240_pw_gzrf2rcfha=pw_gzrf2rcfha,&_temp240_pw_gzrf2rcfha) : &pw_gzrf2rcfha,
                      _pw_gzrf2rcfh.fixedflag ? (_temp241_pw_gzrf2rcfh=pw_gzrf2rcfh,&_temp241_pw_gzrf2rcfh) : &pw_gzrf2rcfh, _pw_gzrf2rcfhd.fixedflag ? (_temp242_pw_gzrf2rcfhd=pw_gzrf2rcfhd,&_temp242_pw_gzrf2rcfhd) : &pw_gzrf2rcfhd) == FAILURE) 
        {
            epic_error(use_ermes, "%s failed in PScveval.",
                       EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
                       STRING_ARG, "amppwgrad:gzrf2rcfh"); 
    ...
    if (amppwlcrsh(&psd_cfhleftcrush, &psd_cfhrightcrush,
                   ...
                   MIN_PLATEAU_TIME, cfhloggrd.zrt, _dummy_pw.fixedflag ? (_temp243_dummy_pw=dummy_pw,&_temp243_dummy_pw) : &dummy_pw) == FAILURE) 
    {
    ...
    if( presscfh_ctrl != PRESSCFH_NONE && cfh_steam_flag == PSD_ON )
    {
        ...
        if (amppwgrad(area_g1, cfhloggrd.tx, 0.0, 0.0, cfhloggrd.xrt,
                      MIN_PLATEAU_TIME, _a_gx1cfh.fixedflag ? (_temp244_a_gx1cfh=a_gx1cfh,&_temp244_a_gx1cfh) : &a_gx1cfh, _pw_gx1cfha.fixedflag ? (_temp245_pw_gx1cfha=pw_gx1cfha,&_temp245_pw_gx1cfha) : &pw_gx1cfha,
                      _pw_gx1cfh.fixedflag ? (_temp246_pw_gx1cfh=pw_gx1cfh,&_temp246_pw_gx1cfh) : &pw_gx1cfh, _pw_gx1cfhd.fixedflag ? (_temp247_pw_gx1cfhd=pw_gx1cfhd,&_temp247_pw_gx1cfhd) : &pw_gx1cfhd) == FAILURE) 
        {
            epic_error(use_ermes, "%s failed in PScveval.",
                       EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
        ...
        if (amppwgrad(area_g1, cfhloggrd.ty, 0.0, 0.0, cfhloggrd.yrt,
                      MIN_PLATEAU_TIME, _a_gy1cfh.fixedflag ? (_temp248_a_gy1cfh=a_gy1cfh,&_temp248_a_gy1cfh) : &a_gy1cfh, _pw_gy1cfha.fixedflag ? (_temp249_pw_gy1cfha=pw_gy1cfha,&_temp249_pw_gy1cfha) : &pw_gy1cfha,
                      _pw_gy1cfh.fixedflag ? (_temp250_pw_gy1cfh=pw_gy1cfh,&_temp250_pw_gy1cfh) : &pw_gy1cfh, _pw_gy1cfhd.fixedflag ? (_temp251_pw_gy1cfhd=pw_gy1cfhd,&_temp251_pw_gy1cfhd) : &pw_gy1cfhd) == FAILURE) 
        {
            epic_error(use_ermes, "%s failed in PScveval.",
                       EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
    ...
    if (amppwgrad(area_gykcfh, cfhloggrd.ty, 0.0, 0.0, cfhloggrd.yrt,
                  MIN_PLATEAU_TIME, _a_gykcfh.fixedflag ? (_temp252_a_gykcfh=a_gykcfh,&_temp252_a_gykcfh) : &a_gykcfh, _pw_gykcfha.fixedflag ? (_temp253_pw_gykcfha=pw_gykcfha,&_temp253_pw_gykcfha) : &pw_gykcfha,
                  _pw_gykcfh.fixedflag ? (_temp254_pw_gykcfh=pw_gykcfh,&_temp254_pw_gykcfh) : &pw_gykcfh, _pw_gykcfhd.fixedflag ? (_temp255_pw_gykcfhd=pw_gykcfhd,&_temp255_pw_gykcfhd) : &pw_gykcfhd) == FAILURE) 
    {
        epic_error(use_ermes, "%s failed in cfh.", EM_PSD_SUPPORT_FAILURE,
...
PScveval( void )
{
    ...
    if (obloptimize(&cfhloggrd, &phygrd, cfh_info, num_cfhlocs,
                    PSD_OBL, 0, obl_method, cfhobl_debug, _cfh_newgeo.fixedflag ? (_temp256_cfh_newgeo=cfh_newgeo,&_temp256_cfh_newgeo) : &cfh_newgeo, cfsrmode)==FAILURE)
    {
...
FTGcveval( void )
{
    ...
    if( ampslice(_a_gzrf1ftg.fixedflag ? (_temp257_a_gzrf1ftg=a_gzrf1ftg,&_temp257_a_gzrf1ftg) : &a_gzrf1ftg, bw_rf1ftg, FTGopslthickz1, gscale_rf1ftg, TYPDEF)
        == FAILURE )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( ampslice(_a_gzrf2ftg.fixedflag ? (_temp258_a_gzrf2ftg=a_gzrf2ftg,&_temp258_a_gzrf2ftg) : &a_gzrf2ftg, bw_rf2ftg, FTGopslthickz2, gscale_rf2ftg, TYPDEF)
        == FAILURE )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( ampslice(_a_gzrf3ftg.fixedflag ? (_temp259_a_gzrf3ftg=a_gzrf3ftg,&_temp259_a_gzrf3ftg) : &a_gzrf3ftg, bw_rf3ftg, FTGopslthickz3, gscale_rf3ftg, TYPDEF)
        == FAILURE ) {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
                   EE_ARGS(1), STRING_ARG, "ampslice for gzrf3ftg.");
        return FAILURE;
    }
    if( optramp(_pw_gzrf1ftga.fixedflag ? (_temp260_pw_gzrf1ftga=pw_gzrf1ftga,&_temp260_pw_gzrf1ftga) : &pw_gzrf1ftga, a_gzrf1ftg, ps1loggrd.tz, ps1loggrd.zrt, TYPDEF)
        == FAILURE ) {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (optramp(_pw_gzrf2ftga.fixedflag ? (_temp261_pw_gzrf2ftga=pw_gzrf2ftga,&_temp261_pw_gzrf2ftga) : &pw_gzrf2ftga, a_gzrf2ftg, ps1loggrd.tz, ps1loggrd.zrt, TYPDEF)
        == FAILURE) {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( amppwgz1(_a_gz1ftg.fixedflag ? (_temp262_a_gz1ftg=a_gz1ftg,&_temp262_a_gz1ftg) : &a_gz1ftg, _pw_gz1ftg.fixedflag ? (_temp263_pw_gz1ftg=pw_gz1ftg,&_temp263_pw_gz1ftg) : &pw_gz1ftg, _pw_gz1ftga.fixedflag ? (_temp264_pw_gz1ftga=pw_gz1ftga,&_temp264_pw_gz1ftga) : &pw_gz1ftga, _pw_gz1ftgd.fixedflag ? (_temp265_pw_gz1ftgd=pw_gz1ftgd,&_temp265_pw_gz1ftgd) : &pw_gz1ftgd,
    ...
    if( amppwgz1(_a_gz2ftg.fixedflag ? (_temp266_a_gz2ftg=a_gz2ftg,&_temp266_a_gz2ftg) : &a_gz2ftg, _pw_gz2ftg.fixedflag ? (_temp267_pw_gz2ftg=pw_gz2ftg,&_temp267_pw_gz2ftg) : &pw_gz2ftg, _pw_gz2ftga.fixedflag ? (_temp268_pw_gz2ftga=pw_gz2ftga,&_temp268_pw_gz2ftga) : &pw_gz2ftga, _pw_gz2ftgd.fixedflag ? (_temp269_pw_gz2ftgd=pw_gz2ftgd,&_temp269_pw_gz2ftgd) : &pw_gz2ftgd,
    ...
    if( optramp(_pw_gzrf3ftga.fixedflag ? (_temp270_pw_gzrf3ftga=pw_gzrf3ftga,&_temp270_pw_gzrf3ftga) : &pw_gzrf3ftga, a_gzrf3ftg, ps1loggrd.tz, ps1loggrd.zrt, TYPDEF)
        == FAILURE ) {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( amppwgz1(_a_gz2bftg.fixedflag ? (_temp271_a_gz2bftg=a_gz2bftg,&_temp271_a_gz2bftg) : &a_gz2bftg, _pw_gz2bftg.fixedflag ? (_temp272_pw_gz2bftg=pw_gz2bftg,&_temp272_pw_gz2bftg) : &pw_gz2bftg, _pw_gz2bftga.fixedflag ? (_temp273_pw_gz2bftga=pw_gz2bftga,&_temp273_pw_gz2bftga) : &pw_gz2bftga, _pw_gz2bftgd.fixedflag ? (_temp274_pw_gz2bftgd=pw_gz2bftgd,&_temp274_pw_gz2bftgd) : &pw_gz2bftgd,
    ...
    if( amppwgz1(_a_gz3ftg.fixedflag ? (_temp275_a_gz3ftg=a_gz3ftg,&_temp275_a_gz3ftg) : &a_gz3ftg, _pw_gz3ftg.fixedflag ? (_temp276_pw_gz3ftg=pw_gz3ftg,&_temp276_pw_gz3ftg) : &pw_gz3ftg, _pw_gz3ftga.fixedflag ? (_temp277_pw_gz3ftga=pw_gz3ftga,&_temp277_pw_gz3ftga) : &pw_gz3ftga, _pw_gz3ftgd.fixedflag ? (_temp278_pw_gz3ftgd=pw_gz3ftgd,&_temp278_pw_gz3ftgd) : &pw_gz3ftgd,
    ...
    if( ampfov(_a_gxw1ftg.fixedflag ? (_temp279_a_gxw1ftg=a_gxw1ftg,&_temp279_a_gxw1ftg) : &a_gxw1ftg, echo1ftg_filt.bw, FTGfov) == FAILURE )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( amppwgx1(_a_gx1ftg.fixedflag ? (_temp280_a_gx1ftg=a_gx1ftg,&_temp280_a_gx1ftg) : &a_gx1ftg, _pw_gx1ftg.fixedflag ? (_temp281_pw_gx1ftg=pw_gx1ftg,&_temp281_pw_gx1ftg) : &pw_gx1ftg, _pw_gx1ftga.fixedflag ? (_temp282_pw_gx1ftga=pw_gx1ftga,&_temp282_pw_gx1ftga) : &pw_gx1ftga, _pw_gx1ftgd.fixedflag ? (_temp283_pw_gx1ftgd=pw_gx1ftgd,&_temp283_pw_gx1ftgd) : &pw_gx1ftgd,
    ...
    if( amppwgx1(_a_gx2test.fixedflag ? (_temp284_a_gx2test=a_gx2test,&_temp284_a_gx2test) : &a_gx2test, _pw_gx2test.fixedflag ? (_temp285_pw_gx2test=pw_gx2test,&_temp285_pw_gx2test) : &pw_gx2test, _pw_gx2testa.fixedflag ? (_temp286_pw_gx2testa=pw_gx2testa,&_temp286_pw_gx2testa) : &pw_gx2testa, _pw_gx2testd.fixedflag ? (_temp287_pw_gx2testd=pw_gx2testd,&_temp287_pw_gx2testd) : &pw_gx2testd,
                 TYPSPIN, area_gxw1ftg, .0,
                 (int)1000000, 1.0, MIN_PLATEAU_TIME,
                 ftg_xrt, ftg_tx_xz) == FAILURE )
    {
    ...
    if( amppwgx1(_a_gx1bftg.fixedflag ? (_temp288_a_gx1bftg=a_gx1bftg,&_temp288_a_gx1bftg) : &a_gx1bftg, _pw_gx1bftg.fixedflag ? (_temp289_pw_gx1bftg=pw_gx1bftg,&_temp289_pw_gx1bftg) : &pw_gx1bftg, _pw_gx1bftga.fixedflag ? (_temp290_pw_gx1bftga=pw_gx1bftga,&_temp290_pw_gx1bftga) : &pw_gx1bftga, _pw_gx1bftgd.fixedflag ? (_temp291_pw_gx1bftgd=pw_gx1bftgd,&_temp291_pw_gx1bftgd) : &pw_gx1bftgd,
                 TYPSPIN, area_gxw1ftg, .0,
                 (int)1000000, 1.0, MIN_PLATEAU_TIME,
    ...
    if( optramp(_pw_gxw1ftga.fixedflag ? (_temp292_pw_gxw1ftga=pw_gxw1ftga,&_temp292_pw_gxw1ftga) : &pw_gxw1ftga, a_gxw1ftg, ftg_tx,
    ...
    if( amppwgx1(_a_postgxw1ftg.fixedflag ? (_temp293_a_postgxw1ftg=a_postgxw1ftg,&_temp293_a_postgxw1ftg) : &a_postgxw1ftg, _pw_postgxw1ftg.fixedflag ? (_temp294_pw_postgxw1ftg=pw_postgxw1ftg,&_temp294_pw_postgxw1ftg) : &pw_postgxw1ftg, _pw_postgxw1ftga.fixedflag ? (_temp295_pw_postgxw1ftga=pw_postgxw1ftga,&_temp295_pw_postgxw1ftga) : &pw_postgxw1ftga,
                 _pw_postgxw1ftgd.fixedflag ? (_temp296_pw_postgxw1ftgd=pw_postgxw1ftgd,&_temp296_pw_postgxw1ftgd) : &pw_postgxw1ftgd, TYPSPIN, area_postgxw1ftg, .0,
                 (int)1000000, 1.0, MIN_PLATEAU_TIME,
    ...
    if( ampfov(_a_gxw2ftg.fixedflag ? (_temp297_a_gxw2ftg=a_gxw2ftg,&_temp297_a_gxw2ftg) : &a_gxw2ftg, echo1ftg_filt.bw, FTGfov) == FAILURE )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( optramp(_pw_gxw2ftga.fixedflag ? (_temp298_pw_gxw2ftga=pw_gxw2ftga,&_temp298_pw_gxw2ftga) : &pw_gxw2ftga, a_gxw2ftg, ftg_tx,
                ftg_xrt, TYPDEF) == FAILURE )
    { 
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( amppwgrad(area_gx2ftg, ftg_tx_xz,
                  0.0, 0.0, ftg_xrt,
                  MIN_PLATEAU_TIME, _a_gx2ftg.fixedflag ? (_temp299_a_gx2ftg=a_gx2ftg,&_temp299_a_gx2ftg) : &a_gx2ftg, _pw_gx2ftga.fixedflag ? (_temp300_pw_gx2ftga=pw_gx2ftga,&_temp300_pw_gx2ftga) : &pw_gx2ftga,
                  _pw_gx2ftg.fixedflag ? (_temp301_pw_gx2ftg=pw_gx2ftg,&_temp301_pw_gx2ftg) : &pw_gx2ftg, _pw_gx2ftgd.fixedflag ? (_temp302_pw_gx2ftgd=pw_gx2ftgd,&_temp302_pw_gx2ftgd) : &pw_gx2ftgd) == FAILURE ) {
...
XTGcveval( void )
{
    ...
    if(PSD_ON == local_tg)
    {
        ...
        if( ampslice(_a_gyrf1xtg.fixedflag ? (_temp303_a_gyrf1xtg=a_gyrf1xtg,&_temp303_a_gyrf1xtg) : &a_gyrf1xtg, bw_rf1xtg, TGopslthicky, gscale_rf1xtg, TYPDEF)
            == FAILURE )
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
        ...
        if( optramp(_pw_gyrf1xtga.fixedflag ? (_temp304_pw_gyrf1xtga=pw_gyrf1xtga,&_temp304_pw_gyrf1xtga) : &pw_gyrf1xtga, a_gyrf1xtg, ps1loggrd.ty, ps1loggrd.yrt, TYPDEF)
            == FAILURE )
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    else
    {
        ...
        if( ampslice(_a_gzrf1xtg.fixedflag ? (_temp305_a_gzrf1xtg=a_gzrf1xtg,&_temp305_a_gzrf1xtg) : &a_gzrf1xtg, bw_rf1xtg, TGopslthick, gscale_rf1xtg, TYPDEF)
        == FAILURE )
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
        ...
        if( optramp(_pw_gzrf1xtga.fixedflag ? (_temp306_pw_gzrf1xtga=pw_gzrf1xtga,&_temp306_pw_gzrf1xtga) : &pw_gzrf1xtga, a_gzrf1xtg, ps1loggrd.tz, ps1loggrd.zrt, TYPDEF)
            == FAILURE )
        {
            epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( ampslice(_a_gzrf2xtg.fixedflag ? (_temp307_a_gzrf2xtg=a_gzrf2xtg,&_temp307_a_gzrf2xtg) : &a_gzrf2xtg, bw_rf2xtg, TGopslthick, gscale_rf2xtg, TYPDEF)
        == FAILURE )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (optramp(_pw_gzrf2xtga.fixedflag ? (_temp308_pw_gzrf2xtga=pw_gzrf2xtga,&_temp308_pw_gzrf2xtga) : &pw_gzrf2xtga, a_gzrf2xtg, ps1loggrd.tz, ps1loggrd.zrt, TYPDEF)
        == FAILURE)
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (amppwgrad(area_xtgzkiller, xtg_tz_xyz, 0.0, 0.0, ps1loggrd.zrt,
                  MIN_PLATEAU_TIME, _a_gz2xtg.fixedflag ? (_temp309_a_gz2xtg=a_gz2xtg,&_temp309_a_gz2xtg) : &a_gz2xtg, _pw_gz2xtga.fixedflag ? (_temp310_pw_gz2xtga=pw_gz2xtga,&_temp310_pw_gz2xtga) : &pw_gz2xtga,
                  _pw_gz2xtg.fixedflag ? (_temp311_pw_gz2xtg=pw_gz2xtg,&_temp311_pw_gz2xtg) : &pw_gz2xtg, _pw_gz2xtgd.fixedflag ? (_temp312_pw_gz2xtgd=pw_gz2xtgd,&_temp312_pw_gz2xtgd) : &pw_gz2xtgd) == FAILURE)
    {
    ...
    if (amppwgrad((area_xtgzkiller-area_g1xtg), xtg_tz_xyz, 0.0, 0.0, ps1loggrd.zrt,
                  MIN_PLATEAU_TIME, _a_gz1xtg.fixedflag ? (_temp313_a_gz1xtg=a_gz1xtg,&_temp313_a_gz1xtg) : &a_gz1xtg, _pw_gz1xtga.fixedflag ? (_temp314_pw_gz1xtga=pw_gz1xtga,&_temp314_pw_gz1xtga) : &pw_gz1xtga,
                  _pw_gz1xtg.fixedflag ? (_temp315_pw_gz1xtg=pw_gz1xtg,&_temp315_pw_gz1xtg) : &pw_gz1xtg, _pw_gz1xtgd.fixedflag ? (_temp316_pw_gz1xtgd=pw_gz1xtgd,&_temp316_pw_gz1xtgd) : &pw_gz1xtgd) == FAILURE) 
    {
        epic_error(use_ermes, "%s failed in XTGcveval.",
                   EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
    ...
    if( ampfov(_a_gxw1xtg.fixedflag ? (_temp317_a_gxw1xtg=a_gxw1xtg,&_temp317_a_gxw1xtg) : &a_gxw1xtg, echo1xtg_filt.bw, XTGfov) == FAILURE )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if( optramp(_pw_gxw1xtga.fixedflag ? (_temp318_pw_gxw1xtga=pw_gxw1xtga,&_temp318_pw_gxw1xtga) : &pw_gxw1xtga, a_gxw1xtg, xtg_tx,
    ...
    if( amppwgx1(_a_gx1bxtg.fixedflag ? (_temp319_a_gx1bxtg=a_gx1bxtg,&_temp319_a_gx1bxtg) : &a_gx1bxtg, _pw_gx1bxtg.fixedflag ? (_temp320_pw_gx1bxtg=pw_gx1bxtg,&_temp320_pw_gx1bxtg) : &pw_gx1bxtg, _pw_gx1bxtga.fixedflag ? (_temp321_pw_gx1bxtga=pw_gx1bxtga,&_temp321_pw_gx1bxtga) : &pw_gx1bxtga, _pw_gx1bxtgd.fixedflag ? (_temp322_pw_gx1bxtgd=pw_gx1bxtgd,&_temp322_pw_gx1bxtgd) : &pw_gx1bxtgd,
    ...
    if (amppwgrad(area_xtgykiller-area_g1xtg, xtg_ty_xyz, 0.0, 0.0, xtg_yrt,
                  MIN_PLATEAU_TIME, _a_gykxtgl.fixedflag ? (_temp323_a_gykxtgl=a_gykxtgl,&_temp323_a_gykxtgl) : &a_gykxtgl, _pw_gykxtgla.fixedflag ? (_temp324_pw_gykxtgla=pw_gykxtgla,&_temp324_pw_gykxtgla) : &pw_gykxtgla,
                  _pw_gykxtgl.fixedflag ? (_temp325_pw_gykxtgl=pw_gykxtgl,&_temp325_pw_gykxtgl) : &pw_gykxtgl, _pw_gykxtgld.fixedflag ? (_temp326_pw_gykxtgld=pw_gykxtgld,&_temp326_pw_gykxtgld) : &pw_gykxtgld) == FAILURE) {
    ...
    if (amppwgrad(area_xtgykiller, xtg_ty_xyz, 0.0, 0.0, xtg_yrt,
                  MIN_PLATEAU_TIME, _a_gykxtgr.fixedflag ? (_temp327_a_gykxtgr=a_gykxtgr,&_temp327_a_gykxtgr) : &a_gykxtgr, _pw_gykxtgra.fixedflag ? (_temp328_pw_gykxtgra=pw_gykxtgra,&_temp328_pw_gykxtgra) : &pw_gykxtgra,
                  _pw_gykxtgr.fixedflag ? (_temp329_pw_gykxtgr=pw_gykxtgr,&_temp329_pw_gykxtgr) : &pw_gykxtgr, _pw_gykxtgrd.fixedflag ? (_temp330_pw_gykxtgrd=pw_gykxtgrd,&_temp330_pw_gykxtgrd) : &pw_gykxtgrd) == FAILURE) {
        epic_error(use_ermes, "%s failed in xtg.", EM_PSD_SUPPORT_FAILURE,
...
RScveval( void )
{
 ...
 ((void)(RUP_GRD(pw_rf1rs/2.0+(float)rs_off90*pw_rf1rs/rfpulse[RF1_RFSHIM_SLOT].nom_pw)), rs_iso_delay) : RUP_GRD(pw_rf1rs/2.0+(float)rs_off90*pw_rf1rs/rfpulse[RF1_RFSHIM_SLOT].nom_pw);
    ...
    if (FAILURE==ampslice(_a_gzrf1rs.fixedflag ? (_temp331_a_gzrf1rs=a_gzrf1rs,&_temp331_a_gzrf1rs) : &a_gzrf1rs, bw_rf1rs, rfshim_slthick, gscale_rf1rs, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==optramp(_pw_gzrf1rsa.fixedflag ? (_temp332_pw_gzrf1rsa=pw_gzrf1rsa,&_temp332_pw_gzrf1rsa) : &pw_gzrf1rsa, a_gzrf1rs, rsloggrd.tz_xyz, rsloggrd.zrt, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==amppwgrad(area_gzkbsrs, rsloggrd.tz_xyz, 0.0, 0.0, rsloggrd.zrt,
                           MIN_PLATEAU_TIME, _a_gzkbsrs.fixedflag ? (_temp333_a_gzkbsrs=a_gzkbsrs,&_temp333_a_gzkbsrs) : &a_gzkbsrs, _pw_gzkbsrsa.fixedflag ? (_temp334_pw_gzkbsrsa=pw_gzkbsrsa,&_temp334_pw_gzkbsrsa) : &pw_gzkbsrsa,
                           _pw_gzkbsrs.fixedflag ? (_temp335_pw_gzkbsrs=pw_gzkbsrs,&_temp335_pw_gzkbsrs) : &pw_gzkbsrs, _pw_gzkbsrsd.fixedflag ? (_temp336_pw_gzkbsrsd=pw_gzkbsrsd,&_temp336_pw_gzkbsrsd) : &pw_gzkbsrsd ))
    {
    ...
    if (amppwgz1(_a_gz1rs.fixedflag ? (_temp337_a_gz1rs=a_gz1rs,&_temp337_a_gz1rs) : &a_gz1rs,_pw_gz1rs.fixedflag ? (_temp338_pw_gz1rs=pw_gz1rs,&_temp338_pw_gz1rs) : &pw_gz1rs,_pw_gz1rsa.fixedflag ? (_temp339_pw_gz1rsa=pw_gz1rsa,&_temp339_pw_gz1rsa) : &pw_gz1rsa,_pw_gz1rsd.fixedflag ? (_temp340_pw_gz1rsd=pw_gz1rsd,&_temp340_pw_gz1rsd) : &pw_gz1rsd,area_gz1rs,
    ...
    if (FAILURE==amppwgrad(area_gzkrs, rsloggrd.tz_xyz, 0.0, 0.0, rsloggrd.zrt,
                           MIN_PLATEAU_TIME, _a_gzkrs.fixedflag ? (_temp341_a_gzkrs=a_gzkrs,&_temp341_a_gzkrs) : &a_gzkrs, _pw_gzkrsa.fixedflag ? (_temp342_pw_gzkrsa=pw_gzkrsa,&_temp342_pw_gzkrsa) : &pw_gzkrsa,
                           _pw_gzkrs.fixedflag ? (_temp343_pw_gzkrs=pw_gzkrs,&_temp343_pw_gzkrs) : &pw_gzkrs, _pw_gzkrsd.fixedflag ? (_temp344_pw_gzkrsd=pw_gzkrsd,&_temp344_pw_gzkrsd) : &pw_gzkrsd ))
    {
        epic_error(use_ermes, "%s failed in RScveval.",
                   EM_PSD_SUPPORT_FAILURE,EE_ARGS(1),
    ...
    if ( FAILURE==ampfov(_a_gxwrs.fixedflag ? (_temp345_a_gxwrs=a_gxwrs,&_temp345_a_gxwrs) : &a_gxwrs, echo1rs_filt.bw, rfshim_fov) )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==optramp(_pw_gxwrsa.fixedflag ? (_temp346_pw_gxwrsa=pw_gxwrsa,&_temp346_pw_gxwrsa) : &pw_gxwrsa, a_gxwrs, rsloggrd.tx_xyz, rsloggrd.xrt, TYPDEF)) 
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==amppwgrad(area_gxkbsrs, rsloggrd.tx_xyz, 0.0, 0.0, rsloggrd.xrt,
                           MIN_PLATEAU_TIME, _a_gxkbsrs.fixedflag ? (_temp347_a_gxkbsrs=a_gxkbsrs,&_temp347_a_gxkbsrs) : &a_gxkbsrs, _pw_gxkbsrsa.fixedflag ? (_temp348_pw_gxkbsrsa=pw_gxkbsrsa,&_temp348_pw_gxkbsrsa) : &pw_gxkbsrsa,
                           _pw_gxkbsrs.fixedflag ? (_temp349_pw_gxkbsrs=pw_gxkbsrs,&_temp349_pw_gxkbsrs) : &pw_gxkbsrs, _pw_gxkbsrsd.fixedflag ? (_temp350_pw_gxkbsrsd=pw_gxkbsrsd,&_temp350_pw_gxkbsrsd) : &pw_gxkbsrsd ))
    {
    ...
    if (FAILURE==amppwgrad(area_gx1rs, rsloggrd.tx_xyz, 0.0, 0.0, rsloggrd.xrt,
                           MIN_PLATEAU_TIME, _a_gx1rs.fixedflag ? (_temp351_a_gx1rs=a_gx1rs,&_temp351_a_gx1rs) : &a_gx1rs, _pw_gx1rsa.fixedflag ? (_temp352_pw_gx1rsa=pw_gx1rsa,&_temp352_pw_gx1rsa) : &pw_gx1rsa,
                           _pw_gx1rs.fixedflag ? (_temp353_pw_gx1rs=pw_gx1rs,&_temp353_pw_gx1rs) : &pw_gx1rs, _pw_gx1rsd.fixedflag ? (_temp354_pw_gx1rsd=pw_gx1rsd,&_temp354_pw_gx1rsd) : &pw_gx1rsd ))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==amppwgrad(area_gxkrs, rsloggrd.tx_xyz, 0.0, 0.0, rsloggrd.xrt,
                           MIN_PLATEAU_TIME, _a_gxkrs.fixedflag ? (_temp355_a_gxkrs=a_gxkrs,&_temp355_a_gxkrs) : &a_gxkrs, _pw_gxkrsa.fixedflag ? (_temp356_pw_gxkrsa=pw_gxkrsa,&_temp356_pw_gxkrsa) : &pw_gxkrsa,
                           _pw_gxkrs.fixedflag ? (_temp357_pw_gxkrs=pw_gxkrs,&_temp357_pw_gxkrs) : &pw_gxkrs, _pw_gxkrsd.fixedflag ? (_temp358_pw_gxkrsd=pw_gxkrsd,&_temp358_pw_gxkrsd) : &pw_gxkrsd ))
    {
        epic_error(use_ermes, "%s failed in RScveval.",
                   EM_PSD_SUPPORT_FAILURE,EE_ARGS(1),
    ...
    if(rfshim_etl >=2)
    {
        ...
        if (FAILURE==amppwgrad(area_gx2rs, rsloggrd.tx_xy, 0.0, 0.0, rsloggrd.xrt,
                               MIN_PLATEAU_TIME, _a_gx2rs.fixedflag ? (_temp359_a_gx2rs=a_gx2rs,&_temp359_a_gx2rs) : &a_gx2rs, _pw_gx2rsa.fixedflag ? (_temp360_pw_gx2rsa=pw_gx2rsa,&_temp360_pw_gx2rsa) : &pw_gx2rsa,
                               _pw_gx2rs.fixedflag ? (_temp361_pw_gx2rs=pw_gx2rs,&_temp361_pw_gx2rs) : &pw_gx2rs, _pw_gx2rsd.fixedflag ? (_temp362_pw_gx2rsd=pw_gx2rsd,&_temp362_pw_gx2rsd) : &pw_gx2rsd ))
        {
    ...
    /* Scale the waveform amps for the phase encodes 
     * so each phase instruction jump is an integer step */
    if ( FAILURE==endview((int)(rfshim_yres), _endview_iamprs.fixedflag ? (_temp363_endview_iamprs=endview_iamprs,&_temp363_endview_iamprs) : &endview_iamprs) )
    {
        epic_error(use_ermes, supfailfmt, EM_PSD_SUPPORT_FAILURE,
    ...
    if ( FAILURE==amppwtpe(_a_gy1rsa.fixedflag ? (_temp364_a_gy1rsa=a_gy1rsa,&_temp364_a_gy1rsa) : &a_gy1rsa, _a_gy1rsb.fixedflag ? (_temp365_a_gy1rsb=a_gy1rsb,&_temp365_a_gy1rsb) : &a_gy1rsb, _pw_gy1rs.fixedflag ? (_temp366_pw_gy1rs=pw_gy1rs,&_temp366_pw_gy1rs) : &pw_gy1rs, _pw_gy1rsa.fixedflag ? (_temp367_pw_gy1rsa=pw_gy1rsa,&_temp367_pw_gy1rsa) : &pw_gy1rsa, _pw_gy1rsd.fixedflag ? (_temp368_pw_gy1rsd=pw_gy1rsd,&_temp368_pw_gy1rsd) : &pw_gy1rsd,
...
DTGcveval( void )
{
 ...
 ((void)(RUP_GRD(pw_rf1dtg/2.0+(float)dtg_off90*pw_rf1dtg/rfpulse[RF1_DYNTG_SLOT].nom_pw)), dtg_iso_delay) : RUP_GRD(pw_rf1dtg/2.0+(float)dtg_off90*pw_rf1dtg/rfpulse[RF1_DYNTG_SLOT].nom_pw);
    ...
    if (FAILURE==ampslice(_a_gzrf1dtg.fixedflag ? (_temp369_a_gzrf1dtg=a_gzrf1dtg,&_temp369_a_gzrf1dtg) : &a_gzrf1dtg, bw_rf1dtg, dynTG_slthick, gscale_rf1dtg, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==optramp(_pw_gzrf1dtga.fixedflag ? (_temp370_pw_gzrf1dtga=pw_gzrf1dtga,&_temp370_pw_gzrf1dtga) : &pw_gzrf1dtga, a_gzrf1dtg, dtgloggrd.tz_xyz, dtgloggrd.zrt, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==amppwgrad(area_gzkbsdtg, dtgloggrd.tz_xyz, 0.0, 0.0, dtgloggrd.zrt,
                           MIN_PLATEAU_TIME, _a_gzkbsdtg.fixedflag ? (_temp371_a_gzkbsdtg=a_gzkbsdtg,&_temp371_a_gzkbsdtg) : &a_gzkbsdtg, _pw_gzkbsdtga.fixedflag ? (_temp372_pw_gzkbsdtga=pw_gzkbsdtga,&_temp372_pw_gzkbsdtga) : &pw_gzkbsdtga,
                           _pw_gzkbsdtg.fixedflag ? (_temp373_pw_gzkbsdtg=pw_gzkbsdtg,&_temp373_pw_gzkbsdtg) : &pw_gzkbsdtg, _pw_gzkbsdtgd.fixedflag ? (_temp374_pw_gzkbsdtgd=pw_gzkbsdtgd,&_temp374_pw_gzkbsdtgd) : &pw_gzkbsdtgd ))
    {
    ...
    if (amppwgz1(_a_gz1dtg.fixedflag ? (_temp375_a_gz1dtg=a_gz1dtg,&_temp375_a_gz1dtg) : &a_gz1dtg,_pw_gz1dtg.fixedflag ? (_temp376_pw_gz1dtg=pw_gz1dtg,&_temp376_pw_gz1dtg) : &pw_gz1dtg,_pw_gz1dtga.fixedflag ? (_temp377_pw_gz1dtga=pw_gz1dtga,&_temp377_pw_gz1dtga) : &pw_gz1dtga,_pw_gz1dtgd.fixedflag ? (_temp378_pw_gz1dtgd=pw_gz1dtgd,&_temp378_pw_gz1dtgd) : &pw_gz1dtgd,area_gz1dtg,
    ...
    if (FAILURE==amppwgrad(area_gzkdtg, dtgloggrd.tz_xyz, 0.0, 0.0, dtgloggrd.zrt,
                           MIN_PLATEAU_TIME, _a_gzkdtg.fixedflag ? (_temp379_a_gzkdtg=a_gzkdtg,&_temp379_a_gzkdtg) : &a_gzkdtg, _pw_gzkdtga.fixedflag ? (_temp380_pw_gzkdtga=pw_gzkdtga,&_temp380_pw_gzkdtga) : &pw_gzkdtga,
                           _pw_gzkdtg.fixedflag ? (_temp381_pw_gzkdtg=pw_gzkdtg,&_temp381_pw_gzkdtg) : &pw_gzkdtg, _pw_gzkdtgd.fixedflag ? (_temp382_pw_gzkdtgd=pw_gzkdtgd,&_temp382_pw_gzkdtgd) : &pw_gzkdtgd ))
    {
        epic_error(use_ermes, "%s failed in DTGcveval.",
                   EM_PSD_SUPPORT_FAILURE,EE_ARGS(1),
    ...
    if ( FAILURE==ampfov(_a_gxwdtg.fixedflag ? (_temp383_a_gxwdtg=a_gxwdtg,&_temp383_a_gxwdtg) : &a_gxwdtg, echo1dtg_filt.bw, dynTG_fov) )
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==optramp(_pw_gxwdtga.fixedflag ? (_temp384_pw_gxwdtga=pw_gxwdtga,&_temp384_pw_gxwdtga) : &pw_gxwdtga, a_gxwdtg, dtgloggrd.tx_xyz, dtgloggrd.xrt, TYPDEF)) 
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==amppwgrad(area_gxkbsdtg, dtgloggrd.tx_xyz, 0.0, 0.0, dtgloggrd.xrt,
                           MIN_PLATEAU_TIME, _a_gxkbsdtg.fixedflag ? (_temp385_a_gxkbsdtg=a_gxkbsdtg,&_temp385_a_gxkbsdtg) : &a_gxkbsdtg, _pw_gxkbsdtga.fixedflag ? (_temp386_pw_gxkbsdtga=pw_gxkbsdtga,&_temp386_pw_gxkbsdtga) : &pw_gxkbsdtga,
                           _pw_gxkbsdtg.fixedflag ? (_temp387_pw_gxkbsdtg=pw_gxkbsdtg,&_temp387_pw_gxkbsdtg) : &pw_gxkbsdtg, _pw_gxkbsdtgd.fixedflag ? (_temp388_pw_gxkbsdtgd=pw_gxkbsdtgd,&_temp388_pw_gxkbsdtgd) : &pw_gxkbsdtgd ))
    {
        epic_error(use_ermes, "%s failed in DTGcveval.",
                   EM_PSD_SUPPORT_FAILURE,EE_ARGS(1),
    ...
    if (FAILURE==amppwgrad(area_gx1dtg, dtgloggrd.tx_xyz, 0.0, 0.0, dtgloggrd.xrt,
                           MIN_PLATEAU_TIME, _a_gx1dtg.fixedflag ? (_temp389_a_gx1dtg=a_gx1dtg,&_temp389_a_gx1dtg) : &a_gx1dtg, _pw_gx1dtga.fixedflag ? (_temp390_pw_gx1dtga=pw_gx1dtga,&_temp390_pw_gx1dtga) : &pw_gx1dtga,
                           _pw_gx1dtg.fixedflag ? (_temp391_pw_gx1dtg=pw_gx1dtg,&_temp391_pw_gx1dtg) : &pw_gx1dtg, _pw_gx1dtgd.fixedflag ? (_temp392_pw_gx1dtgd=pw_gx1dtgd,&_temp392_pw_gx1dtgd) : &pw_gx1dtgd ))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    if (FAILURE==amppwgrad(area_gxkdtg, dtgloggrd.tx_xyz, 0.0, 0.0, dtgloggrd.xrt,
                           MIN_PLATEAU_TIME, _a_gxkdtg.fixedflag ? (_temp393_a_gxkdtg=a_gxkdtg,&_temp393_a_gxkdtg) : &a_gxkdtg, _pw_gxkdtga.fixedflag ? (_temp394_pw_gxkdtga=pw_gxkdtga,&_temp394_pw_gxkdtga) : &pw_gxkdtga,
                           _pw_gxkdtg.fixedflag ? (_temp395_pw_gxkdtg=pw_gxkdtg,&_temp395_pw_gxkdtg) : &pw_gxkdtg, _pw_gxkdtgd.fixedflag ? (_temp396_pw_gxkdtgd=pw_gxkdtgd,&_temp396_pw_gxkdtgd) : &pw_gxkdtgd ))
    {
        epic_error(use_ermes, "%s failed in DTGcveval.",
                   EM_PSD_SUPPORT_FAILURE,EE_ARGS(1),
    ...
    if(dynTG_etl >= 2)
    {
        ...
        if (FAILURE==amppwgrad(area_gx2dtg, dtgloggrd.tx_xy, 0.0, 0.0, dtgloggrd.xrt,
                               MIN_PLATEAU_TIME, _a_gx2dtg.fixedflag ? (_temp397_a_gx2dtg=a_gx2dtg,&_temp397_a_gx2dtg) : &a_gx2dtg, _pw_gx2dtga.fixedflag ? (_temp398_pw_gx2dtga=pw_gx2dtga,&_temp398_pw_gx2dtga) : &pw_gx2dtga,
                               _pw_gx2dtg.fixedflag ? (_temp399_pw_gx2dtg=pw_gx2dtg,&_temp399_pw_gx2dtg) : &pw_gx2dtg, _pw_gx2dtgd.fixedflag ? (_temp400_pw_gx2dtgd=pw_gx2dtgd,&_temp400_pw_gx2dtgd) : &pw_gx2dtgd ))
        {
            epic_error(use_ermes, "%s failed in DTGcveval.",
                       EM_PSD_SUPPORT_FAILURE,EE_ARGS(1),
    ...
    /* Scale the waveform amps for the phase encodes 
     * so each phase instruction jump is an integer step */
    if ( FAILURE==endview((int)(dynTG_yres), _endview_iampdtg.fixedflag ? (_temp401_endview_iampdtg=endview_iampdtg,&_temp401_endview_iampdtg) : &endview_iampdtg) )
    {
        epic_error(use_ermes, supfailfmt, EM_PSD_SUPPORT_FAILURE,
    ...
    if ( FAILURE==amppwtpe(_a_gy1dtga.fixedflag ? (_temp402_a_gy1dtga=a_gy1dtga,&_temp402_a_gy1dtga) : &a_gy1dtga, _a_gy1dtgb.fixedflag ? (_temp403_a_gy1dtgb=a_gy1dtgb,&_temp403_a_gy1dtgb) : &a_gy1dtgb, _pw_gy1dtg.fixedflag ? (_temp404_pw_gy1dtg=pw_gy1dtg,&_temp404_pw_gy1dtg) : &pw_gy1dtg, _pw_gy1dtga.fixedflag ? (_temp405_pw_gy1dtga=pw_gy1dtga,&_temp405_pw_gy1dtga) : &pw_gy1dtga, _pw_gy1dtgd.fixedflag ? (_temp406_pw_gy1dtgd=pw_gy1dtgd,&_temp406_pw_gy1dtgd) : &pw_gy1dtgd,
...
ExtCalcveval( void )
{
    ...
    if (ampslice(_a_gzrf1cal.fixedflag ? (_temp407_a_gzrf1cal=a_gzrf1cal,&_temp407_a_gzrf1cal) : &a_gzrf1cal, bw_rf1cal, cal_vthick, 1.0, TYPDEF) == FAILURE) {
    ...
    amp_rt_dbdtopt(_cal_amplimit.fixedflag ? (_temp408_cal_amplimit=cal_amplimit,&_temp408_cal_amplimit) : &cal_amplimit, &target_risetime, cal_slewrate, 1.0, calloggrd.tz);
    ...
    if (optramp(_pw_gzrf1cala.fixedflag ? (_temp409_pw_gzrf1cala=pw_gzrf1cala,&_temp409_pw_gzrf1cala) : &pw_gzrf1cala, a_gzrf1cal, target_amp, target_risetime, TYPDEF) == FAILURE) {
        epic_error(use_ermes, supfailfmt,
                   EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
    ...
    if (endview(cal_slq, _endviewz_iampcal.fixedflag ? (_temp410_endviewz_iampcal=endviewz_iampcal,&_temp410_endviewz_iampcal) : &endviewz_iampcal) == FAILURE)
    {
        epic_error(use_ermes,supfailfmt,EM_PSD_SUPPORT_FAILURE,
    ...
    amp_rt_dbdtopt(_cal_amplimit.fixedflag ? (_temp411_cal_amplimit=cal_amplimit,&_temp411_cal_amplimit) : &cal_amplimit, &target_risetime, cal_slewrate, 1.0, calloggrd.tz_xyz);
    ...
    if ( FAILURE==amppwtpe(_a_gzcombcala.fixedflag ? (_temp412_a_gzcombcala=a_gzcombcala,&_temp412_a_gzcombcala) : &a_gzcombcala, _a_gzcombcalb.fixedflag ? (_temp413_a_gzcombcalb=a_gzcombcalb,&_temp413_a_gzcombcalb) : &a_gzcombcalb, _pw_gzcombcal.fixedflag ? (_temp414_pw_gzcombcal=pw_gzcombcal,&_temp414_pw_gzcombcal) : &pw_gzcombcal, 
                           _pw_gzcombcala.fixedflag ? (_temp415_pw_gzcombcala=pw_gzcombcala,&_temp415_pw_gzcombcala) : &pw_gzcombcala, _pw_gzcombcald.fixedflag ? (_temp416_pw_gzcombcald=pw_gzcombcald,&_temp416_pw_gzcombcald) : &pw_gzcombcald,
    ...
    /* Z rewinder */
    if ( FAILURE==amppwtpe(_a_gzprcala.fixedflag ? (_temp417_a_gzprcala=a_gzprcala,&_temp417_a_gzprcala) : &a_gzprcala, _a_gzprcalb.fixedflag ? (_temp418_a_gzprcalb=a_gzprcalb,&_temp418_a_gzprcalb) : &a_gzprcalb, _pw_gzprcal.fixedflag ? (_temp419_pw_gzprcal=pw_gzprcal,&_temp419_pw_gzprcal) : &pw_gzprcal, 
                           _pw_gzprcala.fixedflag ? (_temp420_pw_gzprcala=pw_gzprcala,&_temp420_pw_gzprcala) : &pw_gzprcala, _pw_gzprcald.fixedflag ? (_temp421_pw_gzprcald=pw_gzprcald,&_temp421_pw_gzprcald) : &pw_gzprcald,
                           target_amp, target_risetime,
    ...
    if (FAILURE==optramp(_pw_gxwcala.fixedflag ? (_temp423_pw_gxwcala=pw_gxwcala,&_temp423_pw_gxwcala) : &pw_gxwcala, a_gxwcal, target_amp, target_risetime, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    amp_rt_dbdtopt(_cal_amplimit.fixedflag ? (_temp424_cal_amplimit=cal_amplimit,&_temp424_cal_amplimit) : &cal_amplimit, &target_risetime, cal_slewrate, 1.0, calloggrd.tx_xyz);
    if (amppwgx1(_a_gx1cal.fixedflag ? (_temp425_a_gx1cal=a_gx1cal,&_temp425_a_gx1cal) : &a_gx1cal, _pw_gx1cal.fixedflag ? (_temp426_pw_gx1cal=pw_gx1cal,&_temp426_pw_gx1cal) : &pw_gx1cal, _pw_gx1cala.fixedflag ? (_temp427_pw_gx1cala=pw_gx1cala,&_temp427_pw_gx1cala) : &pw_gx1cala, _pw_gx1cald.fixedflag ? (_temp428_pw_gx1cald=pw_gx1cald,&_temp428_pw_gx1cald) : &pw_gx1cald,
    ...
    /*    Y phase encode                           */
    /**********************************************/
    if (endview(cal_yres, _endview_iampcal.fixedflag ? (_temp429_endview_iampcal=endview_iampcal,&_temp429_endview_iampcal) : &endview_iampcal) == FAILURE)
    {
        epic_error(use_ermes,supfailfmt,EM_PSD_SUPPORT_FAILURE,
    ...
    amp_rt_dbdtopt(_cal_amplimit.fixedflag ? (_temp430_cal_amplimit=cal_amplimit,&_temp430_cal_amplimit) : &cal_amplimit, &target_risetime, cal_slewrate, 1.0, calloggrd.ty_xyz);
    ...
    if ( FAILURE==amppwtpe(_a_gy1cala.fixedflag ? (_temp431_a_gy1cala=a_gy1cala,&_temp431_a_gy1cala) : &a_gy1cala, _a_gy1calb.fixedflag ? (_temp432_a_gy1calb=a_gy1calb,&_temp432_a_gy1calb) : &a_gy1calb, _pw_gy1cal.fixedflag ? (_temp433_pw_gy1cal=pw_gy1cal,&_temp433_pw_gy1cal) : &pw_gy1cal, _pw_gy1cala.fixedflag ? (_temp434_pw_gy1cala=pw_gy1cala,&_temp434_pw_gy1cala) : &pw_gy1cala, _pw_gy1cald.fixedflag ? (_temp435_pw_gy1cald=pw_gy1cald,&_temp435_pw_gy1cald) : &pw_gy1cald,
                           target_amp, target_risetime,
...
AutoCoilcveval( void )
{
    ...
    if (ampslice(_a_gzrf1coil.fixedflag ? (_temp436_a_gzrf1coil=a_gzrf1coil,&_temp436_a_gzrf1coil) : &a_gzrf1coil, bw_rf1coil, coil_vthick, 1.0, TYPDEF) == FAILURE) {
        epic_error(use_ermes, supfailfmt,
    ...
    amp_rt_dbdtopt(_coil_amplimit.fixedflag ? (_temp437_coil_amplimit=coil_amplimit,&_temp437_coil_amplimit) : &coil_amplimit, &target_risetime, coil_slewrate, 1.0, coilloggrd.tz);
    ...
    if (optramp(_pw_gzrf1coila.fixedflag ? (_temp438_pw_gzrf1coila=pw_gzrf1coila,&_temp438_pw_gzrf1coila) : &pw_gzrf1coila, a_gzrf1coil, target_amp, target_risetime, TYPDEF) == FAILURE) {
        epic_error(use_ermes, supfailfmt,
                   EM_PSD_SUPPORT_FAILURE, EE_ARGS(1),
    ...
    if (endview(coil_slq, _endviewz_iampcoil.fixedflag ? (_temp439_endviewz_iampcoil=endviewz_iampcoil,&_temp439_endviewz_iampcoil) : &endviewz_iampcoil) == FAILURE)
    {
        epic_error(use_ermes,supfailfmt,EM_PSD_SUPPORT_FAILURE,
    ...
    amp_rt_dbdtopt(_coil_amplimit.fixedflag ? (_temp440_coil_amplimit=coil_amplimit,&_temp440_coil_amplimit) : &coil_amplimit, &target_risetime, coil_slewrate, 1.0, coilloggrd.tz_xyz);
    target_amp = coil_amplimit/endviewz_scalecal;
    target_risetime = RUP_GRD(ceil(target_risetime/endviewz_scalecal));  /* rise time for target amp */
    /* Z slice encode + gzrf1 refocus */
    if ( FAILURE==amppwtpe(_a_gzcombcoila.fixedflag ? (_temp441_a_gzcombcoila=a_gzcombcoila,&_temp441_a_gzcombcoila) : &a_gzcombcoila, _a_gzcombcoilb.fixedflag ? (_temp442_a_gzcombcoilb=a_gzcombcoilb,&_temp442_a_gzcombcoilb) : &a_gzcombcoilb, _pw_gzcombcoil.fixedflag ? (_temp443_pw_gzcombcoil=pw_gzcombcoil,&_temp443_pw_gzcombcoil) : &pw_gzcombcoil, 
                           _pw_gzcombcoila.fixedflag ? (_temp444_pw_gzcombcoila=pw_gzcombcoila,&_temp444_pw_gzcombcoila) : &pw_gzcombcoila, _pw_gzcombcoild.fixedflag ? (_temp445_pw_gzcombcoild=pw_gzcombcoild,&_temp445_pw_gzcombcoild) : &pw_gzcombcoild,
                           target_amp, target_risetime,
    ...
    /* Z rewinder */
    if ( FAILURE==amppwtpe(_a_gzprcoila.fixedflag ? (_temp446_a_gzprcoila=a_gzprcoila,&_temp446_a_gzprcoila) : &a_gzprcoila, _a_gzprcoilb.fixedflag ? (_temp447_a_gzprcoilb=a_gzprcoilb,&_temp447_a_gzprcoilb) : &a_gzprcoilb, _pw_gzprcoil.fixedflag ? (_temp448_pw_gzprcoil=pw_gzprcoil,&_temp448_pw_gzprcoil) : &pw_gzprcoil, 
                           _pw_gzprcoila.fixedflag ? (_temp449_pw_gzprcoila=pw_gzprcoila,&_temp449_pw_gzprcoila) : &pw_gzprcoila, _pw_gzprcoild.fixedflag ? (_temp450_pw_gzprcoild=pw_gzprcoild,&_temp450_pw_gzprcoild) : &pw_gzprcoild,
                           target_amp, target_risetime,
    ...
    if (FAILURE==optramp(_pw_gxwcoila.fixedflag ? (_temp452_pw_gxwcoila=pw_gxwcoila,&_temp452_pw_gxwcoila) : &pw_gxwcoila, a_gxwcoil, target_amp, target_risetime, TYPDEF))
    {
        epic_error(use_ermes, "%s failed", EM_PSD_SUPPORT_FAILURE,
    ...
    amp_rt_dbdtopt(_coil_amplimit.fixedflag ? (_temp453_coil_amplimit=coil_amplimit,&_temp453_coil_amplimit) : &coil_amplimit, &target_risetime, coil_slewrate, 1.0, coilloggrd.tx_xyz);
    if (amppwgx1(_a_gx1coil.fixedflag ? (_temp454_a_gx1coil=a_gx1coil,&_temp454_a_gx1coil) : &a_gx1coil, _pw_gx1coil.fixedflag ? (_temp455_pw_gx1coil=pw_gx1coil,&_temp455_pw_gx1coil) : &pw_gx1coil, _pw_gx1coila.fixedflag ? (_temp456_pw_gx1coila=pw_gx1coila,&_temp456_pw_gx1coila) : &pw_gx1coila, _pw_gx1coild.fixedflag ? (_temp457_pw_gx1coild=pw_gx1coild,&_temp457_pw_gx1coild) : &pw_gx1coild,
    ...
    /*    Y phase encode                           */
    /**********************************************/
    if (endview(coil_yres, _endview_iampcoil.fixedflag ? (_temp458_endview_iampcoil=endview_iampcoil,&_temp458_endview_iampcoil) : &endview_iampcoil) == FAILURE)
    {
        epic_error(use_ermes,supfailfmt,EM_PSD_SUPPORT_FAILURE,
    ...
    amp_rt_dbdtopt(_coil_amplimit.fixedflag ? (_temp459_coil_amplimit=coil_amplimit,&_temp459_coil_amplimit) : &coil_amplimit, &target_risetime, coil_slewrate, 1.0, coilloggrd.ty_xyz);
    ...
    if ( FAILURE==amppwtpe(_a_gy1coila.fixedflag ? (_temp460_a_gy1coila=a_gy1coila,&_temp460_a_gy1coila) : &a_gy1coila, _a_gy1coilb.fixedflag ? (_temp461_a_gy1coilb=a_gy1coilb,&_temp461_a_gy1coilb) : &a_gy1coilb, _pw_gy1coil.fixedflag ? (_temp462_pw_gy1coil=pw_gy1coil,&_temp462_pw_gy1coil) : &pw_gy1coil, _pw_gy1coila.fixedflag ? (_temp463_pw_gy1coila=pw_gy1coila,&_temp463_pw_gy1coila) : &pw_gy1coila, _pw_gy1coild.fixedflag ? (_temp464_pw_gy1coild=pw_gy1coild,&_temp464_pw_gy1coild) : &pw_gy1coild,
                           target_amp, target_risetime,
...
PS1predownload( void )
{
        ...
        || isRioSystem() || isHRMbSystem())
    {
        ...
        calcTGLimit(_tgcap.fixedflag ? (_temp465_tgcap=tgcap,&_temp465_tgcap) : &tgcap, _tgwindow.fixedflag ? (_temp466_tgwindow=tgwindow,&_temp466_tgwindow) : &tgwindow, maxB1Seq, txCoilInfo[getTxIndex(coilInfo[0])]);
    }
    else
    {
...
DTGpredownload( void )
{
...
#ifdef BURST_MODE_SUPPORT
    {
        ...
        int gradHeatFile_save = gradHeatFile;
        gradHeatFile  = _gradHeatFile.fixedflag ?  ((void)(FALSE), gradHeatFile) : FALSE;
        if ( FAILURE == minseq( _min_seqgrad.fixedflag ? (_temp475_min_seqgrad=min_seqgrad,&_temp475_min_seqgrad) : &min_seqgrad,
                                gradx, GX_FREE,
                                grady, GY_FREE,
                                gradz, GZ_FREE,
...
ExtCalpredownload( void )
{
    ...
    if (FAILURE == generateZyIndex( cal_zyindex,
                                    ...
                                    cal_pfkr_flag,
                                    cal_pfkr_fraction,
                                    _cal_sampledPts.fixedflag ? (_temp477_cal_sampledPts=cal_sampledPts,&_temp477_cal_sampledPts) : &cal_sampledPts ))
    {
        epic_error(use_ermes, "%s failed",
...
AutoCoilpredownload( void )
{
...
#ifdef BURST_MODE_SUPPORT
    {
        ...
        int gradHeatFile_save = gradHeatFile;
        gradHeatFile  = _gradHeatFile.fixedflag ?  ((void)(FALSE), gradHeatFile) : FALSE;
        if ( FAILURE == minseq( _min_seqgrad.fixedflag ? (_temp478_min_seqgrad=min_seqgrad,&_temp478_min_seqgrad) : &min_seqgrad,
                                gradx, GX_FREE,
                                grady, GY_FREE,
                                gradz, GZ_FREE,
    ...
    if (FAILURE == generateZyIndex( coil_zyindex,
                                    ...
                                    coil_pfkr_flag,
                                    coil_pfkr_fraction,
                                    _coil_sampledPts.fixedflag ? (_temp479_coil_sampledPts=coil_sampledPts,&_temp479_coil_sampledPts) : &coil_sampledPts ))
    {
        epic_error(use_ermes, "%s failed",
                   EM_PSD_SUPPORT_FAILURE, EE_ARGS(1), STRING_ARG, "generateZyIndex");
        return FAILURE;
...
STATUS syscheck(INT *p_syscheck_limit, int *p_status_flag)
{
  ...
  if (minseqgrddrv(&syscheck_minseqgrddrvx_t, _powerx.fixedflag ? (_temp480_powerx=powerx,&_temp480_powerx) : &powerx, _pospowerx.fixedflag ? (_temp481_pospowerx=pospowerx,&_temp481_pospowerx) : &pospowerx, _negpowerx.fixedflag ? (_temp482_negpowerx=negpowerx,&_temp482_negpowerx) : &negpowerx,
                   (int)GX_FREE, gradx,
...
if (minseqgrddrv(&syscheck_minseqgrddrvy_t, _powery.fixedflag ? (_temp483_powery=powery,&_temp483_powery) : &powery, _pospowery.fixedflag ? (_temp484_pospowery=pospowery,&_temp484_pospowery) : &pospowery, _negpowery.fixedflag ? (_temp485_negpowery=negpowery,&_temp485_negpowery) : &negpowery,
                   (int)GY_FREE, grady,
...
if (minseqgrddrv(&syscheck_minseqgrddrvz_t, _powerz.fixedflag ? (_temp486_powerz=powerz,&_temp486_powerz) : &powerz, _pospowerz.fixedflag ? (_temp487_pospowerz=pospowerz,&_temp487_pospowerz) : &pospowerz, _negpowerz.fixedflag ? (_temp488_negpowerz=negpowerz,&_temp488_negpowerz) : &negpowerz,
                   (int)GZ_FREE, gradz,
    ...
    if (minseqcoil(&syscheck_minseqcoil_t, _xa2s.fixedflag ? (_temp489_xa2s=xa2s,&_temp489_xa2s) : &xa2s, _ya2s.fixedflag ? (_temp490_ya2s=ya2s,&_temp490_ya2s) : &ya2s, _za2s.fixedflag ? (_temp491_za2s=za2s,&_temp491_za2s) : &za2s, cfsrmode,
                 gradx, grady, gradz,
    ...
    if (cfgradamp == 8920)
    {
        if (minseqcable(&syscheck_minseqcable_t, _xa2s.fixedflag ? (_temp492_xa2s=xa2s,&_temp492_xa2s) : &xa2s, _ya2s.fixedflag ? (_temp493_ya2s=ya2s,&_temp493_ya2s) : &ya2s, _za2s.fixedflag ? (_temp494_za2s=za2s,&_temp494_za2s) : &za2s, cfsrmode,
...
#include "pcasl3dgrase01.cvcopy.cpp"
