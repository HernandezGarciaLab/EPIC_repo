...
double 	ArtSup_Gmax = 1.0; /* default gradient max aplitude in arterial suppression pulse train */
double 	ArtSup_B1max = 200; /* default B1 max aplitude in arterial suppressin pulse train */
...
float	spiralGmax = 2.2; /* G/cm */
float 	myGmax_check = 0.0;
float 	spiralGxmax = 0.0;
float 	spiralGymax = 0.0;
float 	spiralGzmax = 0.0;

int	Ncenter = 20; /* number of gradient samples at the center of k-space*/
...
2*pw_gzspoila);
	...
	*/
	...
	while(slowDown>1.0)
	{
		fprintf(stderr,"\nCalling genspiral3d_io ... ");

		Grad_len = (int)(FID_dur/4.0); /* n. samples for gradient waveform */
		slowDown = genspiral3d_io(
			pfGx, pfGy, pfGz,
			Grad_len, nl, 1,
			gfov, gfov/(float)opxres,
			4.0e-6,
			myGmax,
			R_accel,
			THETA_accel,
		...
		FID_len = (int)(FID_dur / tsp);
		/* recompute the waveform with new duration */ 
		Grad_len = (int)(FID_dur/4.0); /* n. samples for gradient waveform */
		slowDown = genspiral3d_io(
			pfGx, pfGy, pfGz,
			Grad_len, nl, 1,
	...
	fprintf(stderr,"\nSuccessfully called genspiral3d_io ");
...
*/
	fprintf(stderr, "\n maxB1[L_SCAN]=%0.3f After  setScale", maxB1[L_SCAN]);
	...
	for (i = 0; i < nl; i++) {
		...
		*/
		...
		for (s = 0; s < opslquant; s++) {
			...
			if (fillrotations) xi += (float) i / nl * M_PI / (float)opslquant;

			if (doGoldenAngle){
				xi = (float) s * GoldenAngle;
				if (fillrotations) xi += (float) i / nl * GoldenAngle;
			}
...
*/
	...
	*/
	...
	INTWAVE(RHO,
			ASrf_mag,
			RUP_GRD(psd_rf_wait ),
			(float)doArtSup * ArtSup_B1max/my_maxB1Seq ,
			ArtSup_len,
			ArtSup_len*4,
			ArtSup_mag ,
			1,
...
void dodelay(   int* trig)
{
	...
	setiamp(ia_ASrf_mag * doArtSup, &ASrf_mag, 0);
	setiamp(ia_ASrf_grad * doArtSup, &ASrf_grad, 0);
	if(ifr<M0frames) setiamp(0, &ASrf_mag, 0);
...
//		slicen, rf_fraction,phase1, phase2, rf_sign);
	...
	//setiphase(FS_PI/2 , &echo1, 0); /*Bingo!!!!  LHG 7/16/21  */
	...
	*/
	...
	xi = (float)(slicen) * M_PI/(float)opslquant;
	if (fillrotations) xi += (float) leafn / nl * M_PI / (float)opslquant;

	if (doGoldenAngle){
		xi = (float) slicen * GoldenAngle;
		/* increase X rotation with each interleaf*/
		if (fillrotations) xi += (float) leafn / nl * GoldenAngle;
	}

	cxi = cos(xi); sxi = sin(xi);

	/* rotation about Z axis: interleaves*/
	/*
	|  cos    sin   0 |     |   |
	|  -sin    cos  0 |  x  | R |
	|  0       0    1 |     |   |
	*/
	...
	if (0)
	{
		/* old code:  first Rz, then Rx */
		/* first rotation: operate on savrot[] */
		for (k = 0; k < 9; k += 3)
		{
			tmpmat[0][k] =   IRINT( cphi * savrot[slicen][k] + sphi * savrot[slicen][k+1]);
			tmpmat[0][k+1] = IRINT(-sphi * savrot[slicen][k] + cphi * savrot[slicen][k+1]);
			tmpmat[0][k+2] = IRINT( savrot[slicen][k+2]);
		}

		/* second rotation: operate on tmpmat[] */
		for (k = 0; k < 9; k += 3)
		{
			rotmatx[0][k] = IRINT(tmpmat[0][k]);
			rotmatx[0][k+1] = IRINT(cxi*tmpmat[0][k+1] - sxi*tmpmat[0][k+2]);
			rotmatx[0][k+2] = IRINT(sxi*tmpmat[0][k+1] + cxi*tmpmat[0][k+2]);
		}
	}
	else
	{
