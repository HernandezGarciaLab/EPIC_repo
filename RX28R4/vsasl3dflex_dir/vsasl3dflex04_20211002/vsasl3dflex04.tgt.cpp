...
STATUS pulsegen(void)
{
			...
			
    {
        ...
        createinstr( &gzrf1a,(long)(RUP_GRD(tlead+pw_gzrf1a+psd_rf_wait)-pw_gzrf1a),
                     pw_gzrf1a,ia_gzrf1);
        createconst(&gzrf1,ZGRAD,pw_gzrf1,MAX_PG_WAMP);
        createinstr( &gzrf1,(long)(RUP_GRD(tlead+pw_gzrf1a+psd_rf_wait)),
                     pw_gzrf1,ia_gzrf1);
        createramp(&gzrf1d,ZGRAD,pw_gzrf1d,MAX_PG_WAMP,
                   (short)0,(short)(maxGradRes*(pw_gzrf1d/GRAD_UPDATE_TIME)),
                   pg_beta);
        createinstr( &gzrf1d,(long)(RUP_GRD(tlead+pw_gzrf1a+psd_rf_wait)+pw_gzrf1),
                     pw_gzrf1d,ia_gzrf1);
    ...
    trapezoid((WF_PROCESSOR)wg_gzrf1r, "gzrf1r", &gzrf1r, &gzrf1ra,
              ...
              ia_gzrf1r, 0, 0, 0, 0, RUP_GRD(tlead+psd_rf_wait+2*pw_gzrf1a+pw_gzrf1+pw_gzrf1ra)-pw_gzrf1ra, TRAP_ALL,
              &loggrd);
	    ...
	    
  {
    ...
    createseq(&tipdown_core,t_tipdown_core-timessi, off_tipdown_core);
#if defined(HOST_TGT)
    ...
    trapezoid((WF_PROCESSOR)wg_gz180crush1, "gz180crush1", &gz180crush1, &gz180crush1a,
              ...
              ia_gz180crush1, 0, 0, 0, 0, RUP_GRD(pw_gz180crush1a)-pw_gz180crush1a, TRAP_ALL,
              &loggrd);
			...
			
    {
        ...
        createramp(&gzrf2a,ZGRAD,pw_gzrf2a,(short)0,
                   MAX_PG_WAMP,(short)(maxGradRes*(pw_gzrf2a/
                       GRAD_UPDATE_TIME)),pg_beta);
        createinstr( &gzrf2a,(long)(pend(&gz180crush1d,"gz180crush1d",0)+pw_gzrf2a+tlead+psd_rf_wait-pw_gzrf2a),
                     pw_gzrf2a,ia_gzrf2);
        createconst(&gzrf2,ZGRAD,pw_gzrf2,MAX_PG_WAMP);
        createinstr( &gzrf2,(long)(pend(&gz180crush1d,"gz180crush1d",0)+pw_gzrf2a+tlead+psd_rf_wait),
                     pw_gzrf2,ia_gzrf2);
        createramp(&gzrf2d,ZGRAD,pw_gzrf2d,MAX_PG_WAMP,
                   (short)0,(short)(maxGradRes*(pw_gzrf2d/GRAD_UPDATE_TIME)),
                   pg_beta);
        createinstr( &gzrf2d,(long)(pend(&gz180crush1d,"gz180crush1d",0)+pw_gzrf2a+tlead+psd_rf_wait+pw_gzrf2),
                     pw_gzrf2d,ia_gzrf2);

        /* Now create the rf pulse */
        createsinc(&rf2,(WF_PROCESSOR)wg_rf2,res_rf2,
                   MAX_PG_WAMP,cyc_rf2, alpha_rf2);
        createinstr( &rf2,(long)(pend(&gz180crush1d,"gz180crush1d",0)+pw_gzrf2a+tlead+psd_rf_wait) + psd_rf_wait,
                     pw_rf2,ia_rf2);
        linkpulses(4,&rf2,&gzrf2,&gzrf2a,
                   &gzrf2d);
        addrfbits(&rf2,off_rf2,(long)(pend(&gz180crush1d,"gz180crush1d",0)+pw_gzrf2a+tlead+psd_rf_wait) + psd_rf_wait,
                  pw_rf2);
    }
  ...
  {
    pulsename(&refocus_core,"refocus_core");
    createseq(&refocus_core,t_refocus_core+textra-timessi, off_refocus_core);
#if defined(HOST_TGT)
    /* Update sequence counter and get current sequence entry index */
    updateIndex( &idx_refocus_core );
    printDebug( DBLEVEL1, (dbLevel_t)seg_debug, "SEQLENGTH",
                "idx_refocus_core = %d\n", idx_refocus_core );
#endif
  }
  ...
  {
	  pulsename(&gz,"gz");
	  createreserve(&gz, ZGRAD, res_gz);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_gz; i++)
			  ibuf[i] = Gz[i];
	  else 			/* load reversed */
		  for (i=0; i<res_gz; i++)
			  ibuf[i] = -Gz[(res_gz-i-1)];
	  movewaveimm(ibuf,&gz,(int)0,res_gz,TOHARDWARE);
	  setweos(EOS_DEAD,&gz,res_gz-1);



	  /* LHG 10.26.15  */
	  createinstr(&gz, (LONG)(readpos) , pw_gz, ia_gz);
	  if ((ZGRAD==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&gz, 0 , (LONG)(readpos) , pw_gz);
	  }
  }
...
*/
	       ...
	       
  {
    ...
    createseq(&seqcore,t_seqcore+textra-timessi, off_seqcore);
#if defined(HOST_TGT)
    /* Update sequence counter and get current sequence entry index */
  ...
  {
	  pulsename(&vsitag1,"vsitag1");
	  createreserve(&vsitag1, RHO, res_vsitag1);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_vsitag1; i++)
			  ibuf[i] = vsi_pulse_mag[i];
	  else 			/* load reversed */
		  for (i=0; i<res_vsitag1; i++)
			  ibuf[i] = -vsi_pulse_mag[(res_vsitag1-i-1)];
	  movewaveimm(ibuf,&vsitag1,(int)0,res_vsitag1,TOHARDWARE);
	  setweos(EOS_DEAD,&vsitag1,res_vsitag1-1);



	  /* LHG 10.26.15  */
	  createinstr(&vsitag1, (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsitag1, ia_vsitag1);
	  if ((RHO==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&vsitag1, 0 , (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsitag1);
	  }
  }
  ...
  {
	  pulsename(&vsitag1_theta,"vsitag1_theta");
	  createreserve(&vsitag1_theta, THETA, res_vsitag1_theta);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_vsitag1_theta; i++)
			  ibuf[i] = vsi_pulse_phs[i];
	  else 			/* load reversed */
		  for (i=0; i<res_vsitag1_theta; i++)
			  ibuf[i] = -vsi_pulse_phs[(res_vsitag1_theta-i-1)];
	  movewaveimm(ibuf,&vsitag1_theta,(int)0,res_vsitag1_theta,TOHARDWARE);
	  setweos(EOS_DEAD,&vsitag1_theta,res_vsitag1_theta-1);



	  /* LHG 10.26.15  */
	  createinstr(&vsitag1_theta, (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsitag1_theta, ia_vsitag1_theta);
	  if ((THETA==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&vsitag1_theta, 0 , (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsitag1_theta);
	  }
  }
  ...
  {
	  pulsename(&gztag1,"gztag1");
	  createreserve(&gztag1, ZGRAD, res_gztag1);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_gztag1; i++)
			  ibuf[i] = vsi_pulse_grad[i];
	  else 			/* load reversed */
		  for (i=0; i<res_gztag1; i++)
			  ibuf[i] = -vsi_pulse_grad[(res_gztag1-i-1)];
	  movewaveimm(ibuf,&gztag1,(int)0,res_gztag1,TOHARDWARE);
	  setweos(EOS_DEAD,&gztag1,res_gztag1-1);



	  /* LHG 10.26.15  */
	  createinstr(&gztag1, (LONG)(0) , pw_gztag1, ia_gztag1);
	  if ((ZGRAD==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&gztag1, 0 , (LONG)(0) , pw_gztag1);
	  }
  }
	       ...
	       
  {
    ...
    createseq(&astcore,RUP_RF(astseqtime+textra_astcore-timessi), off_astcore);
#if defined(HOST_TGT)
    /* Update sequence counter and get current sequence entry index */
  ...
  {
	  pulsename(&vsictl1,"vsictl1");
	  createreserve(&vsictl1, RHO, res_vsictl1);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_vsictl1; i++)
			  ibuf[i] = vsi_pulse_mag[i];
	  else 			/* load reversed */
		  for (i=0; i<res_vsictl1; i++)
			  ibuf[i] = -vsi_pulse_mag[(res_vsictl1-i-1)];
	  movewaveimm(ibuf,&vsictl1,(int)0,res_vsictl1,TOHARDWARE);
	  setweos(EOS_DEAD,&vsictl1,res_vsictl1-1);



	  /* LHG 10.26.15  */
	  createinstr(&vsictl1, (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsictl1, ia_vsictl1);
	  if ((RHO==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&vsictl1, 0 , (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsictl1);
	  }
  }
  ...
  {
	  pulsename(&vsictl1_theta,"vsictl1_theta");
	  createreserve(&vsictl1_theta, THETA, res_vsictl1_theta);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_vsictl1_theta; i++)
			  ibuf[i] = vsi_pulse_ctl_phs[i];
	  else 			/* load reversed */
		  for (i=0; i<res_vsictl1_theta; i++)
			  ibuf[i] = -vsi_pulse_ctl_phs[(res_vsictl1_theta-i-1)];
	  movewaveimm(ibuf,&vsictl1_theta,(int)0,res_vsictl1_theta,TOHARDWARE);
	  setweos(EOS_DEAD,&vsictl1_theta,res_vsictl1_theta-1);



	  /* LHG 10.26.15  */
	  createinstr(&vsictl1_theta, (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsictl1_theta, ia_vsictl1_theta);
	  if ((THETA==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&vsictl1_theta, 0 , (LONG)(RUP_GRD(psd_rf_wait)) , pw_vsictl1_theta);
	  }
  }
  ...
  {
	  pulsename(&gzctl1,"gzctl1");
	  createreserve(&gzctl1, ZGRAD, res_gzctl1);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_gzctl1; i++)
			  ibuf[i] = vsi_pulse_ctl_grad[i];
	  else 			/* load reversed */
		  for (i=0; i<res_gzctl1; i++)
			  ibuf[i] = -vsi_pulse_ctl_grad[(res_gzctl1-i-1)];
	  movewaveimm(ibuf,&gzctl1,(int)0,res_gzctl1,TOHARDWARE);
	  setweos(EOS_DEAD,&gzctl1,res_gzctl1-1);



	  /* LHG 10.26.15  */
	  createinstr(&gzctl1, (LONG)(0) , pw_gzctl1, ia_gzctl1);
	  if ((ZGRAD==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&gzctl1, 0 , (LONG)(0) , pw_gzctl1);
	  }
  }
	       ...
	       
  {
    ...
    createseq(&controlcore,RUP_RF(astseqtime+textra_astcore-timessi), off_controlcore);
#if defined(HOST_TGT)
    /* Update sequence counter and get current sequence entry index */
  ...
  {
    pulsename(&nothingcore,"nothingcore");
    createseq(&nothingcore,astseqtime+textra_astcore-timessi, off_nothingcore);
#if defined(HOST_TGT)
    /* Update sequence counter and get current sequence entry index */
    updateIndex( &idx_nothingcore );
    printDebug( DBLEVEL1, (dbLevel_t)seg_debug, "SEQLENGTH",
                "idx_nothingcore = %d\n", idx_nothingcore );
#endif
  }
  ...
  {
    pulsename(&vsi_gapcore,"vsi_gapcore");
    createseq(&vsi_gapcore,vsi_timegap+textra_vsi_gapcore-timessi, off_vsi_gapcore);
#if defined(HOST_TGT)
    /* Update sequence counter and get current sequence entry index */
    updateIndex( &idx_vsi_gapcore );
    printDebug( DBLEVEL1, (dbLevel_t)seg_debug, "SEQLENGTH",
                "idx_vsi_gapcore = %d\n", idx_vsi_gapcore );
#endif
  }
  ...
  {
	  pulsename(&ASrf_mag,"ASrf_mag");
	  createreserve(&ASrf_mag, RHO, res_ASrf_mag);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_ASrf_mag; i++)
			  ibuf[i] = ArtSup_mag[i];
	  else 			/* load reversed */
		  for (i=0; i<res_ASrf_mag; i++)
			  ibuf[i] = -ArtSup_mag[(res_ASrf_mag-i-1)];
	  movewaveimm(ibuf,&ASrf_mag,(int)0,res_ASrf_mag,TOHARDWARE);
	  setweos(EOS_DEAD,&ASrf_mag,res_ASrf_mag-1);



	  /* LHG 10.26.15  */
	  createinstr(&ASrf_mag, (LONG)(RUP_GRD(psd_rf_wait)) , pw_ASrf_mag, ia_ASrf_mag);
	  if ((RHO==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&ASrf_mag, 0 , (LONG)(RUP_GRD(psd_rf_wait)) , pw_ASrf_mag);
	  }
  }
  ...
  {
	  pulsename(&ASrf_theta,"ASrf_theta");
	  createreserve(&ASrf_theta, THETA, res_ASrf_theta);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_ASrf_theta; i++)
			  ibuf[i] = ArtSup_phs[i];
	  else 			/* load reversed */
		  for (i=0; i<res_ASrf_theta; i++)
			  ibuf[i] = -ArtSup_phs[(res_ASrf_theta-i-1)];
	  movewaveimm(ibuf,&ASrf_theta,(int)0,res_ASrf_theta,TOHARDWARE);
	  setweos(EOS_DEAD,&ASrf_theta,res_ASrf_theta-1);



	  /* LHG 10.26.15  */
	  createinstr(&ASrf_theta, (LONG)(RUP_GRD(psd_rf_wait)) , pw_ASrf_theta, ia_ASrf_theta);
	  if ((THETA==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&ASrf_theta, 0 , (LONG)(RUP_GRD(psd_rf_wait)) , pw_ASrf_theta);
	  }
  }
  ...
  {
	  pulsename(&ASrf_grad,"ASrf_grad");
	  createreserve(&ASrf_grad, ZGRAD, res_ASrf_grad);
	  if(1>0) 		/* load normal */
		  for (i=0; i<res_ASrf_grad; i++)
			  ibuf[i] = ArtSup_grad[i];
	  else 			/* load reversed */
		  for (i=0; i<res_ASrf_grad; i++)
			  ibuf[i] = -ArtSup_grad[(res_ASrf_grad-i-1)];
	  movewaveimm(ibuf,&ASrf_grad,(int)0,res_ASrf_grad,TOHARDWARE);
	  setweos(EOS_DEAD,&ASrf_grad,res_ASrf_grad-1);



	  /* LHG 10.26.15  */
	  createinstr(&ASrf_grad, (LONG)(0) , pw_ASrf_grad, ia_ASrf_grad);
	  if ((ZGRAD==TYPRHO1)) 
			/* || ($[int_wgname]==TYPRHO2) || ($[int_wgname]==TYPTHETA) || ($[int_wgname]==TYPOMEGA))*/
	  {
		  addrfbits(&ASrf_grad, 0 , (LONG)(0) , pw_ASrf_grad);
	  }
  }
			...
			
  {
    ...
    if ((wg_rf0==TYPRHO1)||(wg_rf0==TYPRHO2) ||
      ...
      {
        createinstr( &rf0,(long)(RUP_GRD(AStime-fatsattime+psd_rf_wait)),
                    pw_rf0,ia_rf0);
        addrfbits(&rf0,off_rf0,(long)(RUP_GRD(AStime-fatsattime+psd_rf_wait)),
                  pw_rf0);
      }
    else
      {
    ...
    trapezoid((WF_PROCESSOR)wg_gz0, "gz0", &gz0, &gz0a,
              ...
              ia_gz0, 0, 0, 0, 0, RUP_GRD(AStime-fatsattime+pwrf0+pw_gz0a)-pw_gz0a, TRAP_ALL,
              &loggrd);
  ...
  {
    pulsename(&art_fat_suppress_core,"art_fat_suppress_core");
    createseq(&art_fat_suppress_core,AStime-timessi, off_art_fat_suppress_core);
#if defined(HOST_TGT)
    /* Update sequence counter and get current sequence entry index */
    updateIndex( &idx_art_fat_suppress_core );
    printDebug( DBLEVEL1, (dbLevel_t)seg_debug, "SEQLENGTH",
                "idx_art_fat_suppress_core = %d\n", idx_art_fat_suppress_core );
#endif
  }
