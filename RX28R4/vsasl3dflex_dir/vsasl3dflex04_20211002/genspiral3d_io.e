		...
		)
{
	...
	float SLEWMAX = 18000; // max allowed slew rate in G/cm/s
	float tt;

	gammabar = gamma/2.0/M_PI;

	Kmax = 1.0/(fXres); /*error above?*/
	deltaK = 1.0/(fFOV);
	Nturns_theta = Kmax/deltaK/2.0;

	Nturns_theta = THETA_accel * Nturns_theta / Nshots_theta;

	Npoints -= Ncenter ; // make room for some zeroes in the middle of the trajectory
	
    
	/* Calculation using the spacing between the turns*/
	dr = 	2.0/Npoints;

	sfactor = M_PI/3;              // this factor slows down the theta rotation.
	dtheta = 2.0*sfactor/Npoints;  // but THEN we double the step size, so we can cover both the IN and the OUT
  ...
  // stay at the center of kspace for Ncenter points
	for (i=Npoints/2; i<Npoints/2+Ncenter ; i++)
	{
		r[i]=0.0;
		theta[i] = 0.0;
		phi[i] = 0.0;
	...
	for (i=0; i<Npoints/2; i++)
    {
        //tt goes from THETA_max down to 0
        tt = THETA_MAX*((float)(Npoints/2-i)) / (float)(Npoints/2.0);
        // the inflection point is 
        // r[i] *= (1 - 0.5*(1 + tanh(-tt*10 + 2*M_PI)));
	...
	*/
	...
	if ((maxgx/GMAX) >= slowFactor) slowFactor = (maxgx/GMAX);
	if ((maxgy/GMAX) >= slowFactor) slowFactor = (maxgy/GMAX);
	if ((maxgz/GMAX) >= slowFactor) slowFactor = (maxgz/GMAX);
    /*
	if ((maxslewx/SLEWMAX) >= slowFactor) slowFactor = (maxslewx/SLEWMAX);
	if ((maxslewy/SLEWMAX) >= slowFactor) slowFactor = (maxslewy/SLEWMAX);
	if ((maxslewz/SLEWMAX) >= slowFactor) slowFactor = (maxslewz/SLEWMAX);
    */
