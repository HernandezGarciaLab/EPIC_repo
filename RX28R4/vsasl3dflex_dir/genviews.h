		...
		float rotAnglex, float rotAngley, float rotAnglez)
{
	...
	float T[9], T_tmp[9], Rx[9], Ry[9], Rz[9];
	float Tz[9] = {1, 0, 0, 0, 1, 0, 0, 0, 1};
	char axis;
	FILE* f_kviews = fopen("./kviews.txt","wt");
	for (leafn = 0; leafn < N_leaves; leafn++) {
		for (slicen = 0; slicen < N_slices; slicen++) {
			...
			/* Reset translation matrix to initial */	
			for (k = 0; k < 9; k++) T[k] = T_0[k];

			/* Generate translation matrix for kz stepping and multiply it to translation matrix */
			Tz[8] = kzf;
			multmat(3,3,3,T,Tz,T_tmp);
			...
			/* Print translation matrix to file */
			for (k = 0; k < 9; k++) fprintf(f_kviews, "%f \t", T[k]);
			fprintf(f_kviews, "\n");
