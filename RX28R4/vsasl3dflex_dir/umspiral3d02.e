 ...
 *				9/8/14 - include new fields in the interface: phase_correction, domap
 */
/*@End*************************************************************/
...
int vdflag = 1 with {0,2,,VIS, "variable-density flag",};
...
/* int t_rfrepeat = 2500; */ /* pseudo-continuous labeling RF pulse spacing */
int 	tmin_rfrepeat;
...
abstract("Real Time, interactive, Pseudo CASL with 3D, Spiral acquisition for fMRI sequence");
psdname("pCASLrt3d06");
...
int cveval()
{
	...
	*/
	...
	/*LHG 9/8/14 
	cvdesc(opuser11,"Do Real Time mode (0=No, 1=Yes");
	cvmin(opuser11,0);
	cvmax(opuser11,1);
	cvdef(opuser11,0);
	opuser11 = 0;
	...
	*/	
	...
	cvdesc(opuser13, "vd (1) or cd (0), (2) for 3D SPINS");
	cvdef(opuser13, 0);
	opuser13 = 2;
	cvmin(opuser13, 0);
	cvmax(opuser13, 2);
	...
	*/
	...
	cvdesc(opuser24,"Use Flow Crushers (0=NO  1=YES)");
	cvmin(opuser24,0);
	cvmax(opuser24, 1);
	cvdef(opuser24,0);
	opuser24 = 0;
...
int predownload()
{
	...
	FILE *fpin;
        FILE *fpRot;
        float eulermat[9];
        float eulermat_array[opslquant*nl][9];
        float theta, phi;
	...
	*/
	...
	rf1_bw = 1000000.0 * cyc_rf1 / (float)(pw_rf1);
	/*
	pw_gzrf1 = (double)pwrf1;
	pw_gzrf1a = (double)myramptime;
	pw_gzrf1d = (double)myramptime;
	pw_gzrf1r = 200;  
	pw_gzrf1ra = (double)myramptime;
	pw_gzrf1rd = (double)myramptime;
	zfov = (double)opslquant*opslthick/10;
	a_gzrf1 =  (double)rf1_bw / (slab_fraction*zfov*GAMMA_H1/(2*3.14159)) ;
	area_gzrf1 = (double)a_gzrf1 * (pw_gzrf1 + pw_gzrf1d);
        area_gzrf1r = -(double)area_gzrf1 * 0.5;
	...
	*/
	...
	case 1:
		if(!(genspiralcdvd(gfov, opxres, tadmax*1.e-6, 4.e-6, alpha, kmaxfrac))) {
         ...
         case 2:
                        myaqtime = 0.040; /* ms*/
                        mydatasize = (int)(myaqtime*1e3*oprbw*2);
                        pw_gx = (int)(myaqtime*1e6);
                        pw_gy = (int)(myaqtime*1e6);
                        pw_gz = (int)(myaqtime*1e6);
                        /* insert new nl and opslquant calcualtions here: */
                        gres = (int)(myaqtime/4.0e-6);
                        fprintf(stderr, "\n gres is set to %d, and pw_gx is set to %d\n", gres, pw_gx);

                        fprintf(stderr,"\nCalling genspins3d ... ");

                        /* remember that gfov is in cm. */
                        /* Note to self... must ramp down to zero */
                        genspins3d(pfGx, pfGy, pfGz, gres, (float)opslquant, (float)nl, gfov, gfov/(float)opxres, 4.0e-6, opuser17);
                        fprintf(stderr,"\nSuccessfully called genspins3d ");

                        /* convert to DAC */
                        for (i=0; i<gres; i++)  {
                                Gx[i] = (2*(int)(.5*pfGx[i]*MAX_PG_WAMP/gamp));
                                Gy[i] = (2*(int)(.5*pfGy[i]*MAX_PG_WAMP/gamp));
                                Gz[i] = (2*(int)(.5*pfGz[i]*MAX_PG_WAMP/gamp));
                                
                                if(pfGx[i]>gamp) fprintf(stderr,"\nGx gamp violation: %f ", pfGx[i]);
                                if(pfGy[i]>gamp) fprintf(stderr,"\nGy gamp violation: %f ", pfGy[i]);
                                if(pfGz[i]>gamp) fprintf(stderr,"\nGz gamp violation: %f ", pfGz[i]);

                        }
                        Gx[0]=0; Gy[0] = 0; Gz[0]=0;

                        /* write out the rotation matrices for recon purposes */
                        fprintf(stderr, "\n writing rotation matrices to a file (still in predownload)");
                        fpRot = fopen("rotmats.txt","w");
                        ctr = 0;
                        for(i=0; i<opslquant; i++){
                                for(j=0; j<nl; j++){
                                        theta   = 2.0*M_PI*(i)/(float)opslquant;
                                        phi     = 2.0*M_PI*(j)/(float)nl;
                                        fprintf(stderr, "\ntheta is %f, phi is %f", theta, phi);
                                        euler2mat2(theta, 0.0, phi, eulermat);
                                        for (k = 0; k < 9; k += 3)
                                                fprintf (fpRot, "\n %f\t%f\t%f ", eulermat[k],  eulermat[k+1], eulermat[k+2]);

                                        /* stick them into an array for later */
                                        for (k = 0; k< 9; k++)
                                                eulermat_array[ctr][k] = eulermat[k];
                                        ctr++;
...
@inline loadrheader.e rheaderinit
	...
	*/
	...
	rhfrsize = (res_gx-RES_GRAMP)*4us/tsp;      /* num points sampled */
        if (vdflag == 2) rhfrsize = mydatasize;
        fprintf(stderr, "\nold rhrfsize %d",rhfrsize);
	rhfrsize = 4*(rhfrsize/4);          /* wants to be divisible by 4 */
        fprintf(stderr, "\nnew rhrfsize %d",rhfrsize);
...
@inline Prescan.e PSpredownload
	...
	/* needed for conversion from DV22 to DV23, see cookbook */
        eepf = 0;
        oepf = 0;
        eeff = 0;
        oeff = 0;
...
STATUS pulsegen(void)
{
	...
	float rdx, rdy;
	float cphi, sphi, x;
        long seqcore_time, readcore_time;
        int readTime;
        int RGwait;
...
/*
	...
	*/
	...
	TRAPEZOID(ZGRAD,
			...
			-.5*a_gzrf1*(pw_rf1+pw_gzrf1d),
			1,
			loggrd);

		
	waitloc = RUP_GRD(pendall(&gzrf1r,0));
	...
	/*fprintf(stderr, "\npulsegen: generating z phase-encode gradient... ");  Not Needed for 3D spins
	TRAPEZOID(ZGRAD,
		  gzphase1,
	...
	if(gtype<2)  {
        ...
        getseqtime(&seqcore_time);

        fprintf(stderr, "\nseqcore time is %d\n", seqcore_time);

	SEQLENGTH(seqcore, readpos , seqcore);
	...
	ACQUIREDATA(echo1, tlead+daqdel,,, ); 
	
	pulsename(&thetrec, "thetrec");
	createreserve(&thetrec, THETA, gres);
	createinstr(&thetrec, RUP_RF(tlead+daqdel+thetdeloff), pw_gx, max_pg_iamp);
...
*/
	...
	/* z phase-encode rewinder JFN 09.29.10 
		...
		-ia_gzphase,
                ...
                TRAP_ALL, &loggrd);
	------------------------------------------------------*/
/*	fprintf(stderr, "\npulsegen: generating z phase-encode rewinder... ");  Also dont need
	TRAPEZOID(ZGRAD,
		  gzphase2,
	...
	*/
	...
	TRAPEZOID(ZGRAD, 
		gzspoil, 
		RUP_GRD(pendall(&gx, 0)+pw_gzspoila),
		0,
		TYPNDEF, loggrd);
	...
	if(1)  {
		TRAPEZOID(XGRAD, 
			gxspoil, 
			RUP_GRD(pendall(&gzspoil,0)+pw_gxspoila),
			0,
			TYPNDEF, loggrd);
		TRAPEZOID(YGRAD, 
			gyspoil, 
			RUP_GRD(pendall(&gxspoil,0)+pw_gyspoila),
			0,
			TYPNDEF, loggrd);
	}
	...
	fprintf(stderr, "\ngres = %d, pw_gx =%d , pw_gy = %d",gres , pw_gx, pw_gy);
	fprintf(stderr, "\npsdseqtime, seqtr, tmin, optr=  %d   %d  %d  %d\n", psdseqtime,  seqtr,tmin,optr);

        fprintf(stderr, "\nSEQLENGTH is set to %d\n",psdseqtime-readpos+textra);
        fprintf(stderr, "all grads time is %d\n\n", pw_gx+tlead+3*pw_gzspoil+6*pw_gzspoila);
        
        readTime =  pw_gx + tlead 
		+3*pw_gzspoil + 6*pw_gzspoila
		+ 1ms ;
	
	SEQLENGTH(readcore, readTime ,readcore);
	getperiod(&readtime_core, &readcore, 0);
...
int doleaf(int leafn, int framen, int slicen, int* trig, int* bangn, int dabop, int dtype, float* eulermat);
/*---------------------------------------------------------
...
#define RESERVED_IPG_MEMORY (0xbfff00)
	...
	printf("\nEntering scancore   pre = %d\n", pre);
        printf("\nPrecomputing the rotation matrices for SPINS ");
        ctr = 0;
        for(i=0; i<opslquant; i++){
                for(j=0; j<nl; j++){
                        theta   = 2.0*M_PI*(i)/(float)opslquant;
	...
	if (mycrush) {
        ...
        /*   setting up frist rotation matrix    */
            	 for(k=0; k<9; k++) eulermat[k]=0.0;
                  eulermat[0] = 1.0; eulermat[4] = 1.0; eulermat[8]=1.0;
	...
	if(pre)  {
		...
		for (iv = 0; iv < ndisdaq; iv++) {	/*  disdaqs  */
			...
			if (vdflag != 2){
					...
					doleaf(0,0, opslquant/2 -1-isl, &trig, &bangn, dabop, DABOFF, eulermat);
					doleaf(0,0, opslquant/2 +isl,   &trig, &bangn, dabop, DABOFF, eulermat);
                		}
			}
			if (vdflag == 2){
				for (isl = 0; isl < opslquant; isl++){
					doleaf(0,0, isl,   &trig, &bangn, dabop, DABOFF, eulermat);
				}
			}
		...
		for (iv = 0; iv < 10000; iv++) {	/* get data  */
			...
			if (dopresat) {
				printf("\nDoing a dummy bang");
				doleaf(0, 0, opslquant/2-1, &trig, &bangn, dabop, DABOFF, eulermat);
				bangn-=1;
			}
			...
			if (vdflag != 2){
				for (isl = 0; isl < opslquant/2; isl++) {
					...
					doleaf(0,0, opslquant/2 -1-isl, &trig, &bangn, dabop, dtype, eulermat);
					doleaf(0,0, opslquant/2 +isl,   &trig, &bangn, dabop, dtype, eulermat);
				}
			}
			if (vdflag == 2){
				for (isl = 0; isl < opslquant; isl++){
					dtype = (pre==1 || isl==0)? DABON:DABOFF;
					doleaf(0,0, isl,   &trig, &bangn, dabop, dtype, eulermat);
				}
			}
                	getiamp(&chopamp, &rf1, 0);
			setiamp(-chopamp, &rf1, 0);
	...
	fprintf(stderr,"\ndomap = %d", domap);
        ...
        fprintf(stderr, "predownload(): area_gzrf1 = %f, area_gzrf1r = %f \n", area_gzrf1, area_gzrf1r);
        fprintf(stderr, "predownload()method2: area_gzrf1 = %f, area_gzrf1r = %f \n", a_gzrf1*(pw_gzrf1+pw_gzrf1d), a_gzrf1r*(pw_gzrf1r+pw_gzrf1rd));
        fprintf(stderr, "predownload(): pw_gzrf1 = %d, a_gzrf1 = %f, pw_rf1 = %d \n",pw_gzrf1, a_gzrf1, pw_rf1);
        fprintf(stderr, "predownload(): pw_gzrf1a = %d, pw_gzrf1d = %d \n",pw_gzrf1a, pw_gzrf1d);
        fprintf(stderr, "predownload(): pw_gzrf1r = %d, a_gzrf1r = %f \n",pw_gzrf1r, a_gzrf1r);
        fprintf(stderr, "predownload(): pw_gzrf1ra = %d, pw_gzrf1rd = %d \n",pw_gzrf1ra, pw_gzrf1rd);
		...
		for (iv = 0; iv < nl; iv++)  {
			...
			if (vdflag != 2){
				for (k = 0; k <9; k++)
                                      eulermat[k] = eulermat_array[bangn][k];
                                for (isl = 0; isl < opslquant/2; isl++) {
                                        doleaf(iv,ifr, opslquant/2 -1-isl, &trig, &bangn, dabop, DABON, eulermat);
                                        doleaf(iv,ifr, opslquant/2 +isl,   &trig, &bangn, dabop, DABON, eulermat);
                                }
                        }
			if (vdflag == 2){
				bangn = 0;
				for (isl = 0; isl < opslquant; isl++){
					...
					doleaf(iv,ifr, isl,   &trig, &bangn, dabop, DABON, eulermat);
				}
			}
 			if(opuser2 == 2) trig = TRIG_AUX;  	/* wait for trig */
	...
	*bangn = *bangn%nbang;      /*  for mps2  */
	...
	if (scluster==1)  {
		...
		if(slicen < nerror)		/* spread the error ... */
			tmp = readtime_core - trerror/nerror;	/* ... while keeping the sign */
	setperiod(RUP_GRD(tmp), &readcore, 0);
	...
	if (vdflag==2)
	{
        FILE *fid1;
        FILE *fid2;
        fid1 = fopen("rotmatsp.txt","a");
        fid2 = fopen("savrot.txt","w");
        fprintf(fid1, "\nslice number is %d, frame number is %d", slicen, framen);

		for (k = 0; k < 9; k += 3)
		{
		/*   LGH
                 	rotmatx_sp[0][k]   = IRINT(eulermat[0]*savrot[0][k] + eulermat[1]*savrot[0][k+1] + eulermat[2]*savrot[0][k+2]);
			rotmatx_sp[0][k+1] = IRINT(eulermat[3]*savrot[0][k] + eulermat[4]*savrot[0][k+1] + eulermat[5]*savrot[0][k+2]);
			rotmatx_sp[0][k+2] = IRINT(eulermat[6]*savrot[0][k] + eulermat[7]*savrot[0][k+1] + eulermat[8]*savrot[0][k+2]);
                */

                        /* RK */

                        rotmatx_sp[0][k]   = (int)(eulermat[0]*savrot[slicen][k] + eulermat[3]*savrot[slicen][k+1] + eulermat[6]*savrot[slicen][k+2]);
                        rotmatx_sp[0][k+1] = (int)(eulermat[1]*savrot[slicen][k] + eulermat[4]*savrot[slicen][k+1] + eulermat[7]*savrot[slicen][k+2]);
                        rotmatx_sp[0][k+2] = (int)(eulermat[2]*savrot[slicen][k] + eulermat[5]*savrot[slicen][k+1] + eulermat[8]*savrot[slicen][k+2]);


			/* fprintf (stderr, "\n |%f\t%f\t%f| ", eulermat[k],  eulermat[k+1], eulermat[k+2]); */
			/* fprintf (stderr, "\t |%f\t%f\t%f| ", rotmatx[0][k],  rotmatx[0][k+1], rotmatx[0][k+2]);  */
			/* fprintf (stderr, "\t |%f\t%f\t%f| ", rotmatx_sp[k],  rotmatx_sp[k+1], rotmatx_sp[k+2]); */
			 fprintf (fid1, "\n%d\t%d\t%d", rotmatx_sp[0][k],  rotmatx_sp[0][k+1], rotmatx_sp[0][k+2]);
                         fprintf (fid2, "\n%d\t%d\t%d", savrot[0][k],  savrot[0][k+1], savrot[0][k+2]);
			 fprintf (fid2, "\n%f\t%f\t%f\n", savrot[0][k],  savrot[0][k+1], savrot[0][k+2]);
		}
	...
	/* 3D spiral rotation matrix*/
	if (vdflag==2){
		scalerotmats(rotmatx_sp, &loggrd, &phygrd, 1, 0);
		setrotate(rotmatx_sp[0],slicen);
	}



	/* reset offset and go for it */
	boffset(off_readcore);
	startseq(slicen, MAY_PAUSE);
