...
*
*   vsasl3dflex05  8/12/20	major change:  goes back to stack of spiralsspiral-in/spiral-out centered on the spin echo
*				improved timing and replacing kx blips with kx waveform
*
*/
/*@End*************************************************************/
...
double 	vsi_Gcontrol = 1.5 ; /* default gradient max aplitude in control pulse train */
...
#define RESERVED_IPG_MEMORY (0xbfff00)
	...
	/* instead we'll make an outward ordering of the kz steps:
	0, -1, 1, -2, 2, -3, 3 ... */
	int count = 1;
    float dk;
	FILE *pfkzfile;
	pfkzfile = fopen("/usr/g/bin/kzsteps.txt","w");
...
//		slicen, rf_fraction,phase1, phase2, rf_sign);
	...
	*/
	...
	for (k = 0; k < 9; k += 3)
	{
		...
		rotmatx[0][k+2] = IRINT( savrot[slicen][k+2]);
	}
	...
	/*set the amplitude of the kz gradient*/
	setiamp( (int) (max_pg_iamp * kzfraction[slicen] * spiralGzmax / loggrd.zfs), &gz, 0);
